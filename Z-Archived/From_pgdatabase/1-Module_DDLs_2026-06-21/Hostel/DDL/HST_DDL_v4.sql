...
-- ─────────────────────────────────────────────────────────────────────────────
-- SECTION 1 : ALLOTMENT & OCCUPANCY MANAGEMENT
-- ─────────────────────────────────────────────────────────────────────────────
  ...
  -- Table Attendance session (Open Roll-Call Session
	...
	-- This table captures individual student attendance entries for each roll-call session, allowing for detailed tracking of attendance status (present, absent, leave, late) and related metadata (late remarks, check-in time). The use of a separate table for attendance entries enables efficient storage and querying of attendance data without needing to denormalize into the main attendance session table.
...
-- ─────────────────────────────────────────────────────────────────────────────
    -- This tbale captures daily Visitor Entries for hostel. Visitors can be parents, guardians, siblings, relatives or others. The table also captures details of the visit such as in-time, out-time, purpose of visit, etc. Additionally, it has fields to capture if the visitor was allowed outside visiting hours and the reason for the same. The visitor's photo and signed register can also be stored as media references.
	-- This table is not a Log table but it is a Transaction table.
	...
	-- This table captures the in/out movements of students for each day, including details about their departure and return times, destinations, purposes, and any gate passes issued. The inclusion of expected return time allows for monitoring overdue returns, while the is_overnight flag helps identify movements that span across midnight and may require different handling. Parent consent fields are included to support cases where parental approval is needed for certain movements, and the associated media ID allows for storing scanned consent forms or related documents. This comprehensive movement log supports both operational management and safety monitoring of students in the hostel.
	-- This table is not a Log table but it is a Transaction table.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- SECTION 3 : FQCILITY MANAGEMENT
-- ─────────────────────────────────────────────────────────────────────────────

	-- This table captures the inventory of items in each room, along with their condition and any damage reports. This allows hostel management to track the state of room furnishings and fixtures over time, identify maintenance needs, and assign responsibility for damages when applicable. The inclusion of visual evidence (photo_media_id) also helps in documenting the condition of items and supporting any repair or disciplinary actions that may arise from damage incidents.
	...
	-- Brijesh - Need to add hst_room_damage_reports table for reporting damages independent of inventory (e.g. for common areas, or when damage occurs but item-level inventory record not yet created). This would include fields like: id, room_id, reported_by_student_id, report_date, damage_description, photo_media_id, status (open/in_progress/resolved), resolution_description, resolved_by_staff_id, resolved_date, etc. This allows for a more flexible damage reporting workflow that isn't strictly tied to inventory items and can capture incidents in shared spaces or before formal inventory assessment. We can link to inventory records when applicable but not require it for every report.
	-- Incentory Items Damage may or may not chargable to the student. If chargeable, then we can create a damage report linked to the inventory item, and once the report is resolved with a cost, we can push that charge to the student's fee record (hst_fee_demands) and mark it in the inventory record as charge_pushed_to_fee = 1. This allows for a clear audit trail from damage incident to financial charge without mixing the operational details of damage reporting with the financial processing of charges.
	-- If Damage chargable then Entry will be made into Accounts module with reference to the demand raised in hst_fee_demands. This allows the Accounts module to handle all financial transactions and reporting, while the Hostel module manages the operational aspects of damage incidents and their associated charges.
	-- Charges may be applicable to a particuler Bed 9Student) or to the Room as a whole (e.g. if damage occurred in common area of the room and not attributable to a specific student). In either case, we can capture the responsible_student_id in the inventory record for individual items, and/or in the damage report for room-level incidents. This allows for flexibility in assigning responsibility while maintaining clear records for both operational management and financial processing.
	-- If Applicable to the Room then Charges will be split equally among all occupants of the room. This can be handled in the fee processing logic by looking up the current occupants of the room at the time of the damage report and dividing the charge accordingly when creating demands in hst_fee_demands.

    -- This table is not a Log table but it is a Transaction table.
	-- This table captures the lifecycle of maintenance issues related to beds and rooms, allowing hostel management to track reported problems, their severity, assignment to maintenance staff, resolution actions, and associated costs. The inclusion of status and severity fields linked to a dynamic status master allows for flexible categorization and tracking of maintenance tickets without requiring code changes for new statuses. The ability to block a bed until an issue is resolved helps ensure student safety and operational efficiency. Additionally, linking to media for before-and-after photos provides valuable documentation for maintenance work and supports accountability.
	...
	-- This table captures the daily housekeeping activities performed in each room and common area, allowing hostel management to track cleaning schedules, service types, staff assignments, quality ratings, and any issues found during inspections. The inclusion of both cleaned_by (staff name) and cleaned_by_user_id (sys_users.id) allows for flexibility in recording cleaning staff who may not be internal employees. The verified_by field enables tracking of supervisory inspections, while the quality_rating and issues_found fields support ongoing quality assurance efforts. Additionally, linking to media for before-and-after photos or evidence of issues provides valuable documentation for housekeeping operations.
	-- This table is not a Log table but it is a Transaction table.
	...
	-- Brijesh - Add floor_id to `hst_housekeeping_log` for better reporting and assignment of housekeeping duties. This allows us to track which floors are being serviced and to assign specific housekeeping staff to particular floors if needed. The floor_id will be a nullable foreign key referencing hst_floors, since some housekeeping tasks may be common-area specific rather than floor-specific.
	...
	-- Brijesh - `ticket_number` Calculation: The `ticket_number` can be generated using a combination of LT-YYYYMMDD-student_admission_number-HHMMSS
	...
	-- Add floor_id into `hst_complaints` table to allow for more granular location tracking of complaints, especially for common area issues that may not be tied to a specific room. This would involve adding `floor_id` as a nullable foreign key referencing `hst_floors.id`, and updating the complaint reporting interface to allow selection of floor when applicable. This enhancement would improve the ability to analyze complaint patterns by location and prioritize maintenance efforts accordingly.
	-- Brijesh - Add esclation matrix (1st levt to Warden, 2nd level to Chief Warden, 3rd level to Director of Hostels) and SLA tracking for complaints. This would involve adding fields like `escalation_level` (TINYINT), `escalated_to_user_id` (INT UNSIGNED), and `sla_due_at` (TIMESTAMP) to the `hst_complaints` table. The complaint management workflow would then include automatic escalation based on time thresholds and manual escalation options for staff, with clear tracking of who is responsible at each stage and when actions are due.

	-- This table serves as a universal audit log for all changes across the hostel management system. 
	-- Whenever a record in any hst_* table is created, updated, deleted, or has a status change, an entry should be made here with the relevant details. 
	-- The before_json and after_json fields allow for a complete snapshot of the record state pre- and post-change, while field_diff_json provides a concise view of what exactly changed. 
	-- This design supports comprehensive auditing, debugging, and historical tracking without needing separate audit tables for each model.
	-- This is a Log Table. System will enter data. No user Entry allowed in this table.
	CREATE TABLE IF NOT EXISTS `hst_audit_log` (
		...
		`before_json`    JSON             NULL,  -- Full record state before change (if applicable)
		`after_json`     JSON             NULL,  -- Full record state after change (if applicable)
		`field_diff_json` JSON            NULL  COMMENT 'Only changed fields with old/new values (compact)',  -- e.g., {"status": {"old": "pending", "new": "approved"}, "approved_by": {"old": null, "new": 123}}
	...
	-- Brijesh - This table serves as a universal audit log for all changes across the hostel management system. 
	-- Whenever a record in any hst_* table is created, updated, deleted, or has a status change, an entry should be made here with the relevant details. 
	-- The before_json and after_json fields allow for a complete snapshot of the record state pre- and post-change, while field_diff_json provides a concise view of what exactly changed. 
	-- This design supports comprehensive auditing, debugging, and historical tracking without needing separate audit tables for each model.

	-- This table can also be used to drive an admin UI for viewing change history, and to support features like undo/restore or change approval workflows in the future.
	-- This is a Log Table. System will enter data. No user Entry allowed in this table.
...
-- ─────────────────────────────────────────────────────────────────────────────
	...
	-- brijesh - Escalation will be automatic, can not be manually set. We can have a scheduled job that runs daily to check for any incidents that meet escalation criteria (e.g. not resolved within 7 days, or severity is "serious") and update the is_escalated flag and escalated_at timestamp accordingly. This ensures consistent application of escalation rules without relying on manual intervention.
	-- Warning Letter will be sent if Incident_type is set to "serious" or if an incident remains unresolved for more than 7 days. This can be handled in the same scheduled job that checks for escalations, by also checking the severity and resolution status of incidents and updating the warning_letter_sent flag accordingly. This allows for timely communication with students and parents about serious incidents without relying on manual tracking.
	-- Check other Field also

	-- This table captures media attachments related to discipline incidents. By linking each media item to a specific incident and categorizing it by type (e.g., photo, video, document), we can maintain a comprehensive record of all evidence and related materials for each incident. This can be invaluable for investigations, reporting, and historical reference. The is_active flag allows for soft deletion of media items without losing the association with the incident, preserving the integrity of the incident record.
	...
	-- This table captures sick bay admissions, which may or may not be linked to hospital referrals (via hpc_record_id). This allows us to track all health-related incidents in the hostel, even those that don't result in hospital visits, and to maintain a complete history of sick bay usage for each student. The is_hospital_referred flag and discharge notes also allow us to capture outcomes of each sick bay admission, which can be valuable for health monitoring and reporting purposes.
	-- This is not a Log Table but a Transaction Table. System will enter data as well as user entry allowed in this table.
...
-- Brijesh - if `mess_bill_credit_applied` = 1, then the monthly amount need to be credited back to the student’s mess bill for the relevant month(s). This allows for accurate billing adjustments based on meal opt-outs, and prevents double-crediting if the opt-out was already accounted for in a previous billing cycle. The status field allows for tracking the approval process of opt-out requests, and the is_recurring flag with recurrence_pattern_json enables support for regular meal opt-outs (e.g., fasting every Friday lunch) without needing multiple records.
	...
	CREATE TABLE IF NOT EXISTS `hst_mess_bills` (
		...
		`total_meals_planned` SMALLINT UNSIGNED NOT NULL,  -- Total meals the student was supposed to have based on their meal plan and days stayed in the hostel during the month
