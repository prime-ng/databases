...
-- -------------------------------------------------
  ...
  -- All period timings are defined here. Period sets (tt_period_set) and
  -- their items (tt_period_set_period_jnt) reference this table instead of
  ...
  -- -------------------------------------------------------------------------
  CREATE TABLE IF NOT EXISTS `tt_period_config` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `shift_id` TINYINT UNSIGNED NOT NULL          COMMENT 'FK → tt_shift.id. Timeslots can differ by shift (morning vs afternoon)',
    ...
    `period_type_id` TINYINT UNSIGNED NOT NULL    COMMENT 'FK → tt_period_type.id. Default type for this slot (TEACHING, BREAK, LUNCH, ASSEMBLY)',
    ...
    CONSTRAINT `fk_pc_shift` FOREIGN KEY (`shift_id`) REFERENCES `tt_shift` (`id`) ON DELETE RESTRICT,
    CONSTRAINT `fk_pc_period_type` FOREIGN KEY (`period_type_id`) REFERENCES `tt_period_type` (`id`) ON DELETE RESTRICT,
  ...
  -- Timeslots are now centralized in tt_period_config. This table only defines WHICH period range a class group uses.
  CREATE TABLE IF NOT EXISTS `tt_period_set` (
    ...
    `name` VARCHAR(100) NOT NULL,
    `description` VARCHAR(255) DEFAULT NULL,
    `shift_id` TINYINT UNSIGNED NOT NULL           COMMENT 'FK → tt_shift.id. Which shift timing grid this set uses (v7.7)',
    `from_period_ord` TINYINT UNSIGNED NOT NULL    COMMENT 'First slot_ord from tt_period_config that classes in this set attend (v7.7)',
    `to_period_ord` TINYINT UNSIGNED NOT NULL      COMMENT 'Last slot_ord from tt_period_config that classes in this set attend (v7.7)',
    ...
    CONSTRAINT `fk_periodset_shift` FOREIGN KEY (`shift_id`) REFERENCES `tt_shift` (`id`) ON DELETE RESTRICT,
    CONSTRAINT `chk_periodset_ord_range` CHECK (`to_period_ord` >= `from_period_ord`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='v7.7: Period set defines WHICH period range a class group uses. Timeslots come from tt_period_config.';
  -- Example:
  ...
  -- Timing is now inherited from tt_period_config. This table defines which slots from the master grid
  ...
  CREATE TABLE IF NOT EXISTS `tt_period_set_period_jnt` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `period_set_id` INT UNSIGNED NOT NULL         COMMENT 'FK → tt_period_set.id',
    `period_config_id` INT UNSIGNED NOT NULL      COMMENT 'FK → tt_period_config.id. Timeslot reference — timing comes from here (v7.7)',
    ...
    `period_type_id` INT UNSIGNED NOT NULL        COMMENT 'FK → tt_period_type.id. Can override tt_period_config default if needed',
    -- REMOVED : start_time, end_time, duration_minutes (now refere from tt_period_config)
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    ...
    CONSTRAINT `fk_psp_period_config` FOREIGN KEY (`period_config_id`) REFERENCES `tt_period_config` (`id`) ON DELETE RESTRICT,
    CONSTRAINT `fk_psp_period_type` FOREIGN KEY (`period_type_id`) REFERENCES `tt_period_type` (`id`) ON DELETE RESTRICT
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='v7.7: Maps which timeslots from tt_period_config are included in each period set. Timing is inherited, not stored.';
...
-- -------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `tt_teacher_unavailable` (
    ...
    `constraint_type_id` INT UNSIGNED DEFAULT NULL,  -- FK to tt_constraint.id
...
-- -----------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `tt_sub_activity` (
    ...
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `sub_activity_id` INT UNSIGNED NOT NULL,  -- FK to tt_sub_activity.id
...
-- New Changes:
  ...
  -- 4. Added `min_required_consecutive_periods` to `tt_requirement_consolidation` table
