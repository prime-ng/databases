...
CREATE TABLE IF NOT EXISTS `lms_quiz_quest_attempt_answers` (
  ...
  `attachment_data`       JSON DEFAULT NULL,                    -- File upload metadata: {file_name, file_path, file_size, mime_type}
  -- Evaluation
...
CREATE TABLE IF NOT EXISTS `lms_exam_attempts` (
  ...
  -- Offline Metadata
  `is_present_offline`          TINYINT(1) NOT NULL DEFAULT 1,  -- Attendance flag (Present / Absent)
  `answer_sheet_number`         VARCHAR(50) DEFAULT NULL,       -- Physical sheet reference
  `offline_paper_uploaded_path` JSON DEFAULT NULL,  
  -- Online Proctoring
...
-- Purpose: Entry for OFFLINE exams in BULK_TOTAL(marks_entry_mode = 'BULK_TOTAL') mode.
--          Teacher enters total marks obtained without per-question breakdown. One row per attempt (UNIQUE).
-- -------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `lms_offline_exam_upload_marks` (
  `id`                    INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `exam_attempt_id`       INT UNSIGNED NOT NULL,              -- FK → lms_exam_attempts.id (only for OFFLINE attempts with BULK_TOTAL mode)
  `marks_entry_mode`      ENUM('BULK_TOTAL','QUESTION_WISE') DEFAULT 'QUESTION_WISE', -- How marks will be entered
  ...
  -- Upload only (If marks_entry_mode = 'BULK_TOTAL', teacher can optionally upload single answer sheet and consolidated Mark for a particuler Paper Attempt.)
  `is_sheet_uploaded`     TINYINT(1) NULL DEFAULT 0,          -- Is Answer sheet uploaded by Teacher or not?
  `attachment_data`       JSON DEFAULT NULL,                  -- File upload metadata: {file_name, file_path, file_size, mime_type}
  `uploaded_by`           INT UNSIGNED NULL,                  -- FK → sys_users.id (teacher)
  `uploaded_at`           DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `upload_remarks`        VARCHAR(255) DEFAULT NULL, 
  -- Evaluation (Will be filled in both cases of BULK_TOTAL and QUESTION_WISE, but for BULK_TOTAL it will be the only source of marks entry, whereas for QUESTION_WISE, details will be filled in lms_offline_exam_upload_detail table.)
  `is_evaluated`          TINYINT(1) NOT NULL DEFAULT 0,        -- Flag to indicate if this answer has been evaluated (auto or manual)
  `max_paper_marks`       DECIMAL(5,2) DEFAULT NULL,            -- Cached from lms_paper_set_questions
  ...
  `deleted_at`            TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_offline_exme_attempt`  (`exam_attempt_id`),
  KEY `idx_exme_entered_by`             (`uploaded_by`),
  KEY `idx_exme_is_active`              (`is_active`),
  CONSTRAINT `fk_exme_attempt` FOREIGN KEY (`exam_attempt_id`) REFERENCES `lms_exam_attempts` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_exme_enterer` FOREIGN KEY (`uploaded_by`) REFERENCES `sys_users` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_evaluated_by` FOREIGN KEY (`evaluated_by`) REFERENCES `sys_users` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `chk_marks_entry_mode` CHECK ((marks_entry_mode = 'BULK_TOTAL' AND is_ques_wise_file_upload = 0) OR (marks_entry_mode = 'QUESTION_WISE' AND (is_ques_wise_file_upload IN (0,1))))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Bulk total marks entry for offline exams in BULK_TOTAL mode. One row per attempt.';
-- Condition
-- if marks_entry_mode = BULK_TOTAL then is_ques_wise_file_upload = 0.
-- if marks_entry_mode = QUESTION_WISE then is_ques_wise_file_upload = 0 OR is_ques_wise_file_upload = 1.

CREATE TABLE IF NOT EXISTS `lms_offline_exam_upload_detail` (
  `id`                     INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `offline_exam_upload_id` INT UNSIGNED NOT NULL,                -- FK → lms_offline_exam_upload_marks.id (only for OFFLINE attempts with QUESTION_WISE mode)
  `question_id`            INT UNSIGNED NOT NULL,                -- FK → qns_questions_bank.id 
  `question_type_id`       INT UNSIGNED DEFAULT NULL,            -- FK - slb_question_types.id (cached for evaluation logic)
  -- The Response
  `selected_option_id`     INT UNSIGNED DEFAULT NULL,            -- Single-MCQ
  `selected_option_ids`    JSON DEFAULT NULL,                    -- Multi-MCQ (array of option IDs)
  `attachment_data`        JSON DEFAULT NULL,                    -- File upload metadata: {file_name, file_path, file_size, mime_type}
  -- Evaluation per Question (Will be filled only for QUESTION_WISE mode)
  `marks_obtained_for_question` DECIMAL(5,2) NOT NULL DEFAULT 0.00, 
  `max_marks_for_question` DECIMAL(5,2) DEFAULT NULL,            -- Cached from lms_paper_set_questions
  `is_answer_correct`      ENUM('Correct','Incorrect','Partially-Correct','Evaluation-Pending','Graced') DEFAULT 'Evaluation-Pending',
  ...
  KEY `idx_exme_is_active`     (`is_active`),
  CONSTRAINT `fk_exme_attempt` FOREIGN KEY (`offline_exam_upload_id`) REFERENCES `lms_offline_exam_upload_marks` (`id`) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `lms_exam_grievances` (
  ...
  `exam_paper_id`         INT UNSIGNED NOT NULL,                -- FK → lms_exam_papers.id (redundant cache for fast query)
  ...
  CONSTRAINT `fk_exgrv_exam_paper` FOREIGN KEY (`exam_paper_id`) REFERENCES `lms_exam_papers` (`id`) ON DELETE SET NULL,
