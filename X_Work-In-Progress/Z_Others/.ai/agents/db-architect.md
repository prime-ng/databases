...
4. Check existing schema (ONLY use consolidated DDLs): `/Users/bkwork/WorkFolder/1-Development/0-Git_Work/prime-ai_db/databases/1-master_dbs/1-DDLs/`
   - `global_db.sql` — All global tables
   - `prime_db.sql` — All prime/central tables
   - `tenant_db.sql` — All tenant tables
   > **IMPORTANT:** Do NOT reference any other schema files in the `databases/` folder. Many old/unused schemas exist in subfolders like `2-Prime_Modules/`, `2-Tenant_Modules/`, `0-Policies/`, `Working/`, etc. These are outdated and superseded by the consolidated DDLs above.
