...
| **Date** | 2026-06-29 |
| **Status** | Draft |
| **Supersedes** | CMP_FRD_Old_v1.md (2026-06-27) |
| **Prepared By** | Business Analysis — Prime-AI |
| **Reviewed By** | (Pending) |
| **Approved By** | (Pending) |

> **Version note:** This edition preserves every requirement, business-rule, and report identifier from v1.0 (the downstream gap-analysis contract) and keeps all scope counts unchanged. It refines a small number of business rules and acceptance criteria for testability, incorporating design decisions and review findings accumulated since the v1.0 draft — chiefly that the resolution deadline must be *persisted* (not merely displayed) at registration, and that private-note and anonymous masking must be enforced at the data-retrieval layer, not only on screen. No requirements were added, removed, or renumbered.
...
2. Department-specific SLA rules that override category defaults for targeted entities (departments, designations, roles, entity groups, users, vehicles, vendors)
...
1. **Feedback Collection** — collecting post-resolution satisfaction surveys is documented as a planned sub-feature (REQ-CMP-014, Enhanced/P2) and is not part of the core release
2. **Parent Portal direct submission** — the Parent Portal module owns its own submission interface; this module owns only the complaint data
...
| Complainant | The person raising the complaint — a student, parent, staff member, vendor, anonymous individual, or member of the public |
...
| Department SLA | A school-specific override of category SLA rules for a particular target entity (department, designation, role, entity group, user, vehicle, or vendor) |
| Resolution Due Date | The deadline by which a complaint must be resolved, set automatically and stored on the ticket at registration time from the applicable SLA rule |
| Action Timeline | The chronological audit log of every action taken on a complaint ticket — creation, assignment, status change, note, escalation, and resolution |
...
| Escalation Risk Score | A 0–100 composite score indicating the likelihood that a complaint will escalate further, based on severity, frequency, sentiment, and pending time |
...
| Private Note | A timeline entry visible only to School Admin and Principal — not to other staff or the complainant |
...
| Student | An enrolled student of the school | Submits complaints through the Student Portal; tracks the status of their own complaints |
| Parent | A parent or guardian of an enrolled student | Submits complaints through the Student Portal; tracks the status of complaints they filed |
...
| BR-CMP-002 | Escalation hours must be strictly ascending. The Level 1 threshold must be greater than the expected resolution hours; each subsequent escalation level must be greater than the previous one. | Validation |
...
2. The system rejects a category where any escalation threshold is less than or equal to the previous one (or to the expected resolution hours for Level 1)
...
- Standard category seeder with pre-built CBSE/ICSE category trees (Academic, Infrastructure, Behavioural, Staff Conduct, Health & Safety, Fee-Related) so new schools do not configure from scratch (ENH-CMP-011)
...
#### Acceptance Criteria
This feature is considered complete when:
1. A School Admin can create a Department SLA rule linking a category to a specific target entity type (department, designation, role, entity group, vehicle, vendor, or specific user)
...
- Sends to: Complaint Registration — provides the applicable resolution hours used to calculate the resolution due date
...
Any authorised user can register a complaint in the system. The complainant provides the nature of the complaint (category and optional sub-category), who the complaint is against (target entity), a title and description, and optional supporting details such as the incident date, location, and an attached photograph. Upon submission the system automatically generates a unique ticket number, assigns the severity and priority from the selected category, flags whether a medical check is needed, calculates **and stores** the resolution due date from the applicable SLA rule, and triggers AI processing to compute risk scores. A notification is sent to the School Admin on every new complaint. Students and parents submit complaints through the Student Portal; school staff register complaints directly from the admin panel.
...
| Rule ID | Business Rule | Rule Type |
|---------|--------------|-----------|
| BR-CMP-007 | Every complaint must receive a unique ticket number in the format CMP-YYYY-NNNNNN (e.g., CMP-2026-000042). The serial resets to 000001 at the start of each calendar year and is unique within the school. | Validation |
...
| BR-CMP-009 | The complaint's severity level, priority score, and medical-check requirement are automatically taken from the selected category at registration. Staff cannot manually enter these fields during complaint creation. | Workflow |
| BR-CMP-010 | The resolution due date is calculated automatically at registration from the Department SLA (if a matching rule exists for the complaint's category and target) or from the category's default expected resolution hours, **and is saved on the ticket at that moment** — it is not merely displayed and not manually editable at creation time. | Workflow |
...
3. Opening the ticket immediately after registration shows a populated Resolution Due Date stored on the record, reflecting the applicable SLA (Department SLA if matched, otherwise category default)
...
#### Enhancement Notes (Future)
- WhatsApp or email acknowledgement sent to the complainant's registered contact with the ticket number upon submission (ENH-CMP-010)
- Multi-file attachment support for complaint evidence — currently limited to a single photograph (ENH-CMP-001)
...
| BR-CMP-014 | Only valid status transitions are permitted: Open → In-Progress; In-Progress → Resolved / Closed / Rejected; Resolved → Reopened (see REQ-CMP-012). Backward or arbitrary status jumps must be blocked by the system, not merely discouraged by the screen. | Workflow |

#### Acceptance Criteria
This feature is considered complete when:
1. An authorised user can change the status of a complaint they are assigned to or have permission to manage
...
3. Attempting an invalid transition (e.g., Closed → Open, or Open → Resolved skipping In-Progress) is rejected with a clear message and leaves the status unchanged
4. Every status change appears in the complaint timeline within seconds
5. The complaint detail screen shows the current status, who it is assigned to, and the resolution due date with a visual indicator if overdue

#### Integration with Other Modules
- Sends to: Notification module — resolution notification to complainant; escalation notification to Principal

#### Enhancement Notes (Future)
- Colour-coded SLA status badge on the complaint list showing time remaining or overdue duration (ENH-CMP-002)
...
Every action taken on a complaint — its creation, assignment, status change, resolution, note, or escalation — is permanently recorded in a chronological timeline. The timeline is the backbone of accountability: it shows exactly who did what and when throughout the complaint's lifecycle. Authorised staff can also manually add notes (such as investigation findings or interim updates) to the timeline. Private notes can be added that are visible only to School Admin and Principal, allowing sensitive handling notes to be recorded without exposing them to other staff.
...
| Rule ID | Business Rule | Rule Type |
|---------|--------------|-----------|
| BR-CMP-015 | Private notes are visible only to School Admin and Principal. The restriction must be enforced when the timeline data is retrieved — private entries are never returned to other roles — not only hidden on screen. | Permission |
| BR-CMP-016 | The timeline must always be displayed in chronological order from the oldest action to the most recent, ordered by the recorded action time. | Workflow |
...
3. A staff member who is not a School Admin or Principal never receives private notes — they are absent from the data returned to that user, not merely hidden in the layout
...
#### Enhancement Notes (Future)
- Embed the timeline as a panel within the complaint detail page (currently displayed on a separate screen)
...
| Rule ID | Business Rule | Rule Type |
|---------|--------------|-----------|
| BR-CMP-017 | A medical-check record may only be created for a complaint whose category is configured to require a medical check. The system must verify this requirement on the complaint before allowing the record to be saved, not only hide the entry option. | Validation |
...
2. Attempting to create a medical check for a complaint whose category does not require one is blocked with a clear message and nothing is saved
...
Every complaint submitted on the Prime-AI platform is automatically analysed by a built-in rules-based intelligence engine. Without any manual input, the engine produces three scores: a sentiment score reflecting how negative or distressed the complaint description is; an escalation risk score predicting the likelihood of the complaint escalating further; and a safety risk score identifying physical welfare or safety concerns based on the language used. These scores are stored as an AI Insight record linked one-to-one with each complaint, and power the analytics dashboard and AI risk reports. The engine runs automatically every time a complaint is created or updated.
...
#### Integration with Other Modules
- None — the AI Insight Engine is internal to the Complaint module

#### Enhancement Notes (Future)
- Integration with a Python machine-learning microservice for AI category prediction — currently the engine assigns the same category the user selected (ENH-CMP-012)
- Hindi/regional language keyword support for sentiment and safety scoring to improve accuracy for Indian school complaints (ENH-CMP-009)
...
| Rule ID | Business Rule | Rule Type |
|---------|--------------|-----------|
| BR-CMP-020 | The SLA Violation Report must only include complaints that are currently Open or In-Progress. Resolved, Closed, and Rejected complaints — including the Resolved status — are excluded from this report. | Validation |
...
4. The SLA Violation Report only shows Open or In-Progress tickets (Resolved tickets do not appear)

#### Integration with Other Modules
- None — all report data comes from complaint tables within this module

#### Enhancement Notes (Future)
- Grievance Redressal Committee auto-report (CBSE compliance) — auto-generated monthly list of complaints unresolved for more than 30 days (ENH-CMP-008)
...
| Rule ID | Business Rule | Rule Type |
|---------|--------------|-----------|
| BR-CMP-021 | For complaints submitted anonymously, the complainant's name and contact details must be masked for every role except School Admin and Principal. The masking must be applied when the complaint data is retrieved, so identifying fields are never returned to other staff — not merely hidden on screen. | Permission |
...
3. An anonymous complaint never returns the complainant's name or contact to staff who are not School Admin or Principal (the fields are absent from their data, not merely blanked in the layout)
...
#### Enhancement Notes (Future)
- Parent Portal direct submission, separate from Student Portal (ENH-CMP-013)
...
A complaint marked as Resolved can be reopened if the complainant or School Admin is unsatisfied with the resolution or if the issue has recurred. Reopening requires a reason to be documented. The complaint status reverts to In-Progress, the resolution data is cleared, and the reopen reason is permanently logged in the complaint timeline. This ensures that "resolved" tickets cannot be prematurely closed and that complainants have a formal escalation path if the issue persists.
...
2. The complainant can submit a 1–5 star rating and optional comments
...
| BR-CMP-007 | Every complaint must receive a unique ticket number in the format CMP-YYYY-NNNNNN; the serial resets to 000001 each calendar year and is unique within the school. | REQ-CMP-003 | Validation | P0 |
...
| BR-CMP-010 | Resolution due date is calculated automatically at registration from the applicable SLA and saved on the ticket at that moment; staff cannot manually set it at creation time. | REQ-CMP-003 | Workflow | P0 |
...
| BR-CMP-014 | Only valid status transitions are allowed (Open → In-Progress; In-Progress → Resolved / Closed / Rejected; Resolved → Reopened); backward or arbitrary jumps are blocked by the system. | REQ-CMP-005 | Workflow | P0 |
| BR-CMP-015 | Private notes are visible only to School Admin and Principal and are filtered out at data retrieval for all other roles, not only on screen. | REQ-CMP-006 | Permission | P0 |
| BR-CMP-016 | The complaint timeline must always be displayed in chronological order (oldest action first), ordered by the recorded action time. | REQ-CMP-006 | Workflow | P0 |
| BR-CMP-017 | A medical-check record may only be created for a complaint whose category is configured to require a medical check; the system verifies the flag on the complaint before saving. | REQ-CMP-007 | Validation | P1 |
...
| BR-CMP-021 | For anonymous complaints, the complainant's name and contact are masked at data retrieval for all roles except School Admin and Principal. | REQ-CMP-011 | Permission | P1 |
...
**What it represents:** The master classification of complaint types used at the school, organised in a parent-child hierarchy (Main Category → Sub-category). Each category carries the default SLA and escalation rules that apply to complaints filed under it.
...
| Severity Level | The default severity assigned to complaints in this category (Low / Medium / High / Critical) | No | Sourced from the global dropdown master; school can extend the values |
...
| Medical Check Required | Whether complaints in this category require a formal medical examination to be documented | No | Yes/No; defaults to No |
...
**Data Retention:** Categories are soft-deleted and retained indefinitely for historical complaint traceability. Permanent deletion is only possible when no complaints or child categories reference the record.
...
**What it represents:** A target-specific override of the category's default SLA timelines. Defines different resolution commitments and escalation structures for complaints filed against specific departments, designations, roles, entity groups, vehicles, vendors, or users.
...
| Complaint Sub-category | Optional further restriction to a specific sub-category | No | If blank, the rule applies to all sub-categories of the category |
...
**Data Retention:** Soft-deleted and retained. Rules are auditable even after deactivation.
...
| Complainant Contact | Contact number for anonymous/external complainants | No | Masked for non-admin roles when complaint is anonymous |
...
| Status | Current lifecycle status (Open / In-Progress / Resolved / Closed / Rejected) | Yes | Initially set to Open by the system |
| Assigned To (Role & User) | The staff role and/or individual responsible for resolution | No | Set during complaint assignment |
| Resolution Due Date | The deadline for resolution, calculated from the applicable SLA and stored at registration | No | System-calculated and persisted; not manually set at creation |
...
| Escalation Level | Current time-based escalation level (0 = Pending; 1–5 = Level; Breached) | Yes | Updated by the scheduled system process |
...
| Supporting Evidence | Whether a photograph or file has been attached | No | Yes/No flag |
...
**Data Retention:** Complaint tickets are soft-deleted and never permanently removed without explicit authorisation. Retained for a minimum of 5 years for compliance purposes.
...
**What it represents:** A single chronological entry in the complaint's audit log. Automatically created by the system on every significant action (registration, assignment, status change, resolution); can also be created manually by authorised staff as a note.
...
| Action Timestamp | The date and time this action occurred | Yes | Set automatically by the system; the field the timeline is ordered by |

**Relationships:**
- Belongs to: Complaint Ticket

**Data Retention:** Timeline entries are permanent and cannot be deleted. They form the official audit trail.
...
| Evidence Attached | Whether photographic or documentary evidence has been uploaded | No | Yes/No |

**Relationships:**
- Belongs to: Complaint Ticket

**Data Retention:** Medical-check records are sensitive welfare data and must be retained for a minimum of 7 years.
...
**What it represents:** The system-generated risk assessment for a complaint, computed by the AI Insight Engine every time the complaint is saved. Stores three risk scores and a sentiment classification, forming a one-to-one record with each complaint.
...
| Sentiment Label | A human-readable classification (Calm / Neutral / Urgent / Angry) | No | Derived from the sentiment score; sourced from the global dropdown master |
...
| Safety Risk Score | Level of physical-safety concern, on a scale of 0 to 100 | No | Based on safety-related keywords in the description |
| Engine Version | The version of the rules engine used for this analysis | No | Allows future comparison when the engine is updated |
| Processed At | The date and time the AI analysis was last run | Yes | |

**Relationships:**
- Belongs to: Complaint Ticket (unique one-to-one relationship)

**Data Retention:** Insight records follow the same retention rules as the parent complaint.
...
**Trigger:** A complaint is submitted by any authorised user (admin panel or student portal)
...
1. **Registration:** Complainant (or Front Office Staff on their behalf) selects a category, provides complaint details, identifies the target entity, and optionally attaches a photograph and enters the incident date/location.
   - System auto-generates the ticket number
   - System auto-assigns severity, priority, and medical-check flag from the category
   - System calculates and **stores** the resolution due date from the applicable SLA rule
   - System sets status to Open
   - System logs a "Created" action in the complaint timeline
   - System triggers AI analysis (sentiment, escalation risk, safety risk)
   - System sends a creation notification to School Admin
...
3. **Assignment:** School Admin or Principal assigns the complaint to a responsible role and/or user
   ...
   - System logs an "Assigned" action in the complaint timeline
   - System notifies the assigned user/role
   - Decision: if the complaint breaches SLA thresholds (time-based) → the scheduled process updates the escalation level (see Workflow 6.3)
...
4. **Investigation / Resolution:** Assigned handler investigates, adds notes to the timeline, and resolves the issue
   ...
   - If a medical check is required → a medical-check record is created (see REQ-CMP-007)
   - Handler marks the complaint as Resolved, entering the resolution summary and resolution date
...
5. **Closure:**
   - **5a Resolved:** Status set to Resolved; timeline logs resolution; complainant notified
     ...
     - Decision: if complainant is unsatisfied → complainant or Admin reopens the complaint (see Workflow 6.2)
   - **5b Rejected:** School Admin rejects the complaint; timeline logs the rejection with reason; complainant notified
   - **5c Closed:** Admin closes the ticket (administrative closure); timeline logged
...
- If the complainant is anonymous: no complainant notification can be sent; School Admin must communicate the resolution through other means
...
2. **Validation:** System verifies the complaint status is Resolved; blocks the action if the status is Closed or Rejected
3. **Reopen:** System:
...
2. **Calculate Escalation Level:** For each complaint, the system calculates how much time has elapsed since the ticket date, then compares it against the cumulative escalation-hour thresholds defined in the applicable SLA rule:
...
| Rule | Description |
|------|-------------|
| BR-CMP-020 | Only Open or In-Progress complaints are included; Resolved, Closed, and Rejected tickets are excluded |
...
| Rule | Description |
|------|-------------|
| Pareto threshold | A visual line or highlight marks the point where the cumulative % crosses 80% |
...
| Category | Colour coding of each bubble |
...
| ENH-CMP-002 | SLA status badge on complaint list (colour-coded) | Instant visual signal of overdue and at-risk tickets without opening each complaint | Product Team | P1 | Backlog |
...
| ENH-CMP-004 | Post-resolution satisfaction rating (1–5 star) | Enables tracking of resolution quality, not just resolution speed; feeds frustration reports (links to REQ-CMP-014) | Product Team | P2 | Backlog |
...
| ENH-CMP-009 | Hindi / regional language AI keywords | Current sentiment and safety scoring is English-only; adding Hindi transliterations improves accuracy for the Indian school context | Product Team | P2 | Backlog |
...
| Complaint list | The complaint list screen must be paginated; no single request loads more than 25–50 records |
...
| Escalation job | The scheduled escalation-level update completes within 60 seconds for a school with up to 10,000 open complaints |
...
| Data isolation | School A's complaint data must never be visible to School B; all complaint records are isolated within the school's own tenant database |
| Audit trail | All changes to complaint records must record who made them and when; deletion must be a soft-delete, never permanent without explicit authorisation |
| Anonymous masking | Complaints submitted anonymously must mask complainant identity in all non-admin, non-principal contexts, enforced at the point the data is retrieved |
| Private note protection | Notes marked private must only be returned to School Admin and Principal; the system must enforce this when data is retrieved, not just on the display layer |
...
| Timeline readability | The complaint action timeline must be clearly formatted with timestamps, actor names, and action types so any authorised user can understand the full complaint history at a glance |
...
| BR-CMP-010 | Auto-calculate and persist resolution due date from SLA | REQ-CMP-003 | No | Yes | Yes |
...
| BR-CMP-014 | Valid status transitions only (system-enforced) | REQ-CMP-005 | Yes | Yes | Yes |
| BR-CMP-015 | Private notes filtered at retrieval for non-admin roles | REQ-CMP-006 | No | Yes | Yes |
| BR-CMP-016 | Timeline ordered chronologically by action time | REQ-CMP-006 | No | No | No |
...
| BR-CMP-020 | SLA report excludes resolved/closed/rejected | REQ-CMP-010 | No | Yes | No |
| BR-CMP-021 | Anonymous masking at retrieval in non-admin views | REQ-CMP-011 | No | Yes | Yes |
...
|---------|------|---------|--------|
| 1.0 | 2026-06-27 | Initial draft — synthesised from V2 technical requirement document (CMP_Complaint_Requirement.md), DDL (Complaint_DDL_v2.sql), and Complaint module code structure | Business Analysis — Prime-AI |
| 2.0 | 2026-06-29 | Superseding edition. Preserved all REQ-/BR-/RPT-/ENH- IDs and scope counts. Refined for testability: BR-CMP-010 now requires the resolution due date to be **persisted** at registration; BR-CMP-014 requires the status FSM to be **system-enforced** (invalid/backward transitions blocked); BR-CMP-015 and BR-CMP-021 now require private-note and anonymous-masking to be enforced at **data retrieval**, not just display; BR-CMP-016 ties timeline ordering to the action-time field; BR-CMP-017 requires the medical-check eligibility flag to be verified before save; BR-CMP-020 explicitly excludes the Resolved status. Added table-prefix/DB-layer metadata to the header. | Business Analysis — Prime-AI |
