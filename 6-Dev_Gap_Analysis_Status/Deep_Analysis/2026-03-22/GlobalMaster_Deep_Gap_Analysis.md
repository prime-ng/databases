...
📝 Developer Comments (SECTION 1 — Missing Features)
🆔 MF-001

Comment:
AcademicSession model is referenced in the GlobalMaster module but does not exist within the module. The model is currently available in the Prime module, which may lead to dependency and runtime resolution issues.

Decision:
This will be reviewed and validated by Tarun to determine the correct module ownership and implementation approach.

🆔 MF-002

Comment:
BoardController is referenced under the GlobalMaster namespace in routes; however, no corresponding controller exists within the module. The implementation currently relies on the Prime module controller, leading to inconsistency.

Decision:
This will be reviewed by Tarun to confirm whether the controller should be moved, duplicated, or referenced correctly.

🆔 MF-003

Comment:
Translation management is partially defined at the database level (glb_translations table), but no corresponding model, controller, or routes exist within the GlobalMaster module.

Decision:
Feature scope and implementation will be reviewed by Tarun before proceeding further.

🆔 MF-004

Comment:
Menu-related tables exist in the GlobalMaster schema, but menu management functionality is handled in the SystemConfig module. This creates a separation inconsistency between data and logic.

Decision:
Module responsibility and architecture will be reviewed by Tarun to ensure proper alignment.

🆔 MF-005

Comment:
glb_languages table does not include standard timestamp fields (created_at, updated_at, deleted_at), which deviates from project conventions.

Decision:
This will be reviewed by Tarun to decide whether schema changes are required to align with project standards.
---
...
Comment:
activityLog() is being invoked twice during the state update process—once with detailed change-tracking information and again with a generic success message. This results in duplicate log entries for a single update operation, leading to redundant data and reduced clarity in audit logs.

Decision:
The duplicate generic log entry will be removed, retaining only the detailed change-tracking log to ensure accurate and concise audit records.
...
Comment:
activityLog() is invoked twice during the module update process—once with detailed change-tracking information and again with a generic success message. This results in duplicate log entries for a single update operation, reducing clarity and increasing unnecessary log data.

Decision:
The redundant generic log entry will be removed, and only the detailed change-tracking log will be retained to ensure accurate and meaningful audit records.
...
Comment:
The conditional check if (!$academicSession->is_active === true) suffers from incorrect operator precedence, causing the negation to be evaluated before the comparison. This results in inverted logic, allowing active sessions to be deleted while blocking inactive ones.

Decision:
The condition will be corrected to if ($academicSession->is_active === true) to ensure active sessions are properly protected from deletion.
...
Comment:
AcademicSessionRequest does not validate required fields such as start_date, end_date, and is_current, despite these fields being defined as NOT NULL in the database schema. This creates a mismatch between validation logic and database constraints, potentially leading to runtime errors and inconsistent data.

Decision:
Validation rules will be updated to include start_date, end_date, and is_current, ensuring alignment with database requirements and maintaining data integrity.
...
📝 Developer Comments (Separated)
🆔 BUG-006-A (AcademicSessionController)

Comment:
Gate permission naming in AcademicSessionController uses mixed prefixes (prime.* and global-master.*). This structure appears to be aligned with existing module dependencies and access control design.

Decision:
No changes will be made to the current permission naming to avoid breaking existing functionality and dependencies.

🆔 BUG-006-B (LanguageController — Missing Authorization)

Comment:
Several methods in LanguageController (create(), store(), edit(), update()) do not implement any Gate::authorize() checks, leaving these endpoints unprotected and accessible without proper permissions.

Decision:
Authorization checks will be added to all missing methods to ensure secure and consistent access control.

🆔 BUG-006-C (LanguageController — Inconsistent Permission Naming)

Comment:
LanguageController uses inconsistent permission prefixes (prime.* in index() and global-master.* in destroy()), leading to lack of standardization in permission handling.

Decision:
Permission naming will be standardized across the controller using a consistent convention (global-master.language.*) without affecting existing access configurations.
