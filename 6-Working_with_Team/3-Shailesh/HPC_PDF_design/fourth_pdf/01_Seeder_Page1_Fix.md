# HPC Fourth Template — Seeder Page 1 Fix (`seedPage1Fourth`)

**Date:** 2026-03-17
**File:** `database/seeders/HPCTemplateSeeder.php`
**Method:** `seedPage1Fourth(int $tId, int $pId)`
...
The fourth template PDF first page has been shared. The `seedPage1Fourth()` method currently has
incorrect rubric groupings — fields that should be on separate rows are grouped together, and
fields that should be on the same row are separate. Fix them to exactly match the PDF page 1 layout.
...
1. `database/seeders/HPCTemplateSeeder.php` — full `seedPage1Fourth()` method (lines ~2433–2507)
2. The shared **fourth template PDF page 1** — identify exact row order and field groupings

---

## Known Issues in Current `seedPage1Fourth()`
...
- **Grades shown** — fourth template = Grade 9, Grade 10, Grade 11, Grade 12 (verify checkboxes in seeder)
- **ATTENDANCE section title** — confirm label matches PDF exactly
- **INTERESTS section** — fourth template may or may NOT have an interests section; confirm from PDF
...
## Grade Keys for Fourth Template

The current seeder uses a dynamic `array_search` loop for grades — verify the generated `html_object_name` values:
- `grade_grade9`
- `grade_grade10`
- `grade_grade11`
- `grade_grade12`

If the loop-generated keys are incorrect, replace with explicit `ri()` calls:
```php
$r = $this->insertRubric($tId, $pId, $s2, 'GRADE_SELECTION', 'Grade/Class', 3, 1);
$this->ri($r, 'grade_grade9',  1, 'Boolean', 'Grade 9',  'Grade 9',  1);
$this->ri($r, 'grade_grade10', 2, 'Boolean', 'Grade 10', 'Grade 10', 1);
$this->ri($r, 'grade_grade11', 3, 'Boolean', 'Grade 11', 'Grade 11', 1);
$this->ri($r, 'grade_grade12', 4, 'Boolean', 'Grade 12', 'Grade 12', 1);
```
...
Check:
- [ ] `seedPage1Fourth()` has NO grouped rubrics that should be separate rows
