...
## 🔴 SUPREME RULE: FOLLOW EXISTING CODE PATTERN
- **The user's actual working code is the SOURCE OF TRUTH** — NOT the generic rule files.
- Before writing any code, first examine the **existing completed views and controllers** in the same module.
- Match the exact patterns used: permission format (`cafeteria.menu-items.*` not `tenant.cafeteria.*`), layout structure, component usage, controller style.
- The **Menu Items (cafeteria::menu-items.*)** is the Gold Standard reference for the Cafeteria module — all other Cafeteria views must match its pattern.
- If there's a conflict between a rule file and the actual code, **the actual code wins**.
...
13. **Follow the patterns already established in existing modules.** When in doubt, look at how `Modules/Vendor` implements similar features — it is the canonical gold-standard for:
    - Tab-Based Hub Module Pattern → `VendorController` + `tab_module/tab.blade.php`
    - Standard single-resource CRUD → `VndItemController` + `vendor-item/*.blade.php`
    - Role & permission checks in Blade → `vendor/index.blade.php`, `vendor/trash.blade.php`
    - Gate::authorize, activityLog, $request->validated() → `VendorController`, `VndItemController`
...
> 📄 **For full CRUD templates and Tab-Hub pattern code:** See `rules/crud-patterns.md`
> 📄 **For permission naming and Blade @can patterns:** See `rules/permission-rules.md`
