...
-- This table will collect all asset categories
-- ===========================================================================
...
CREATE TABLE IF NOT EXISTS `mnt_asset_categories` (
    ...
    `default_priority`        ENUM('Low','Medium','High','Critical') NOT NULL DEFAULT 'Medium' COMMENT 'Default ticket priority when no keyword matches; Low|Medium|High|Critical',
...
-- ===========================================================================
-- LAYER 2 — Depends on vnd_vendors (cross-module); placed before mnt_assets to break the mnt_assets ↔ mnt_amc_contracts circular dependency.
...
CREATE TABLE IF NOT EXISTS `mnt_amc_contracts` (
    ...
    `payment_frequency`       ENUM('Monthly','Quarterly','Half_Yearly','Yearly') NULL DEFAULT NULL COMMENT 'Payment schedule: Monthly|Quarterly|Half_Yearly|Yearly',
    `visit_frequency`         VARCHAR(100) NULL DEFAULT NULL COMMENT 'Free-text e.g. Monthly — 12 visits/year',
    `status`                  ENUM('Active','Expired','Cancelled','Pending_Renewal') NOT NULL DEFAULT 'Active' COMMENT 'Contract status; auto-set to Expired when end_date < today',
