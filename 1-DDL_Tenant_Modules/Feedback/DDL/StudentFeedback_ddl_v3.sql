-- ========================================================================================================
-- GENERIC FEEDBACK MODULE  —  v3.0
-- ========================================================================================================
-- Scope   : Tenant DB (per school)
-- DB      : MySQL 8+
-- Prefix  : fbk_*  (dedicated Feedback module)
-- Author  : DB Architect
-- Date    : 2026-04-09
-- Replaces: StudentFeedback_ddl_v2.sql
--
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════
--  WHAT CHANGED FROM v2 → v3
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════
-- v2 used MySQL ENUM in many places, which locks the list of options to a DDL-level change.
-- v3 removes ENUM in favour of the generic `sys_dropdown_table` system already provided in
-- tenant_db_v2.sql, so business users (PG-Admin / PG-Support) can evolve option lists without
-- schema migrations. ENUM is retained ONLY for fields whose option set is genuinely fixed by
-- code and can never be extended without an application release (there are none left in this module).
--
-- CONVERSION SUMMARY
-- ──────────────────
--   v2 ENUM Column                                            → v3 Replacement
--   -------------------------------------------------------   -------------------------------------------------
--   fbk_target_types.linked_entity_table                      → linked_entity_table_id  FK → sys_dropdown_table
--   fbk_relationship_types.respondent_kind                    → respondent_kind_id       FK → sys_dropdown_table
--   fbk_relationship_types.context_required                   → context_required_id      FK → sys_dropdown_table
--   fbk_templates.respondent_kind                             → respondent_kind_id       FK → sys_dropdown_table
--   fbk_templates.overall_rating_method                       → overall_rating_method_id FK → sys_dropdown_table
--   fbk_questions.question_type                               → question_type_id         FK → sys_dropdown_table
--   fbk_questions.respondent_kind                             → respondent_kind_id       FK → sys_dropdown_table
--   fbk_cycles.status                                         → status_id                FK → sys_dropdown_table
--   fbk_cycle_feedback_types.scope_type                       → scope_type_id            FK → sys_dropdown_table
--   fbk_cycle_feedback_types.target_population_mode (Auto/Manual) → is_auto_populated_targets TINYINT(1) (truly binary)
--   fbk_responses.respondent_kind                             → respondent_kind_id       FK → sys_dropdown_table
--   fbk_responses.status                                      → status_id                FK → sys_dropdown_table
--   fbk_answers.question_type_snapshot                        → question_type_id_snapshot FK → sys_dropdown_table
--
-- Structural tables, polymorphic design, dedup generated columns, and rating pipeline are UNCHANGED.
--
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════
--  MODULE TABLES (11) — unchanged from v2
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════════════════════════════════
--   sys_dropdown_table                — Generic dropdown value store (replaces v2 ENUMs)
--   sys_dropdown_needs                — Registry of which (table,column) pairs may have dropdowns
--   sys_dropdown_need_table_jnt       — Junction: which dropdown rows fulfil which needs
--   sys_users                         — every actor (student, parent, teacher, staff) has a user record
--   sch_employees                     — teacher + non-teaching staff
--   sch_class_section_jnt             — class teacher / assistance_class_teacher references
--   sch_subjects                      — subject context
--   sch_departments                   — department-level targets
...
-- ========================================================================================================
--  SECTION 0 : DROPDOWN REGISTRY — sys_dropdown_needs & sys_dropdown_table SEED
-- ========================================================================================================
-- The Feedback module depends on the following logical dropdowns. These MUST exist in
-- sys_dropdown_needs (one row per (db_type, table_name, column_name)) and the corresponding
-- values MUST exist in sys_dropdown_table before any fbk_* record is inserted.
--
-- The seed statements below are given as INFORMATIVE INSERTS — they can be copied into a seeder.
-- `ordinal` values are suggested; PG-Admin can reorder in the UI.
--
-- ────────────────────────────────────────────────────────────────────────────────────────────────────────
-- DROPDOWN KEYS USED BY THIS MODULE
-- ────────────────────────────────────────────────────────────────────────────────────────────────────────
--   Key                                                   | Purpose
--   -------------------------------------------------------+--------------------------------------------------
--   fbk_target_types.linked_entity_table                  | Base table a target record lives in
--   fbk_relationship_types.respondent_kind                | Who provides feedback (shared with fbk_templates,
--   fbk_templates.respondent_kind                         |   fbk_questions, fbk_responses — identical value set,
--   fbk_questions.respondent_kind                         |   each column has its own need row by convention)
--   fbk_responses.respondent_kind                         |
--   fbk_relationship_types.context_required               | Contextual resolver (Class_Section, Subject, ...)
--   fbk_templates.overall_rating_method                   | Weighted_Average / Simple_Average / Manual_Only / None
--   fbk_questions.question_type                           | Rating_5 / Likert_5 / Emoji_5 / Yes_No / ...
--   fbk_answers.question_type_snapshot                    | Snapshot copy of fbk_questions.question_type
--   fbk_cycles.status                                     | Draft / Active / Closed / Published / Cancelled
--   fbk_cycle_feedback_types.scope_type                   | All / Specific_Classes / Specific_Departments / ...
--   fbk_responses.status                                  | Draft / Submitted / Withdrawn
-- ────────────────────────────────────────────────────────────────────────────────────────────────────────
--
-- Example seed (NOT executed here — place in a seeder / migration):
--
-- INSERT INTO sys_dropdown_needs (db_type,table_name,column_name,menu_category,main_menu,sub_menu,tab_name,field_name,is_system,tenant_creation_allowed,compulsory,is_active)
-- VALUES
--   ('Tenant','fbk_target_types','linked_entity_table','Feedback','Masters','Target Types','Base Table','Linked Entity Table',1,0,1,1),
--   ('Tenant','fbk_relationship_types','respondent_kind','Feedback','Masters','Relationship Types','Actors','Respondent Kind',1,0,1,1),
--   ('Tenant','fbk_relationship_types','context_required','Feedback','Masters','Relationship Types','Context','Context Required',1,0,1,1),
--   ('Tenant','fbk_templates','respondent_kind','Feedback','Templates','Template','Scope','Respondent Kind',1,0,1,1),
--   ('Tenant','fbk_templates','overall_rating_method','Feedback','Templates','Template','Scoring','Overall Rating Method',1,0,1,1),
--   ('Tenant','fbk_questions','question_type','Feedback','Templates','Questions','Type','Question Type',1,0,1,1),
--   ('Tenant','fbk_questions','respondent_kind','Feedback','Templates','Questions','Filter','Respondent Kind',1,0,1,1),
--   ('Tenant','fbk_cycles','status','Feedback','Cycles','Cycle','Lifecycle','Status',1,0,1,1),
--   ('Tenant','fbk_cycle_feedback_types','scope_type','Feedback','Cycles','Cycle','Scope','Scope Type',1,0,1,1),
--   ('Tenant','fbk_responses','respondent_kind','Feedback','Responses','Response','Actors','Respondent Kind',1,0,1,1),
--   ('Tenant','fbk_responses','status','Feedback','Responses','Response','Lifecycle','Status',1,0,1,1),
--   ('Tenant','fbk_answers','question_type_snapshot','Feedback','Responses','Answers','Type','Question Type (snapshot)',1,0,1,1);
--
-- INSERT INTO sys_dropdown_table (ordinal,`key`,value,type,additional_info,is_active) VALUES
-- -- fbk_target_types.linked_entity_table
--   (1,'fbk_target_types.linked_entity_table','sys_users','String',JSON_OBJECT('description','Generic login user'),1),
--   (2,'fbk_target_types.linked_entity_table','std_students','String',JSON_OBJECT('description','Student target'),1),
--   (3,'fbk_target_types.linked_entity_table','sch_employees','String',JSON_OBJECT('description','Employee / staff target'),1),
--   (4,'fbk_target_types.linked_entity_table','sch_departments','String',JSON_OBJECT('description','Department-level aggregate target'),1),
--   (5,'fbk_target_types.linked_entity_table','sch_classes','String',JSON_OBJECT('description','Class-level target'),1),
--   (6,'fbk_target_types.linked_entity_table','sch_sections','String',JSON_OBJECT('description','Section-level target'),1),
--   (7,'fbk_target_types.linked_entity_table','sch_class_section_jnt','String',JSON_OBJECT('description','Class+Section combination target'),1),
--   (8,'fbk_target_types.linked_entity_table','other','String',JSON_OBJECT('description','Fallback'),1),
-- -- fbk_relationship_types.respondent_kind  (repeat the same value set for each respondent_kind column)
--   (1,'fbk_relationship_types.respondent_kind','Student','String',NULL,1),
--   (2,'fbk_relationship_types.respondent_kind','Parent','String',NULL,1),
--   (3,'fbk_relationship_types.respondent_kind','Teacher','String',NULL,1),
--   (4,'fbk_relationship_types.respondent_kind','Staff','String',NULL,1),
--   (5,'fbk_relationship_types.respondent_kind','Admin','String',NULL,1),
--   (6,'fbk_relationship_types.respondent_kind','Self','String',NULL,1),
--   (7,'fbk_relationship_types.respondent_kind','Any','String',NULL,1),
--   (1,'fbk_templates.respondent_kind','Student','String',NULL,1),
--   (2,'fbk_templates.respondent_kind','Parent','String',NULL,1),
--   (3,'fbk_templates.respondent_kind','Teacher','String',NULL,1),
--   (4,'fbk_templates.respondent_kind','Staff','String',NULL,1),
--   (5,'fbk_templates.respondent_kind','Admin','String',NULL,1),
--   (6,'fbk_templates.respondent_kind','Self','String',NULL,1),
--   (7,'fbk_templates.respondent_kind','Any','String',NULL,1),
--   (1,'fbk_questions.respondent_kind','Student','String',NULL,1),
--   (2,'fbk_questions.respondent_kind','Parent','String',NULL,1),
--   (3,'fbk_questions.respondent_kind','Teacher','String',NULL,1),
--   (4,'fbk_questions.respondent_kind','Staff','String',NULL,1),
--   (5,'fbk_questions.respondent_kind','Admin','String',NULL,1),
--   (6,'fbk_questions.respondent_kind','Self','String',NULL,1),
--   (7,'fbk_questions.respondent_kind','Any','String',NULL,1),
--   (1,'fbk_responses.respondent_kind','Student','String',NULL,1),
--   (2,'fbk_responses.respondent_kind','Parent','String',NULL,1),
--   (3,'fbk_responses.respondent_kind','Teacher','String',NULL,1),
--   (4,'fbk_responses.respondent_kind','Staff','String',NULL,1),
--   (5,'fbk_responses.respondent_kind','Admin','String',NULL,1),
--   (6,'fbk_responses.respondent_kind','Self','String',NULL,1),
-- -- (Note: 'Any' not allowed on actual response — omit)
-- -- fbk_relationship_types.context_required
--   (1,'fbk_relationship_types.context_required','None','String',NULL,1),
--   (2,'fbk_relationship_types.context_required','Class_Section','String',NULL,1),
--   (3,'fbk_relationship_types.context_required','Subject','String',NULL,1),
--   (4,'fbk_relationship_types.context_required','Subject_And_Class_Section','String',NULL,1),
--   (5,'fbk_relationship_types.context_required','Transport_Route','String',NULL,1),
--   (6,'fbk_relationship_types.context_required','Hostel','String',NULL,1),
--   (7,'fbk_relationship_types.context_required','Department','String',NULL,1),
--   (8,'fbk_relationship_types.context_required','Custom','String',NULL,1),
-- -- fbk_templates.overall_rating_method
--   (1,'fbk_templates.overall_rating_method','Weighted_Average','String',NULL,1),
--   (2,'fbk_templates.overall_rating_method','Simple_Average','String',NULL,1),
--   (3,'fbk_templates.overall_rating_method','Manual_Only','String',NULL,1),
--   (4,'fbk_templates.overall_rating_method','None','String',NULL,1),
-- -- fbk_questions.question_type  (and reused as snapshot key for fbk_answers)
--   (1,'fbk_questions.question_type','Rating_5','Integer',JSON_OBJECT('scale_max',5),1),
--   (2,'fbk_questions.question_type','Rating_10','Integer',JSON_OBJECT('scale_max',10),1),
--   (3,'fbk_questions.question_type','Likert_5','Integer',JSON_OBJECT('scale_max',5),1),
--   (4,'fbk_questions.question_type','Emoji_5','Integer',JSON_OBJECT('scale_max',5),1),
--   (5,'fbk_questions.question_type','Yes_No','Boolean',NULL,1),
--   (6,'fbk_questions.question_type','Multi_Choice','String',NULL,1),
--   (7,'fbk_questions.question_type','Free_Text','String',NULL,1),
--   (1,'fbk_answers.question_type_snapshot','Rating_5','Integer',NULL,1),
--   (2,'fbk_answers.question_type_snapshot','Rating_10','Integer',NULL,1),
--   (3,'fbk_answers.question_type_snapshot','Likert_5','Integer',NULL,1),
--   (4,'fbk_answers.question_type_snapshot','Emoji_5','Integer',NULL,1),
--   (5,'fbk_answers.question_type_snapshot','Yes_No','Boolean',NULL,1),
--   (6,'fbk_answers.question_type_snapshot','Multi_Choice','String',NULL,1),
--   (7,'fbk_answers.question_type_snapshot','Free_Text','String',NULL,1),
-- -- fbk_cycles.status
--   (1,'fbk_cycles.status','Draft','String',NULL,1),
--   (2,'fbk_cycles.status','Active','String',NULL,1),
--   (3,'fbk_cycles.status','Closed','String',NULL,1),
--   (4,'fbk_cycles.status','Published','String',NULL,1),
--   (5,'fbk_cycles.status','Cancelled','String',NULL,1),
-- -- fbk_cycle_feedback_types.scope_type
--   (1,'fbk_cycle_feedback_types.scope_type','All','String',NULL,1),
--   (2,'fbk_cycle_feedback_types.scope_type','Specific_Classes','String',NULL,1),
--   (3,'fbk_cycle_feedback_types.scope_type','Specific_Departments','String',NULL,1),
--   (4,'fbk_cycle_feedback_types.scope_type','Specific_Targets','String',NULL,1),
--   (5,'fbk_cycle_feedback_types.scope_type','Custom','String',NULL,1),
-- -- fbk_responses.status
--   (1,'fbk_responses.status','Draft','String',NULL,1),
--   (2,'fbk_responses.status','Submitted','String',NULL,1),
--   (3,'fbk_responses.status','Withdrawn','String',NULL,1);
--
-- The junction table sys_dropdown_need_table_jnt must also be seeded to wire each dropdown row
-- to its corresponding need row. The application layer enforces that the *_id FKs below only
-- accept sys_dropdown_table rows whose `key` matches the expected key listed above.
-- ========================================================================================================
...
-- `linked_entity_table_id` is a metadata hint for the application layer — it tells the app
-- which base table a target of this kind lives in (std_students, sch_employees, sch_departments, ...).
-- The option set is stored in sys_dropdown_table under key = 'fbk_target_types.linked_entity_table'.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fbk_target_types` (
  ...
  `linked_entity_table_id`  INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_target_types.linked_entity_table (sys_users, std_students, sch_employees, sch_departments, ...)',
  ...
  INDEX `idx_fbk_tt_linked_entity_table` (`linked_entity_table_id`),
  CONSTRAINT `fk_fbk_tt_linked_entity_table` FOREIGN KEY (`linked_entity_table_id`) REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Master list of target kinds that can be rated by the Feedback module';
-- Seed examples (insert during module setup — linked_entity_table_id must resolve via sys_dropdown_table):
...
--
-- `context_required_id` tells the app what contextual filter is needed to find valid
-- (respondent, target) pairs at runtime. Expected values (stored in sys_dropdown_table
-- under key = 'fbk_relationship_types.context_required'):
...
CREATE TABLE IF NOT EXISTS `fbk_relationship_types` (
  ...
  -- Actors
  `respondent_kind_id`           INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_relationship_types.respondent_kind (Student/Parent/Teacher/Staff/Admin/Self/Any)',
  `target_type_id`               SMALLINT UNSIGNED NOT NULL COMMENT 'FK → fbk_target_types.id',
  -- Context rule
  `context_required_id`          INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_relationship_types.context_required (None/Class_Section/Subject/...)',
  ...
  INDEX `idx_fbk_rt_respondent_target` (`respondent_kind_id`, `target_type_id`),
  INDEX `idx_fbk_rt_context`           (`context_required_id`),
  INDEX `idx_fbk_rt_active`            (`is_active`),
  CONSTRAINT `fk_fbk_rt_respondent_kind`  FOREIGN KEY (`respondent_kind_id`)  REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_fbk_rt_target_type`      FOREIGN KEY (`target_type_id`)      REFERENCES `fbk_target_types`    (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_fbk_rt_context_required` FOREIGN KEY (`context_required_id`) REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Valid (respondent_kind × target_type × context) combinations — acts as an authorisation whitelist';
-- Seed examples (respondent_kind and context_required resolved via sys_dropdown_table):
...
--   STUDENT_TO_PEER_STUDENT             Student → PEER_STUDENT      Class_Section              anon=1 NEP=1
...
--   TEACHER_TO_STUDENT                  Teacher → STUDENT           Subject_And_Class_Section  anon=0 NEP=1
--   TEACHER_TO_PEER_TEACHER             Teacher → TEACHER           None                       anon=1 NEP=0
--   ADMIN_TO_TEACHER                    Admin   → TEACHER           None                       anon=0 NEP=0
...
--   SELF_REFLECTION_STUDENT             Student → STUDENT           None (is_self=1)           anon=0 NEP=0
...
CREATE TABLE IF NOT EXISTS `fbk_templates` (
  ...
  -- Scope
  `target_type_id`                      SMALLINT UNSIGNED NOT NULL COMMENT 'FK → fbk_target_types.id',
  `respondent_kind_id`                  INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_templates.respondent_kind — restricts which respondent kinds can use this template (use Any dropdown row for no restriction)',
  ...
  -- Scoring
  `overall_rating_method_id`            INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_templates.overall_rating_method (Weighted_Average/Simple_Average/Manual_Only/None)',
  ...
  INDEX `idx_fbk_tpl_respondent`     (`respondent_kind_id`),
  INDEX `idx_fbk_tpl_rating_method`  (`overall_rating_method_id`),
  ...
  CONSTRAINT `fk_fbk_tpl_respondent_kind` FOREIGN KEY (`respondent_kind_id`)        REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_fbk_tpl_rating_method`   FOREIGN KEY (`overall_rating_method_id`)  REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT,
...
--
-- question_type_id semantics (resolved from sys_dropdown_table, key=fbk_questions.question_type):
...
--   Free_Text     → open text
-- New question types can be added without schema changes by inserting new dropdown rows.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fbk_questions` (
  ...
  `display_order`         SMALLINT UNSIGNED NOT NULL DEFAULT 1,
  -- Type & config
  `question_type_id`      INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_questions.question_type',
  ...
  `options_json`          JSON DEFAULT NULL COMMENT 'For Multi_Choice: [{"code":"A","label":"Always","value":5}, {"code":"S","label":"Sometimes","value":3}, {"code":"N","label":"Never","value":1}]',
  -- Respondent filter (use the "Any" dropdown row for no restriction)
  `respondent_kind_id`    INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_questions.respondent_kind',
  -- Audit
  `is_active`             TINYINT(1) NOT NULL DEFAULT 1,
  `created_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX `idx_fbk_q_question_type`   (`question_type_id`),
  INDEX `idx_fbk_q_respondent_kind` (`respondent_kind_id`),
  ...
  CONSTRAINT `fk_fbk_q_question_type`   FOREIGN KEY (`question_type_id`)    REFERENCES `sys_dropdown_table`(`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_fbk_q_respondent_kind` FOREIGN KEY (`respondent_kind_id`)  REFERENCES `sys_dropdown_table`(`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- 6. fbk_cycles
-- ---------------------------------------------------------------------------
-- A feedback collection window. One cycle can contain multiple feedback types (see fbk_cycle_feedback_types). 
-- Admin/Principal creates cycles; respondents submit during [start_date..end_date]; admin publishes summaries after cycle closes.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fbk_cycles` (
  ...
  -- Lifecycle
  `status_id`                            INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_cycles.status (Draft/Active/Closed/Published/Cancelled)',
  ...
  `default_min_responses_for_visibility` TINYINT UNSIGNED NOT NULL DEFAULT 3 COMMENT 'Minimum response count before teacher/target can view aggregate (protects anonymity for small groups)',
  -- Instructions
  ...
  INDEX `idx_fbk_c_status`  (`status_id`),
  ...
  CONSTRAINT `fk_fbk_c_status`       FOREIGN KEY (`status_id`)           REFERENCES `sys_dropdown_table`            (`id`) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS `fbk_cycle_feedback_types` (
  ...
  `scope_type_id`                   INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_cycle_feedback_types.scope_type (All/Specific_Classes/Specific_Departments/Specific_Targets/Custom)',
  `scope_filter_json`               JSON DEFAULT NULL COMMENT 'Flexible filter: {"class_section_ids":[1,2], "department_ids":[3], "target_user_ids":[10,20]}',
  -- Target population strategy (truly binary — kept as boolean flag, not a dropdown)
  `is_auto_populated_targets`       TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1 = app auto-populates fbk_cycle_targets from relationships; 0 = admin enters targets manually',
  -- Audit
  `is_active`                       TINYINT(1) NOT NULL DEFAULT 1,
  `created_at`                      TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`                      TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX `idx_fbk_cft_scope_type`         (`scope_type_id`),
  ...
  CONSTRAINT `fk_fbk_cft_scope_type`        FOREIGN KEY (`scope_type_id`)        REFERENCES `sys_dropdown_table`     (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
--               based on respondent_kind_id. respondent_user_id is ALWAYS populated (canonical login identity).
--   Target:     exactly ONE of target_user_id, target_student_id, target_employee_id, target_department_id
--               is populated based on the target_type.linked_entity_table_id.
--
-- DEDUP STRATEGY:
...
CREATE TABLE IF NOT EXISTS `fbk_responses` (
  ...
  `respondent_kind_id`            INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_responses.respondent_kind (Student/Parent/Teacher/Staff/Admin/Self — Any NOT allowed here)',
  `respondent_user_id`            INT UNSIGNED NOT NULL COMMENT 'Always populated: sys_users.id of the logged-in actor',
  `respondent_student_id`         INT UNSIGNED DEFAULT NULL COMMENT 'Populated when respondent is Student, OR when Parent is rating about their specific child',
  `respondent_guardian_id`        INT UNSIGNED DEFAULT NULL COMMENT 'Populated when respondent is Parent',
  `respondent_employee_id`        INT UNSIGNED DEFAULT NULL COMMENT 'Populated when respondent is Teacher/Staff/Admin',
  ...
  -- STATUS FSM
  `status_id`                     INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; key=fbk_responses.status (Draft/Submitted/Withdrawn)',
  ...
  UNIQUE KEY `uq_fbk_r_dedup` (`cycle_feedback_type_id`,`respondent_user_id`,`respondent_student_id_uq`,`target_user_id_uq`,`target_student_id_uq`,`target_employee_id_uq`,`target_department_id_uq`,`subject_id_uq`,`class_section_id_uq`,`deleted_at`),
  -- Query indexes
  INDEX `idx_fbk_r_cycle`           (`cycle_id`, `status_id`),
  INDEX `idx_fbk_r_cft`             (`cycle_feedback_type_id`, `status_id`),
  INDEX `idx_fbk_r_respondent`      (`respondent_user_id`, `cycle_id`),
  INDEX `idx_fbk_r_respondent_kind` (`respondent_kind_id`),
  ...
  INDEX `idx_fbk_r_status`          (`status_id`),
  ...
  CONSTRAINT `fk_fbk_r_respondent_kind`   FOREIGN KEY (`respondent_kind_id`)          REFERENCES `sys_dropdown_table`            (`id`) ON DELETE RESTRICT,
  ...
  CONSTRAINT `fk_fbk_r_status`            FOREIGN KEY (`status_id`)                   REFERENCES `sys_dropdown_table`            (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
--   - Exactly ONE respondent_{student,guardian,employee}_id matches respondent_kind_id
--   - Exactly ONE target_{user,student,employee,department}_id is populated per target_type.linked_entity_table_id
--   - If relationship_type.context_required_id requires it, the relevant context fields must be populated
...
--
-- NOTE: `question_type_id_snapshot` stores the sys_dropdown_table.id at submission time.
-- sys_dropdown_table rows should be soft-deactivated (is_active = 0), never hard-deleted,
-- so historical answers remain resolvable. For extra historical safety the app may also
-- denormalise the value into text_answer's JSON metadata, but the FK is the source of truth.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fbk_answers` (
  ...
  `question_type_id_snapshot`    INT UNSIGNED NOT NULL COMMENT 'FK → sys_dropdown_table.id; snapshot of fbk_questions.question_type_id at submission time (key=fbk_answers.question_type_snapshot)',
  `category_id_snapshot`         INT UNSIGNED DEFAULT NULL,
  `weight_snapshot`              DECIMAL(4,2) NOT NULL DEFAULT 1.00,
  -- Answer payload (populated based on question_type_id_snapshot)
  ...
  `emoji_value`                  VARCHAR(20)  DEFAULT NULL COMMENT 'Emoji_5: angry|sad|neutral|smile|happy — matches public/emoji/*.png asset codes',
  -- Audit
  `created_at`                   TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`                   TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  INDEX `idx_fbk_a_question_type`  (`question_type_id_snapshot`),
  ...
  CONSTRAINT `fk_fbk_a_question_type` FOREIGN KEY (`question_type_id_snapshot`) REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT,
...
-- (No ENUM columns — unchanged from v2.)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `fbk_summary` (
  ...
  `updated_at`                    TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_fbk_s_dedup` (`cycle_feedback_type_id`,`target_user_id_uq`,`target_student_id_uq`,`target_employee_id_uq`,`target_department_id_uq`,`subject_id_uq`,`class_section_id_uq`),
...
-- ========================================================================================================
--
-- NOTE: wherever v2 rules referenced ENUM literals like 'Student' or 'Active', v3 rules should
-- resolve the corresponding sys_dropdown_table.id at runtime (or cache the mapping at boot).
--
-- SUBMISSION RULES
-- ────────────────
-- R1.  Respondent can only submit during cycle window: start_date ≤ NOW() ≤ end_date
--      AND fbk_cycles.status_id = dropdown('fbk_cycles.status','Active').
-- R2.  Eligibility is determined by fbk_relationship_types.context_required_id. Expected values:
--        Class_Section             → respondent and target must share a class-section
...
-- RATING CALCULATION
-- ──────────────────
-- R12. overall_rating is computed server-side on submission per template.overall_rating_method_id:
...
-- INTEGRITY CHECKS
-- ────────────────
-- R20. Exactly ONE respondent_{student,guardian,employee}_id must match respondent_kind_id:
...
-- R21. Exactly ONE target_{user,student,employee,department}_id must match
--      target_type.linked_entity_table_id (dropdown value).
...
-- R23. The sys_dropdown_table row referenced by any fbk_* _id column MUST have the expected `key`
--      (listed in SECTION 0 above). Enforce in Form Requests / model mutators.
-- R24. sys_dropdown_table rows referenced by fbk_* columns must NEVER be hard-deleted. Use is_active = 0
--      to retire an option. Otherwise historical fbk_answers.question_type_id_snapshot breaks.
--
-- ========================================================================================================
--  NEP 2020 USE-CASE MAPPINGS  (unchanged from v2 — logical flows identical)
-- ========================================================================================================
--
...
-- v3.0 (2026-04-09): Removed all MySQL ENUM columns in favour of sys_dropdown_table FKs (generic dropdowns).
--                    Rationale: ENUM locks option sets at DDL level — any addition requires a schema
--                    migration. The Feedback module has many semi-open lists (respondent kinds,
--                    question types, context resolvers, status FSMs, rating methods, scope types)
--                    that schools and future releases will want to extend without DDL changes.
--                    Specific changes:
--                    - fbk_target_types.linked_entity_table          → linked_entity_table_id  (FK → sys_dropdown_table)
--                    - fbk_relationship_types.respondent_kind        → respondent_kind_id       (FK → sys_dropdown_table)
--                    - fbk_relationship_types.context_required       → context_required_id      (FK → sys_dropdown_table)
...
--                    - fbk_cycles.status                             → status_id                (FK → sys_dropdown_table)
--                    - fbk_cycle_feedback_types.scope_type           → scope_type_id            (FK → sys_dropdown_table)
--                    - fbk_cycle_feedback_types.target_population_mode (Auto/Manual)
--                                                                    → is_auto_populated_targets TINYINT(1) boolean
--                    - fbk_responses.respondent_kind                 → respondent_kind_id       (FK → sys_dropdown_table)
--                    - fbk_responses.status                          → status_id                (FK → sys_dropdown_table)
--                    - fbk_answers.question_type_snapshot            → question_type_id_snapshot (FK → sys_dropdown_table)
--                    - Added SECTION 0 listing all sys_dropdown_needs / sys_dropdown_table seeds
--                    - Added R23 / R24 rules about key validation and soft-delete-only of dropdown rows
--                    - Polymorphic design, dedup generated columns, and rating pipeline are UNCHANGED
-- ========================================================================================================
