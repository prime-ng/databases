...
CREATE TABLE IF NOT EXISTS `fbk_relationship_types` (
  ...
  -- Actors
  `respondent_kind`       ENUM('Student','Parent','Teacher','Staff','Admin','Self','Any') NOT NULL
                          COMMENT 'Who provides the feedback',
  ...
  `default_anonymous_to_target` TINYINT(1) NOT NULL DEFAULT 1
                          COMMENT 'Recommended anonymity default for this relationship (peer and self feedback should always be anonymous)',
  -- Audit
  `is_active`             TINYINT(1) NOT NULL DEFAULT 1,
  `created_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `fbk_templates` (
  ...
  `respondent_kind`                     ENUM('Student','Parent','Teacher','Staff','Admin','Self','Any') NOT NULL DEFAULT 'Any'
                                        COMMENT 'Restricts which respondent kinds can use this template',
  `applicable_relationship_codes_json`  JSON DEFAULT NULL
                                        COMMENT 'Optional: array of fbk_relationship_types.code values; if null = any relationship with matching target_type',
  -- Scoring
