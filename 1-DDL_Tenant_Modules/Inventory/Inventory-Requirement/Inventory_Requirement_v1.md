# Inventory Module — Detailed Requirement Document

**Module:** Inventory & Stock Management | **Prefix:** `acc_` (inventory domain within Accounting)
**Database:** tenant_db | **Route:** `/accounting/inventory/*`, `/accounting/stock/*`
**RBS Module:** L — Inventory & Stock Management (50 sub-tasks)
**Date:** 2026-03-19
...
The Inventory module manages **school stock and procurement** — from stationery and lab supplies to sports equipment and furniture. It is a **domain within the Accounting module**, sharing the same voucher engine. Every stock movement (purchase, issue, transfer, adjustment) creates a corresponding voucher in accounting for proper financial tracking.
...
- **Accounting:** Stock movements create Stock Journal / Purchase vouchers via VoucherService
- **Vendor:** Vendor linkage for item procurement (existing `vnd_vendors` module)
- **Fixed Assets:** Asset-type items link to `acc_fixed_assets` for depreciation tracking
...
### In Scope
- Stock groups (hierarchical categories)
- Units of measurement (UOM) with conversion rules
- Stock items (item master with reorder levels, batch/expiry tracking)
- Godowns / storage locations
- Stock entries (inward/outward/transfer/adjustment)
- Procurement workflow (Purchase Requisition → Purchase Order → GRN)
...
- Fixed asset register, depreciation → **Accounting module** (asset domain)
...
|--------|----------|---------------|--------|
| ST.L1.1.1.1 | Define main category and code | `acc_stock_groups.name`, `code` | New |
| ST.L1.1.1.2 | Set parent category | `acc_stock_groups.parent_id` (self-ref) | New |
| ST.L1.1.1.3 | Assign default UOM and tax rules | `acc_stock_groups.default_uom_id` | New |
| ST.L1.1.2.1 | Reorder hierarchy | `acc_stock_groups.sequence` | New |
| ST.L1.1.2.2 | Deactivate category with audit log | `is_active` + activityLog() | New |
| ST.L1.2.1.1 | Enter item name & SKU | `acc_stock_items.name`, `sku` | New |
| ST.L1.2.1.2 | Assign category & UOM | `acc_stock_items.stock_group_id`, `uom_id` | New |
| ST.L1.2.1.3 | Define min/max stock levels | `acc_stock_items.reorder_level`, `reorder_qty`, `min_stock`, `max_stock` | New |
| ST.L1.2.2.1 | Set brand/model | `acc_stock_items.brand`, `model` | New |
| ST.L1.2.2.2 | Enable batch/expiry tracking | `acc_stock_items.has_batch_tracking`, `has_expiry_tracking` | New |

### L2 — Units of Measurement (4 sub-tasks)

| RBS ID | Sub-Task | Entity/Column | Status |
|--------|----------|---------------|--------|
| ST.L2.1.1.1 | Define UOM name | `acc_units_of_measure.name`, `symbol` | New |
| ST.L2.1.1.2 | Set decimal precision | `acc_units_of_measure.decimal_places` | New |
| ST.L2.2.1.1 | Create conversion factors (BOX → PCS) | `acc_uom_conversions.from_uom_id`, `to_uom_id`, `factor` | New |
| ST.L2.2.1.2 | Set effective dates | `acc_uom_conversions.effective_from`, `effective_to` | New |
...
| ST.L5.2.1.1 | Edit quantities | `inv_purchase_order_items.qty` update | New |
...
| RBS ID | Sub-Task | Entity/Column | Status |
|--------|----------|---------------|--------|
| ST.L7.1.1.1 | Update stock ledger | `acc_stock_entries` (type='inward') + StockLedgerService | New |
| ST.L7.1.1.2 | Record supplier details | `acc_stock_entries.party_ledger_id` (vendor) | New |
...
| ST.L8.2.1.2 | Track per department | `inv_stock_issues.department_id` + cost center | New |
...
| ST.L9.2.1.1 | Auto-calc reorder qty | `acc_stock_items.reorder_qty` | New |
...
| RBS ID | Sub-Task | Entity/Column | Status |
|--------|----------|---------------|--------|
| ST.L10.1.1.1 | Assign asset tag | `acc_stock_items.item_type` = 'asset' → link to `acc_fixed_assets` | New |
| ST.L10.1.1.2 | Record warranty info | `acc_stock_items.warranty_months` | New |
...
| RBS ID | Sub-Task | Entity/Column | Status |
|--------|----------|---------------|--------|
| ST.L11.1.1.1 | View item-wise stock | StockReportService.itemWiseBalance() | New |
...
### Tables from Accounting DDL (prefix `acc_`)

#### 4.1 acc_stock_groups
...
| default_uom_id | BIGINT UNSIGNED NULL FK | Default UOM for items in this group |
...
| stock_group_id | BIGINT UNSIGNED FK | FK → acc_stock_groups |
| uom_id | BIGINT UNSIGNED FK | FK → acc_units_of_measure |
...
| purchase_ledger_id | BIGINT UNSIGNED NULL FK | FK → acc_ledgers (Purchase A/c) |
| sales_ledger_id | BIGINT UNSIGNED NULL FK | FK → acc_ledgers (Sales A/c) |
| is_active, created_by, created_at, updated_at, deleted_at | Standard | Standard |
...
| in_charge_employee_id | BIGINT UNSIGNED NULL | Store keeper FK → sch_employees |
| is_active, created_by, created_at, updated_at, deleted_at | Standard | Standard |

#### 4.5 acc_stock_entries
| Column | Type | Description |
|--------|------|-------------|
| id | BIGINT UNSIGNED PK | Primary key |
| stock_item_id | BIGINT UNSIGNED FK | FK → acc_stock_items |
| godown_id | BIGINT UNSIGNED FK | FK → acc_godowns |
| voucher_id | BIGINT UNSIGNED FK | FK → acc_vouchers (ALWAYS linked) |
...
| amount | DECIMAL(15,2) | Total value (qty × rate) |
| batch_number | VARCHAR(50) NULL | Batch tracking |
| expiry_date | DATE NULL | Expiry tracking |
| destination_godown_id | BIGINT UNSIGNED NULL | For transfers: target godown |
| party_ledger_id | BIGINT UNSIGNED NULL | Vendor/dept ledger |
| narration | VARCHAR(500) NULL | Notes |
| is_active, created_by, created_at, updated_at, deleted_at | Standard | Standard |

### New Tables — Procurement (prefix `inv_`)

#### 4.6 acc_uom_conversions
| Column | Type | Description |
|--------|------|-------------|
| id | BIGINT UNSIGNED PK | Primary key |
| from_uom_id | BIGINT UNSIGNED FK | FK → acc_units_of_measure |
| to_uom_id | BIGINT UNSIGNED FK | FK → acc_units_of_measure |
...
|--------|------|-------------|
| id | BIGINT UNSIGNED PK | Primary key |
| item_id | BIGINT UNSIGNED FK | FK → acc_stock_items |
...
| received_qty | DECIMAL(15,3) DEFAULT 0 | Received so far |
...
| voucher_id | BIGINT UNSIGNED NULL FK | FK → acc_vouchers (Stock Journal on accept) |
| is_active, created_by, created_at, updated_at, deleted_at | Standard | Standard |
...
| voucher_id | BIGINT UNSIGNED NULL FK | FK → acc_vouchers (Stock Journal) |
...
│ →Submitted   │     │ →Sent       │     │ →Inspected  │     │ Stock       │
│ →Approved    │     │ →Partial    │     │ →Accepted   │     │ Journal     │
│ →Converted   │     │ →Received   │     │             │     │ Voucher     │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
...
1. **Stock Entry ↔ Voucher Linkage:** Every stock entry MUST have a `voucher_id`. No orphan stock movements.
...
10. **Asset Items:** Items with `item_type = 'asset'` should also create `acc_fixed_assets` entry on GRN acceptance.
...
|-----------|--------|--------|-----------|
| GRN → Accounting | GRN Accepted | Purchase Voucher (Dr Stock-in-Hand, Cr Vendor Creditor) | VoucherService |
| Issue → Accounting | Stock Issued | Stock Journal (Dr Dept Consumption, Cr Stock-in-Hand) | VoucherService |
| Transfer → Accounting | Stock Transfer | Stock Journal (Dr Destination Godown, Cr Source Godown) | VoucherService |
| Adjustment → Accounting | Stock Adjustment | Journal (Dr/Cr Stock-in-Hand, Cr/Dr Stock Adjustment A/c) | VoucherService |
| Inventory → Vendor | Purchase Order | References vnd_vendors for supplier | FK reference |
| Inventory → Fixed Assets | Asset-type GRN | Creates acc_fixed_assets entry | Service call |
...
| Pending PO Report | Outstanding PO items | Vendor, Status |
| GRN Register | All receipts | Date range |
| Reorder Alert Report | Items below reorder level | Current |
| Fast-Moving Items | Top-N consumed items | Period (3/6/12 months) |
| Slow-Moving Items | Items with no movement | Period |
...
### Services (6)
StockLedgerService (post movements, update balances), PurchaseOrderService (PR→PO conversion), GrnPostingService (GRN accept → stock + accounting), ReorderAlertService (check thresholds, auto-PR), StockValuationService (FIFO/WA/LP), InventoryReportService (all reports)
...
inventory.godown.viewAny/create/update
```
