# Account Module — Detailed Requirement Document

**Module:** Accounting | **Prefix:** `acc_` | **Database:** tenant_db | **Route:** `/accounting/*`
**RBS Module:** K — Finance & Accounting (70 sub-tasks)
**Inspired by:** Tally Prime | **Date:** 2026-03-19

---

## 1. Module Overview & Purpose

The Accounting module implements a **Tally-Prime inspired double-entry bookkeeping system** for Indian K-12 schools. Every financial transaction (fee collection, salary payment, stock purchase, expense) flows through a unified **Voucher Engine** as Dr/Cr pairs, ensuring the accounting equation (Assets = Liabilities + Equity) always holds.

**Core Principle:** One voucher engine serves ALL financial domains — accounting, payroll, transport , and inventory. The `acc_vouchers` + `acc_voucher_items` tables are the central nervous system.
...
- Common accounting software used: Tally Prime — our UI/UX mirrors its concepts
...
- Transport Fee & Fine  → **Transport module** (`tpt_*`)

### Boundary Decision: StudentFee Integration
The `acc_ledger_mappings` table bridges Accounting and StudentFee. When a fee payment is received in StudentFee, an event fires and the Accounting module auto-creates a Receipt Voucher:
- Dr Bank/Cash A/c
- Cr Fee Income A/c (per fee head → mapped ledger)

**No `acc_fee_*` tables are created.** Fee structures remain in `fin_*` tables.
...
| ST.K8.1.2.1 | Mark matched items | `acc_bank_reconciliations.is_reconciled` | Covered |
...
| ST.K11.2.1.1 | Synchronize ledgers | TallyExportService.exportLedgers() | Covered |
...
| RBS ID | Sub-Task | Entity/Column | Status |
|--------|----------|---------------|--------|
| ST.K13.1.1.1 | Define HSN/SAC codes for fee heads | `acc_ledgers` with fee head mapping | Covered |
...
| bank_name, bank_account_number, ifsc_code | VARCHAR | Bank details (if bank) |
...
| student_id | BIGINT UNSIGNED NULL | FK → std_students (auto-ledger) |
| employee_id | BIGINT UNSIGNED NULL | FK → acc_employees (auto-ledger) |
...
|--------|------|-------------|
| id | BIGINT UNSIGNED PK | Primary key |
| voucher_number | VARCHAR(50) UNIQUE per FY | Auto-generated |
...
| source_type | VARCHAR(100) NULL | Polymorphic: FeePayment, PayrollRun, etc. |
...
| id | BIGINT UNSIGNED PK | Primary key |
| ledger_id | BIGINT UNSIGNED FK | FK → acc_ledgers |
| source_module | ENUM('Fees','Library','Transport','HR','Vendor','Inventory') | Source module |
| source_type | VARCHAR(100) NULL | e.g., 'FeeHead', 'PayHead' |
...
### 4.13-4.14 Banking Tables
- `acc_bank_reconciliations` — ledger_id, statement_date, closing_balance, statement_path, status, reconciled_by
- `acc_bank_statement_entries` — reconciliation_id, transaction_date, description, reference, debit, credit, balance, is_matched, matched_voucher_item_id

### 4.15-4.17 Fixed Asset Tables
- `acc_asset_categories` — name, code, depreciation_method(SLM/WDV), depreciation_rate, useful_life_years
- `acc_fixed_assets` — name, asset_code, category_id, purchase_date, purchase_cost, salvage_value, current_value, accumulated_depreciation, location, vendor_id, purchase_voucher_id
- `acc_depreciation_entries` — fixed_asset_id, financial_year_id, depreciation_date, depreciation_amount, voucher_id

### 4.18-4.19 Expense Claim Tables
- `acc_expense_claims` — claim_number, employee_id, claim_date, total_amount, status(Draft/Submitted/Approved/Rejected/Paid), approved_by, voucher_id
- `acc_expense_claim_lines` — claim_id, expense_date, ledger_id, description, amount, tax_amount, receipt_path

### 4.20 Export Table
- `acc_tally_export_logs` — export_type, export_date, file_name, exported_by, start_date, end_date, status, error_log
...
acc_account_groups (self-ref parent_id)
    ...
    │       │                              │
    │       │                              ├── acc_financial_years
    │       │                              └── source_type (polymorphic → PayrollRun, FeePayment, etc.)
    │       │
    │       ├── acc_budgets ──── acc_financial_years
    │       │                    acc_cost_centers
    │       │
    │       ├── acc_ledger_mappings (→ Fees, HR, Vendor, Inventory)
    │       │
...
4. **Ledger Balance Computation:** Balance = `opening_balance ± sum(voucher_items for this ledger)`. NEVER store computed balance — always calculate.
5. **System Entity Protection:** Records with `is_system = true` cannot be deleted (28 Tally groups, default ledgers, voucher types).
6. **Fiscal Year Continuity:** Closing balance of FY(n) = Opening balance of FY(n+1). Auto-carried forward on FY close.
7. **Cost Center Optional:** Cost center allocation on voucher items is optional. When used, enables department-wise P&L.
8. **Cancelled Voucher Retention:** Cancelled vouchers are soft-deleted with reason. Never hard-delete for audit trail.
9. **Recurring Template Validation:** Template total_amount must balance (sum Dr = sum Cr in lines).
...
- Ledger code must be unique within active (non-deleted) records
- Account group code must be unique within active records
- Budget amount must be >= 0
- Tax rate must be between 0 and 100
- Asset purchase_cost must be > salvage_value
...
| Source Module | Event/Trigger | Accounting Action |
|-------------|--------------|-------------------|
| StudentFee | FeePaymentReceived | Create Receipt Voucher (Dr Bank/Cash, Cr Fee Income per head) |
| StudentFee | FeeInvoiceGenerated | Create Sales Voucher (Dr Student Debtor, Cr Fee Income) |
| Payroll | PayrollApproved | Create Payroll Journal (Dr Salary Expense, Cr PF/ESI/TDS/Net Payable) |
| Inventory | GoodsReceived | Create Purchase Voucher (Dr Stock-in-Hand, Cr Vendor Creditor) |
| Inventory | StockIssued | Create Stock Journal (Dr Dept Consumption, Cr Stock-in-Hand) |
...
| Accountant | Vouchers, ledgers, reports, bank recon, budgets |
...
| Profit & Loss | Income vs Expense (or Income & Expenditure for trusts) | From date, To date |
...
### Dashboard Widgets
- KPI Cards: Fee Collected MTD, Expenses MTD, Bank Balance, Outstanding Receivables
...
Tally's 28 standard groups + 4 school-specific:
...
### Default Ledgers (9 records)
Cash A/c, Petty Cash, GST Payable, TDS Payable, PF Payable, ESI Payable, PT Payable, Salary Payable, Profit & Loss A/c
...
### Cost Centers (9 records)
Primary Wing, Middle Wing, Senior Wing, Administration, Transport, Sports, Library, Science Lab, Computer Lab
...
### Services (8)
VoucherService, AccountingService (balance calc), ReportService, ReconciliationService, DepreciationService, RecurringJournalService, TallyExportService, FeeIntegrationService

### FormRequests (~15)
Store/Update for: AccountGroup, Ledger, Voucher, CostCenter, Budget, TaxRate, RecurringTemplate, FinancialYear, FixedAsset, ExpenseClaim
