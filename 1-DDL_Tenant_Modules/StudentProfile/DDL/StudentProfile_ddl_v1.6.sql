...
-- we need to create table which will capture all the Subjects opted by the student in that session, because in case of optional subjects, different students can opt different 
-- subjects, so we need to capture Student subject choices. Student can opt subjects at the time of admission or at the time of promotion to next class, we create a separate 
-- table `std_student_opted_subjects` which will capture all the subjects opted by the student in that session, it will have foreign key to `std_student_academic_sessions` table.
