...
**Changes from v2:** Verified ALL tables against `0-DDL_Masters/tenant_db_v2.sql`. Fixed table name singulars. Corrected `sch_employees` enhancement (only 1 existing table, not 3). Documented old DDL migration. Added `sch_categories` as reused table. Fixed `sch_teachers` → `sch_employees.is_teacher=1`.
...
*(Same as v2 — all 70 sub-tasks mapped correctly. No changes.)*
...
#### 4.1-4.12 Core Tables
*(Same as v2 — acc_financial_years, acc_account_groups, acc_ledgers, acc_voucher_types, acc_vouchers, acc_voucher_items, acc_cost_centers, acc_budgets, acc_tax_rates, acc_ledger_mappings, acc_recurring_templates, acc_recurring_template_lines)*
...
| id | BIGINT UNSIGNED PK | Primary key |
| ledger_id | BIGINT UNSIGNED FK | FK → acc_ledgers |
| statement_date | DATE | Voucher date |
| closing_balance | DECIMAL(15,2) | Closing balance amount |
| statement_path | varchar(255) NULL | Reconcilation Statement Path |
| status | ENUM('In Progress','Completed') NOT NULL DEFAULT 'In Progress' | REconcilation Status |
| created_by, created_at, updated_at, deleted_at | Standard | Standard columns |

### 4.13 acc_bank_statement_entries
| Column | Type | Description |
|--------|------|-------------|
| id | BIGINT UNSIGNED PK | Primary key |
| reconciliation_id | BIGINT UNSIGNED FK | FK → acc_bank_reconciliations |
| recurring_template_id | BIGINT UNSIGNED FK | FK → acc_recurring_templates (CASCADE) |
| transaction_date | DATE | Transaction date |
| description | VARCHAR(500) NULL | Human-readable description |
| reference | VARCHAR(255) NULL | Against bank Statement entry |
| debit | DECIMAL(15,2) | Debit Voucher amount |
| credit | DECIMAL(15,2) | Credit Voucher amount |
| balance | DECIMAL(15,2) | Balance amount |
| is_matched | TINYINT(1) | Reconcilation status flag |
| matched_voucher_item_id | BIGINT UNSIGNED FK | FK → acc_voucher_items |
| created_by, created_at, updated_at, deleted_at | Standard | Standard columns |
...
*(Same as v2 — with employee_id → `sch_employees`)*
...
- `acc_expense_claim_lines` — claim_id, expense_date, ledger_id, description, amount, tax_amount, receipt_path
    ...
    CONSTRAINT `expense_claims_journal_entry_id_foreign` FOREIGN KEY (`journal_entry_id`) REFERENCES `journal_entries` (`id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
