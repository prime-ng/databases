...
-- =========================================================================
  ...
  CREATE TABLE IF NOT EXISTS `qns_questions_bank` (
    ...
    `for_quest` TINYINT(1) NOT NULL DEFAULT 0,       -- True if this question is for quest
...
-- This table will store all media (images, videos, audio, pdfs) related to questions, options, explanations, recommendations etc. It will be referenced by qns_question_media_jnt table.
  CREATE TABLE IF NOT EXISTS `qns_media_store` (
    ...
    --`owner_id` INT UNSIGNED NOT NULL, -- question_bank_id or question_option_id based on owner_type
    `class_id` INT UNSIGNED DEFAULT NULL,       -- optional denormalized FK to sch_classes.id for easier querying
    `subject_id` INT UNSIGNED DEFAULT NULL,     -- optional denormalized FK to sch_subjects.id for easier querying
    `lesson_id` INT UNSIGNED DEFAULT NULL,     -- optional denormalized FK to slb_lessons.id for easier querying
    `topic_id` INT UNSIGNED DEFAULT NULL,       -- optional denormalized FK to slb_topics.id for easier querying
    -- `owner_id` INT UNSIGNED NOT NULL, -- question_bank_id or question_option_id based on owner_type
...
-- This table will map questions to topics. using it we can fetch all questions for a topic and also understand the weightage of each question in the topic.
...
-- This table will map questions to competencies. using it we can fetch all questions for a competency and also understand the weightage of each question in the competency.
  CREATE TABLE IF NOT EXISTS `qns_question_performance_category_jnt` (
    ...
    `recommendation_type` INT UNSIGNED NOT NULL,      -- FK to lms_assessment_types.id e.g. 'REVISION','PRACTICE','CHALLENGE'
...
-- Question Statistics Computation Flow
--
