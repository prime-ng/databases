...
--
-- DEPENDENCY TABLE (must exist before this DDL runs):
...
-- =========================================================================
-- SECTION 0: DEPENDENCY — tmp_templates (EXISTING TABLE)
-- =========================================================================
...
--
-- Table structure (from tmp_Template_ddl.sql):
...
CREATE TABLE IF NOT EXISTS `tmp_template_assignments` (
  ...
  `scope_hash`            VARCHAR(80) GENERATED ALWAYS AS (
                            CONCAT(
                              `purpose_id`, ':',
                              `academic_session_id`, ':',
                              COALESCE(CONCAT('C', `class_id`), COALESCE(CONCAT('G', `class_group_id`), 'SCHOOL'))
                            )
                          ) STORED                       COMMENT 'Generated column for uniqueness enforcement across scope types',
  -- Standard
  `is_active`             TINYINT(1) NOT NULL DEFAULT 1,
  `created_by`            INT UNSIGNED DEFAULT NULL      COMMENT 'FK -> sys_users.id',
  `updated_by`            INT UNSIGNED DEFAULT NULL      COMMENT 'FK -> sys_users.id',
  ...
  CONSTRAINT `chk_tmp_ta_scope_target` CHECK (
    NOT (`class_id` IS NOT NULL AND `class_group_id` IS NOT NULL)
  )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
