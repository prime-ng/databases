...
CREATE TABLE IF NOT EXISTS `hst_hostels` (
    ...
    `sick_bay_capacity`    TINYINT UNSIGNED NOT NULL DEFAULT 5 COMMENT 'Capacity of hostels medical room (No. of beds); used by SickBayService::admit() capacity check',
...
-- Conditions:
-- we can create facilities Master table if we want to have predefined set of facilities and then we can store those in JSON array in hostel table.
