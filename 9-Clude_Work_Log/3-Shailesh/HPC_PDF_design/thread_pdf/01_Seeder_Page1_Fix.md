# HPC Third Template — Seeder Page 1 Fix (`seedPage1Third`)

**Date:** 2026-03-17
**File:** `database/seeders/HPCTemplateSeeder.php`
**Method:** `seedPage1Third(int $tId, int $pId)`
...
The third template PDF first page has been shared (`13-HPC-Middle_Form (1)-1.pdf` in this folder).
The `seedPage1Third()` method currently has incorrect rubric groupings — fields that should be on
separate rows are grouped together, and fields that should be on the same row are separate.
Fix them to exactly match the PDF page 1 layout from file "**PDF Reference:** `/Users/bkwork/WorkFolder/1-Development/0-Git_Work/prime-ai_db/databases/9-Clude_Work_Log/3-Shailesh/HPC_PDF_design/thread_pdf/13-HPC-Middle_Form-page-1.pdf`".
...
1. `database/seeders/HPCTemplateSeeder.php` — full `seedPage1Third()` method (lines ~1629–1703)
2. The shared **third template PDF page 1** — `13-HPC-Middle_Form (1)-1.pdf` in this folder

---

## Known Issues in Current `seedPage1Third()`
...
```
Update all subsequent rubric ordinals accordingly.
...
- **Grades shown** — third template = Grade 6, Grade 7, Grade 8 (verify checkboxes in seeder)
- **ATTENDANCE section title** — confirm label matches PDF exactly
- **Any section present in PDF but missing in seeder** — add it

---

## Grade Keys for Third Template

The current seeder uses a `foreach` loop for grade checkboxes. Verify generated `html_object_name` keys:
- `grade_grade6`
- `grade_grade7`
- `grade_grade8`

If the loop generates wrong keys, replace with explicit `ri()` calls:
```php
$r = $this->insertRubric($tId, $pId, $s2, 'GRADE_SELECTION', 'Grade/Class', 3, 1);
$this->ri($r, 'grade_grade6', 1, 'Boolean', 'Grade 6', 'Grade 6', 1);
$this->ri($r, 'grade_grade7', 2, 'Boolean', 'Grade 7', 'Grade 7', 1);
$this->ri($r, 'grade_grade8', 3, 'Boolean', 'Grade 8', 'Grade 8', 1);
```
...
Check:
- [ ] `seedPage1Third()` has NO grouped rubrics that should be separate rows
