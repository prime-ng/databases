# Inventory Module — Detailed Requirement Document v4
...
**Changes from v3:** Completely independent file — all "Same as v2" removed. All table schemas inline. All FK references verified against `tenant_db_v2.sql`. Fixed `sch_departments` → `sch_department` (SINGULAR) in all table schemas.
...
- **SchoolSetup:** Godown in-charge FK → `sch_employees`, department FK → `sch_department` (SINGULAR)
...
| `sch_department` | 476 | PR/issue department (**SINGULAR**) | None |
...
### Cross-Module References (from Accounting)

| Table | Notes |
|-------|-------|
| `acc_tax_rates` | Referenced by `inv_stock_items.tax_rate_id` and `inv_purchase_order_items.tax_rate_id` |
| `acc_vouchers` | Referenced by `inv_stock_entries.voucher_id`, `inv_purchase_orders.voucher_id`, `inv_goods_receipt_notes.voucher_id`, `inv_stock_issues.voucher_id` |
| `acc_ledgers` | Referenced by `inv_stock_items.purchase_ledger_id/sales_ledger_id`, `inv_stock_entries.party_ledger_id` |
| `acc_fixed_assets` | Asset-type items create entries here on GRN acceptance |
...
| department_id | BIGINT UNSIGNED NULL FK | FK → **sch_department** (SINGULAR) |
...
| requested_by | BIGINT UNSIGNED FK | FK → sys_users |
| department_id | BIGINT UNSIGNED FK | FK → **sch_department** (SINGULAR) |
| required_date | DATE | Items needed by |
...
| Inventory → SchoolSetup | Issue department | References `sch_department` (SINGULAR) | FK reference |
...
|-------------------|------------|-------------------|----------|
| Voucher Posting | Accounting | `VoucherServiceInterface`, `acc_vouchers` | New table |
| Tax Rates | Accounting | `acc_tax_rates` | Line 9897 |
| Ledgers | Accounting | `acc_ledgers` | Line 9650 |
| Fixed Assets | Accounting | `acc_fixed_assets` | Line 10189 |
| Vendors | Vendor | `vnd_vendors` | Line 1810 |
| Employees | SchoolSetup | `sch_employees` | Line 955 |
