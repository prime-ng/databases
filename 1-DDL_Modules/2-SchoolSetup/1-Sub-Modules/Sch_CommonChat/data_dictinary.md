...
-- Multi-tenancy: All queries auto-scoped to correct tenant_db
--                by DatabaseTenancyBootstrapper (stancl/tenancy v3.9)
-- MySQL version: 8.0+ required (STORED generated columns with
--                LEAST/GREATEST — not supported on MySQL 5.7)
-- ============================================================
--
...
-- PURPOSE: One row per tenant school. Controls which role-pairs
--          can message each other and sets attachment/retention
--          limits. Seeded with defaults on tenant onboarding.
--          Always query as: SELECT * FROM cht_settings WHERE id = 1 LIMIT 1
-- ============================================================
DROP TABLE IF EXISTS `cht_settings`;
CREATE TABLE `cht_settings` (
    `id`                        BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,

    -- FIELD: allow_student_to_student
    -- When 1, students can initiate DMs to other students in the same school.
    -- Default 0 — most schools require teacher-mediated communication for students.
    `allow_student_to_student`  TINYINT(1) NOT NULL DEFAULT 0,

    -- FIELD: allow_parent_to_parent
    -- When 1, parents can DM other parents. Default 0.
    `allow_parent_to_parent`    TINYINT(1) NOT NULL DEFAULT 0,

    -- FIELD: allow_student_to_parent
    -- When 1, a student can initiate a DM to their own linked parent/guardian.
    -- Default 0. (Parents can always initiate to their child's teachers regardless.)
    `allow_student_to_parent`   TINYINT(1) NOT NULL DEFAULT 0,

    -- FIELD: allow_student_to_staff
    -- When 1, students can DM non-teaching staff (Accountant, Librarian, etc.)
    -- as well as Teachers and Admin. Default 1 (open to staff for practical reasons).
    `allow_student_to_staff`    TINYINT(1) NOT NULL DEFAULT 1,

    -- FIELD: max_group_members
    -- Maximum participants allowed in a single group conversation including the creator.
    -- Hard upper limit enforced at ChatService::createGroup().
    `max_group_members`         SMALLINT UNSIGNED NOT NULL DEFAULT 100,

    -- FIELD: max_attachment_size_mb
    -- Maximum file size (in megabytes) for any single attachment.
    -- Validated server-side against MIME type and byte size.
    `max_attachment_size_mb`    TINYINT UNSIGNED NOT NULL DEFAULT 10,

    -- FIELD: message_retention_days
    -- Auto-purge messages older than this many days (0 = never auto-purge).
    -- Enforcement via artisan command: chat:purge-messages --before=DATE
    `message_retention_days`    SMALLINT UNSIGNED NOT NULL DEFAULT 0,

    -- FIELD: typing_indicator_enabled
    -- When 1, the client shows typing indicators via polling.
    -- Disabling this reduces server polling load for large schools.
    `typing_indicator_enabled`  TINYINT(1) NOT NULL DEFAULT 1,

    -- FIELD: read_receipt_enabled
    -- When 0, cht_message_receipts are still written but the UI does not display them.
    -- Useful for privacy-focused school policies.
...
-- PURPOSE: Per-user notification and privacy preferences for the
--          chat module. Created lazily on first chat access or
--          via upsert. One row per sys_users.id.
-- ============================================================
DROP TABLE IF EXISTS `cht_user_settings`;
CREATE TABLE `cht_user_settings` (
    `id`                             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,

    -- FIELD: user_id
    -- References sys_users.id in the same tenant_db.
    -- UNIQUE enforces one settings row per user.
    `user_id`                        BIGINT UNSIGNED NOT NULL,

    -- FIELD: show_online_status
    -- When 0, the user appears Offline to all other users regardless of
    -- their actual cht_user_presence.last_seen_at value.
    `show_online_status`             TINYINT(1) NOT NULL DEFAULT 1,

    -- FIELD: notify_on_new_message
    -- When 0, no in-app notifications for new chat messages (still
    -- increments unread counts in non-muted conversations).
    `notify_on_new_message`          TINYINT(1) NOT NULL DEFAULT 1,

    -- FIELD: notify_on_mention
    -- When 1, user receives a notification when @mentioned in a group message.
    -- (Phase-2 feature; column seeded now to avoid a migration later.)
    `notify_on_mention`              TINYINT(1) NOT NULL DEFAULT 1,

    -- FIELD: show_message_preview_in_notif
    -- When 0, notification shows only "New message" without body preview.
    -- Privacy option for users sharing devices.
    `show_message_preview_in_notif`  TINYINT(1) NOT NULL DEFAULT 1,
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_cht_user_settings_user_id` (`user_id`),

    CONSTRAINT `fk_cht_user_settings_user_id`
...
-- PURPOSE: Tracks the last time each user pinged the server to
--          support online/offline status display. Uses user_id
--          as PK (one row per user) with an upsert pattern:
--            INSERT INTO cht_user_presence (user_id, last_seen_at, device_type, ...)
--            ON DUPLICATE KEY UPDATE last_seen_at = VALUES(last_seen_at), ...
--          NOT a full audit table — no deleted_at or created_by.
--          Deliberately omits is_active, deleted_at (always current state).
-- ============================================================
DROP TABLE IF EXISTS `cht_user_presence`;
CREATE TABLE `cht_user_presence` (
    -- user_id is both the PK and the FK to sys_users.id
    `user_id`       BIGINT UNSIGNED NOT NULL,

    -- FIELD: last_seen_at
    -- UTC timestamp of the last heartbeat ping. A user is considered
    -- "Online" if last_seen_at > (UTC_TIMESTAMP() - INTERVAL 60 SECOND).
    `last_seen_at`  TIMESTAMP NULL DEFAULT NULL,

    -- FIELD: device_type
    -- Optional: 'web', 'mobile', 'tablet'. Informational only.
    -- Allows showing "Active on mobile" style status in Phase-2.
    ...
    CONSTRAINT `fk_cht_user_presence_user_id`
        FOREIGN KEY (`user_id`) REFERENCES `sys_users` (`id`)
        ON DELETE CASCADE ON UPDATE CASCADE
...
-- ============================================================
DROP TABLE IF EXISTS `cht_conversations`;
CREATE TABLE `cht_conversations` (
    `id`                    BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,

    -- FIELD: conversation_type
    -- 'direct'       — 1:1 private DM between two users.
    -- 'group'        — Named multi-user conversation.
    -- 'announcement' — One-way broadcast; only creator/admins can post.
    `conversation_type`     ENUM('direct', 'group', 'announcement') NOT NULL DEFAULT 'direct',

    -- FIELD: name
    -- NULL for DM conversations (name is derived at query time from the
    -- other participant's sys_users.name). Required for group/announcement.
    `name`                  VARCHAR(150) NULL,

    -- FIELD: description
    -- Optional group or announcement description. NULL for DMs.
    `description`           TEXT NULL,

    -- FIELD: avatar_media_id
    -- Spatie Media Library media.id for the group avatar image.
    -- NULL for DMs and until a group avatar is uploaded.
    `avatar_media_id`       BIGINT UNSIGNED NULL,

    -- FIELD: user_a_id / user_b_id
    -- Only populated for conversation_type = 'direct'.
    -- The service layer MUST ensure user_a_id < user_b_id (LEAST/GREATEST).
    -- NULL for group and announcement types.
    `user_a_id`             BIGINT UNSIGNED NULL,
    `user_b_id`             BIGINT UNSIGNED NULL,

    -- FIELD: dm_pair_hash
    -- STORED generated column: CONCAT(LEAST(user_a_id, user_b_id), '_', GREATEST(...))
    -- for direct conversations; NULL for groups/announcements.
    -- UNIQUE index on this column enforces BR-CHT-001 (one DM per pair).
    -- The hash is a string like "42_178" — intentionally not reversible to a URL.
    `dm_pair_hash`          VARCHAR(40) GENERATED ALWAYS AS (
                                CASE
    ...
    INDEX `idx_cht_conversations_user_a_id`       (`user_a_id`),
    INDEX `idx_cht_conversations_user_b_id`       (`user_b_id`),
    INDEX `idx_cht_conversations_is_archived`     (`is_archived`),

    CONSTRAINT `fk_cht_conversations_user_a_id`
        FOREIGN KEY (`user_a_id`) REFERENCES `sys_users` (`id`)
        ON DELETE SET NULL ON UPDATE CASCADE,

    CONSTRAINT `fk_cht_conversations_user_b_id`
        FOREIGN KEY (`user_b_id`) REFERENCES `sys_users` (`id`)
        ON DELETE SET NULL ON UPDATE CASCADE
...
-- ============================================================
DROP TABLE IF EXISTS `cht_participants`;
CREATE TABLE `cht_participants` (
    `id`               BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- ============================================================
DROP TABLE IF EXISTS `cht_messages`;
...
-- ============================================================
DROP TABLE IF EXISTS `cht_message_receipts`;
CREATE TABLE `cht_message_receipts` (
    `id`          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- ============================================================
DROP TABLE IF EXISTS `cht_attachments`;
CREATE TABLE `cht_attachments` (
    `id`           BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
