...
**DDL Version:** v3 (file header) / v4 (filename — Library_ddl_v4.sql)  
**Tables Reviewed:** 44  
**Total Findings:** 45  
...
The Library module DDL demonstrates ambitious design intent — covering physical circulation, digital resources, fine management, inventory auditing, and ML-ready analytics — but it contains **14 CRITICAL syntax and structural errors** that will cause the entire DDL to fail on execution without correction. The most severe issues are: 
(1) `lib_book_purchases` defines FK constraints for columns that are never declared in the table body; 
(2) the ENUM in `lib_library_status_masters` uses backtick-quoted values instead of single-quoted strings (invalid MySQL syntax); 
(3) all FK references throughout the file use aliased column names like `book_id`, `publisher_id`, etc. that do not exist as PKs — every table PK is simply `id`. An additional pattern of broken partial indexes (`WHERE` clause syntax) will silently fail in MySQL 8 where partial indexes are unsupported. Beyond syntax, there are significant structural gaps: `lib_digital_access_request_types` is referenced but never defined, `lib_digital_access_transactions` is missing the `book_id` and `digital_resource_id` it needs to be useful, and the access-restriction design forces ALL four restriction fields to be NOT NULL, breaking the intended flexibility. The schema needs **significant work before it can be executed** and before production use.

**Readiness Score: REQUIRES-REDESIGN** (DDL will not execute as-is)

---

## Finding Summary Table

| # | Table | Column/Element | Dimension | Severity | Type | Effort |
|---|-------|---------------|-----------|----------|------|--------|
| F-001 | `lib_book_purchases` | `book_id`, `resource_type_id`, etc. | D6 Normalization | CRITICAL | Missing | Significant change |
| F-002 | `lib_library_status_masters` | `status_type` ENUM | D5 Data Types | CRITICAL | Incorrect | Quick-fix |
| F-003 | `lib_book_conditions`, `lib_authors`, `lib_keywords` | Comma before UNIQUE KEY | D1 Naming | CRITICAL | Incorrect | Quick-fix |
| F-004 | All FKs (22 tables) | `book_id`, `publisher_id`, etc. vs `id` | D3 FK Integrity | CRITICAL | Incorrect | Significant change |
| F-005 | `lib_book_condition_jnt` | `CONSTRAINT FOREIGN KEY (col)` syntax | D3 FK Integrity | CRITICAL | Incorrect | Quick-fix |
| F-006 | `lib_book_purchases_items` | Duplicate FK constraint names | D3 FK Integrity | CRITICAL | Incorrect | Quick-fix |
| F-007 | `lib_book_copies` | `is_deleted` in idx_copy_status | D4 Indexes | CRITICAL | Missing | Quick-fix |
| F-008 | `lib_book_copies` | Duplicate `idx_book_id` | D4 Indexes | CRITICAL | Incorrect | Quick-fix |
| F-009 | `lib_transaction_history` | FK on `performed_by` (col is `performed_by_id`) | D3 FK Integrity | CRITICAL | Incorrect | Quick-fix |
| F-010 | `lib_inventory_audit` | FK on `performed_by` (col is `performed_by_id`) | D3 FK Integrity | CRITICAL | Incorrect | Quick-fix |
| F-011 | Index section | Partial indexes `WHERE` clause | D4 Indexes | CRITICAL | Incorrect | Quick-fix |
| F-012 | `lib_background_services` | Table in seed INSERT, never defined | D7 Business Logic | CRITICAL | Missing | Minor refactor |
| F-013 | `lib_membership_types` | Trailing comma before `)` | D1 Naming | CRITICAL | Incorrect | Quick-fix |
| F-014 | All seed INSERTs | Wrong column names in INSERT statements | D1 Naming | CRITICAL | Incorrect | Minor refactor |
| F-015 | `lib_digital_access_request_types` | Table not defined, referenced by FK | D7 Business Logic | HIGH | Missing | Minor refactor |
| F-016 | `lib_books_master` | `cover_image_media_id` VARCHAR(500) | D5 Data Types | HIGH | Incorrect | Quick-fix |
| F-017 | `lib_digital_resource_access_restrictions` | All 4 restriction cols NOT NULL | D6 Normalization | HIGH | Incorrect | Minor refactor |
| F-018 | `lib_transactions` | Missing `book_id` column | D2 Standard Cols | HIGH | Missing | Quick-fix |
| F-019 | `lib_digital_access_transactions` | Missing `book_id`, `digital_resource_id`, `access_request_id` | D2 Standard Cols | HIGH | Missing | Minor refactor |
| F-020 | `lib_reservations` | `queue_position` commented out but in INDEX | D4 Indexes | HIGH | Incorrect | Quick-fix |
| F-021 | `lib_reservations` | `is_renewal_reuest` typo | D1 Naming | HIGH | Incorrect | Quick-fix |
| F-022 | `lib_fine_type` | Missing `is_active` column | D2 Standard Cols | HIGH | Missing | Quick-fix |
| F-023 | `lib_membership_types` | CHECK uses wrong column name | D5 Data Types | HIGH | Incorrect | Quick-fix |
| F-024 | `lib_digital_resources` | `license_count` TINYINT (signed, max 127) | D5 Data Types | HIGH | Incorrect | Quick-fix |
| F-025 | `lib_digital_resources` | `file_size_bytes` INT UNSIGNED (max 4GB) | D5 Data Types | HIGH | Incorrect | Quick-fix |
| F-026 | 7 tables | JSON columns missing `_json` suffix | D1 Naming | HIGH | Incorrect | Minor refactor |
| F-027 | `lib_transactions` | `idx_trans_issued_by` col name mismatch | D4 Indexes | HIGH | Incorrect | Quick-fix |
| F-028 | `lib_reservations` | `idx_reserve_book` references `queue_position` | D4 Indexes | HIGH | Incorrect | Quick-fix |
| F-029 | `lib_members` | Denormalized counters drift risk | D6 Normalization | MEDIUM | Suboptimal | Minor refactor |
| F-030 | `lib_books_master` | `is_available` denormalized | D6 Normalization | MEDIUM | Suboptimal | Minor refactor |
| F-031 | `lib_curricular_alignment`, `lib_reading_behavior_analytics` | `academic_year` VARCHAR vs FK | D3 FK Integrity | MEDIUM | Incorrect | Minor refactor |
| F-032 | `lib_location_master` | `type` ENUM not lookup FK | D5 Data Types | MEDIUM | Suboptimal | Significant change |
| F-033 | `lib_digital_access_transactions` | `access_condition_id` FK to physical condition | D7 Business Logic | MEDIUM | Incorrect | Quick-fix |
| F-034 | `lib_fine_slab_config` | Missing UNIQUE on date range | D6 Normalization | MEDIUM | Missing | Minor refactor |
| F-035 | `lib_digital_resource_access_restrictions` | All 4-col composite index overkill | D4 Indexes | MEDIUM | Suboptimal | Quick-fix |
| F-036 | `lib_collection_health_metrics` | Missing FK constraints for `category_id`, `genre_id` | D3 FK Integrity | MEDIUM | Missing | Quick-fix |
| F-037 | `lib_books_master` | `popularity_rank` TINYINT UNSIGNED max 255 | D5 Data Types | MEDIUM | Incorrect | Quick-fix |
| F-038 | `lib_library_status_masters` | Wrong unique key name + "Inventry" typo | D1 Naming | MEDIUM | Incorrect | Quick-fix |
| F-039 | `lib_book_copies` | UNIQUE KEY names using `unique_` not `uq_` | D1 Naming | MEDIUM | Incorrect | Quick-fix |
| F-040 | `lib_members` | `outstanding_fines` NOT NULL missing + drift | D6 Normalization | MEDIUM | Suboptimal | Minor refactor |
| F-041 | `lib_books_master`, `lib_members` | `language` VARCHAR vs `glb_languages` FK | D9 Localization | LOW | Enhancement | Minor refactor |
| F-042 | `lib_categories`, `lib_genres` | Missing localized name columns | D9 Localization | LOW | Enhancement | Minor refactor |
| F-043 | `lib_curricular_alignment` | Missing NEP 2020 / board-type fields | D9 Localization | LOW | Enhancement | Minor refactor |
| F-044 | `lib_predictive_analytics` | Missing `deleted_at` | D2 Standard Cols | LOW | Missing | Quick-fix |
| F-045 | `lib_reading_behavior_analytics` | Missing `updated_at` | D2 Standard Cols | LOW | Missing | Quick-fix |
...
### Phase 1 — Critical Fixes (Before Production / Before DDL Can Execute)

All 14 CRITICAL findings must be resolved before any deployment. Recommended migration order to respect FK dependencies:

1. **F-002** — Fix `lib_library_status_masters` ENUM syntax (dependency for 9 tables)
2. **F-003** — Fix missing commas in `lib_book_conditions`, `lib_authors`, `lib_keywords`
3. **F-013** — Fix trailing comma in `lib_membership_types`
4. **NT-003** — Create `lib_digital_access_request_types` (dependency for F-015)
5. **NT-005** — Create `lib_background_services` (dependency for F-012 seed data)
6. **F-001** — Add missing columns to `lib_book_purchases`
7. **F-004** — Fix all FK references to use `id` instead of aliased column names (global)
8. **F-005** — Fix `lib_book_condition_jnt` CONSTRAINT syntax
9. **F-006** — Rename duplicate FK constraint names in `lib_book_purchases_items`
10. **F-007** — Fix `lib_book_copies.idx_copy_status` (remove non-existent `is_deleted`)
11. **F-008** — Remove duplicate `idx_book_id` from `lib_book_copies`
12. **F-009** — Fix `lib_transaction_history` FK column name (`performed_by` → `performed_by_id`)
13. **F-010** — Fix `lib_inventory_audit` FK column name
14. **F-011** — Replace all partial indexes (remove WHERE clauses)
15. **F-014** — Fix all seed INSERT column names

### Phase 2 — High Priority (Next Sprint)

16. **F-015** — Create `lib_digital_access_request_types` table (done as NT-003 above)
17. **F-016** — Change `cover_image_media_id` from VARCHAR(500) to INT UNSIGNED
18. **F-017** — Make restriction columns nullable in `lib_digital_resource_access_restrictions`
19. **F-018** — Add `book_id` to `lib_transactions`
20. **F-019** — Complete `lib_digital_access_transactions` (add book_id, digital_resource_id, drop access_condition_id)
21. **F-020 + F-028** — Restore `queue_position` in `lib_reservations`
22. **F-021** — Fix typo `is_renewal_reuest` → `is_renewal_request`
23. **F-022** — Add `is_active` to `lib_fine_type`
24. **F-023** — Fix CHECK constraint in `lib_membership_types`
25. **F-024** — Fix `license_count` to `SMALLINT UNSIGNED NULL`
26. **F-025** — Fix `file_size_bytes` to `BIGINT UNSIGNED`
27. **F-026** — Rename all JSON columns to add `_json` suffix
28. **F-027** — Fix `idx_trans_issued_by` column reference
29. **NT-001** — Create `lib_book_reviews_ratings` table
30. **NT-002** — Create `lib_wishlist` table
31. **NT-004** — Create `lib_library_settings` table

### Phase 3 — Enhancements (Backlog)

**Denormalization & Performance:**
- F-029 — Address denormalized counters in `lib_members`
- F-030 — Remove/manage `is_available` in `lib_books_master`
- F-034 — Add UNIQUE constraint on `lib_fine_slab_config` date ranges
- F-035 — Slim down composite index in `lib_digital_resource_access_restrictions`
- F-040 — Ensure `outstanding_fines` NOT NULL

**Data Type & Structural:**
- F-031 — Migrate `academic_year` VARCHAR to `academic_year_id` FK
- F-032 — Rename `type` to `location_type` in `lib_location_master`
- F-036 — Add FK constraints to `lib_collection_health_metrics`
- F-037 — Change `popularity_rank` to MEDIUMINT
- F-038 — Fix unique key name + typo in `lib_library_status_masters`
- F-039 — Rename UNIQUE KEY prefixes in `lib_book_copies`
- F-033 — Clean up `lib_digital_access_transactions.access_condition_id`

**Localization & Analytics:**
- F-041 — Add language_id FK to `lib_books_master` and `lib_members`
- F-042 — Add regional language name columns to categories/genres
- F-043 — Add NEP 2020 / board-type fields to `lib_curricular_alignment`
- F-044 — Add `deleted_at` to `lib_predictive_analytics`
- F-045 — Add `updated_at` to `lib_reading_behavior_analytics`
