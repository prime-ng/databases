...
-- Version: v5 — Comprehensive fixes + New Tables
...
-- ----------------------------------------------------------------------------
  ...
  -- F-003 fix: added missing comma after INDEX idx_condition_active
  ...
  -- F-003 fix: added missing comma after INDEX idx_author_active
  ...
  -- F-003 fix: added missing comma after INDEX idx_keyword_active
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_location_master` (
    ...
    `location_type` ENUM('Zone', 'Floor', 'Aisle', 'Shelf', 'Rack') NOT NULL,  -- Location type (F-032: renamed from `type` to avoid reserved word)
  ...
  CREATE TABLE IF NOT EXISTS `lib_shelf_locations` (
    ...
    `code`         VARCHAR(30) NOT NULL,           -- Business code (e.g., 'A1-S1-R1')
    `building_id`  INT UNSIGNED NOT NULL,          -- FK to sch_buildings.id
    `zone_id`      MEDIUMINT UNSIGNED NOT NULL,    -- FK to lib_location_master.id, Zone or section
    `floor_id`     MEDIUMINT UNSIGNED NOT NULL,    -- FK to lib_location_master.id, Floor/level in the building
    `aisle_id`     MEDIUMINT UNSIGNED NOT NULL,    -- FK to lib_location_master.id, Aisle identifier
    `rack_id`      MEDIUMINT UNSIGNED NOT NULL,    -- FK to lib_location_master.id, Rack identifier
    `shelf_id`     MEDIUMINT UNSIGNED NOT NULL,    -- FK to lib_location_master.id, Shelf identifier within aisle
...
-- ----------------------------------------------------------------------------
  ...
  -- F-022 fix: added is_active column
  CREATE TABLE IF NOT EXISTS `lib_fine_type` (
    ...
    `name`        VARCHAR(50) NOT NULL, -- Name of the fine type
    `description` VARCHAR(250) NULL,
    `is_active`   TINYINT(1) NOT NULL DEFAULT 1,  -- F-022: Whether this fine type is active
    `created_at`  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at`  TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`  TIMESTAMP NULL,
    UNIQUE KEY `uq_fine_type_code` (`code`),
    UNIQUE KEY `uq_fine_type_name` (`name`),
    INDEX `idx_fineType_active` (`is_active`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

  -- Tab-3.2 : Library Fine Slab Config (Parent)
  -- F-034 fix: added UNIQUE KEY uq_fineSlabConf_memType_fineType_EffFrom
  CREATE TABLE IF NOT EXISTS `lib_fine_slab_config` (
    ...
    `updated_at`          TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`          TIMESTAMP NULL,
    -- NOTE: FK to lib_membership_types added via ALTER TABLE below (after lib_membership_types is created)
    ...
    UNIQUE KEY `uq_fineSlabConf_memType_fineType_EffFrom` (`membership_type_id`, `fine_type_id`, `effective_from`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  -- Fixed duplicate Tab label (was Tab-3.3 in v4)
  ...
  CREATE TABLE IF NOT EXISTS `lib_library_status_masters` (
    ...
    `deleted_at`  TIMESTAMP    NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_lib_status_typeCode` (`status_type`, `code`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  ...
  --   Digital Resource Status  : Available, License_Consumed, License_Expired
  ...
  --   Inventory Audit Status   : In_Progress, Completed, Cancelled
  --   Inventory Audit Detail Status : Found, Missing, Misplaced, Damaged
  --   Digital Access Transaction Status : Active, Expired, Revoked, Completed
...
-- F-004 fix: FK references now use (id) not (publisher_id) / (resource_type_id)
  CREATE TABLE IF NOT EXISTS `lib_books_master` (
    ...
    `cover_image_media_id`       INT UNSIGNED NULL,                   -- F-016: FK to sys_media.id
    ...
    `awards_json`                JSON NULL,                           -- F-026: List of awards won by the book (was awards TEXT)
    ...
    `popularity_rank`            MEDIUMINT UNSIGNED NULL,             -- F-037: Popularity rank (was TINYINT UNSIGNED, max 255)
    ...
    `tags_json`                  JSON NULL,                           -- F-026: Auto-generated tags from AI analysis (was tags)
    `ai_summary`                 TEXT NULL,                           -- AI-generated summary
    `key_concepts_json`          JSON NULL,                           -- F-026: Key concepts extracted from the book (was key_concepts)
    ...
    `updated_at`                 TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`                 TIMESTAMP NULL,
    CONSTRAINT `fk_booksM_publisherId`       FOREIGN KEY (`publisher_id`)       REFERENCES `lib_publishers`(`id`),
    CONSTRAINT `fk_booksM_resourceTypeId`    FOREIGN KEY (`resource_type_id`)   REFERENCES `lib_resource_types`(`id`),
    CONSTRAINT `fk_booksM_coverImageMediaId` FOREIGN KEY (`cover_image_media_id`) REFERENCES `sys_media`(`id`) ON DELETE SET NULL,
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_author_jnt` (
    ...
    `updated_at`   TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`   TIMESTAMP NULL,
    CONSTRAINT `fk_bookAuthorJnt_bookId`   FOREIGN KEY (`book_id`)   REFERENCES `lib_books_master`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookAuthorJnt_authorId` FOREIGN KEY (`author_id`) REFERENCES `lib_authors`(`id`) ON DELETE CASCADE,
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_category_jnt` (
    ...
    `updated_at`  TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`  TIMESTAMP NULL,
    CONSTRAINT `fk_bookCategoryJnt_bookId`     FOREIGN KEY (`book_id`)     REFERENCES `lib_books_master`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookCategoryJnt_categoryId` FOREIGN KEY (`category_id`) REFERENCES `lib_categories`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `uq_bookCategory` (`book_id`, `category_id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_genre_jnt` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL,
    CONSTRAINT `fk_bookGenreJnt_bookId`  FOREIGN KEY (`book_id`)  REFERENCES `lib_books_master`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookGenreJnt_genreId` FOREIGN KEY (`genre_id`) REFERENCES `lib_genres`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `uq_bookGenre` (`book_id`, `genre_id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_subject_jnt` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL,
    CONSTRAINT `fk_bookSubjectJnt_bookId`    FOREIGN KEY (`book_id`)    REFERENCES `lib_books_master`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookSubjectJnt_classId`   FOREIGN KEY (`class_id`)   REFERENCES `sch_classes`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookSubjectJnt_subjectId` FOREIGN KEY (`subject_id`) REFERENCES `sch_subjects`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `uq_bookSubject` (`book_id`, `class_id`, `subject_id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_keyword_jnt` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL,
    CONSTRAINT `fk_bookKeywordJnt_bookId`    FOREIGN KEY (`book_id`)    REFERENCES `lib_books_master`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookKeywordJnt_keywordId` FOREIGN KEY (`keyword_id`) REFERENCES `lib_keywords`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `uq_bookKeyword` (`book_id`, `keyword_id`),
...
--            Removed INDEX idx_book_id which referenced a non-existent column.
  CREATE TABLE IF NOT EXISTS `lib_book_purchases` (
    ...
    `updated_at`   TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`   TIMESTAMP NULL,
    CONSTRAINT `fk_bookPurchase_vendorId` FOREIGN KEY (`vendor_id`) REFERENCES `vnd_vendors`(`id`) ON DELETE RESTRICT,
    INDEX `idx_bookPurchase_vendor` (`vendor_id`),
    INDEX `idx_bookPurchase_date` (`bill_date`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Condition :
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_purchases_items` (
    ...
    `book_amt`             DECIMAL(12,2) NOT NULL DEFAULT 0,  -- Total cost (Price x Quantity)
    ...
    `book_net_amt`         DECIMAL(12,2) NOT NULL DEFAULT 0,  -- Total Book cost including Tax
    `created_at`           TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at`           TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`           TIMESTAMP NULL,
    CONSTRAINT `fk_bookPurchItems_purchId`    FOREIGN KEY (`book_purchase_id`)    REFERENCES `lib_book_purchases`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_bookPurchItems_bookId`     FOREIGN KEY (`book_id`)             REFERENCES `lib_books_master`(`id`) ON DELETE RESTRICT,
    CONSTRAINT `fk_bookPurchItems_resTypeId`  FOREIGN KEY (`resource_type_id`)    REFERENCES `lib_resource_types`(`id`) ON DELETE RESTRICT,
    -- NOTE: FKs for book_copy_id and digital_resource_id added via ALTER TABLE below (forward references)
    INDEX `idx_bookPurchItems_purchaseId` (`book_purchase_id`),
    INDEX `idx_bookPurchItems_bookId` (`book_id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Condition :
...
-- F-004 fix: All FK references now use (id)
  CREATE TABLE IF NOT EXISTS `lib_book_copies` (
    ...
    `current_condition_id` INT UNSIGNED NOT NULL,          -- FK to lib_book_conditions.id
    ...
    UNIQUE KEY `uq_copy_accession`  (`accession_number`),
    UNIQUE KEY `uq_copy_rfid`       (`rfid_tag`),
    CONSTRAINT `fk_bookCopies_bookId`           FOREIGN KEY (`book_id`)              REFERENCES `lib_books_master`(`id`),
    CONSTRAINT `fk_bookCopies_shelfLocationId`  FOREIGN KEY (`shelf_location_id`)    REFERENCES `lib_shelf_locations`(`id`),
    CONSTRAINT `fk_bookCopies_conditionId`      FOREIGN KEY (`current_condition_id`) REFERENCES `lib_book_conditions`(`id`),
    CONSTRAINT `fk_bookCopies_purchaseId`        FOREIGN KEY (`book_purchase_id`)     REFERENCES `lib_book_purchases`(`id`),
    CONSTRAINT `fk_bookCopies_status`            FOREIGN KEY (`status`)               REFERENCES `lib_library_status_masters`(`id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Cross-module fix: file_media_id now references sys_media(id) (not media_files)
  CREATE TABLE IF NOT EXISTS `lib_digital_resources` (
    ...
    `updated_at`            TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`            TIMESTAMP NULL,
    CONSTRAINT `fk_digitalRes_bookId`      FOREIGN KEY (`book_id`)       REFERENCES `lib_books_master`(`id`),
    CONSTRAINT `fk_digitalRes_fileMediaId` FOREIGN KEY (`file_media_id`) REFERENCES `sys_media`(`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_digitalRes_status`      FOREIGN KEY (`status`)        REFERENCES `lib_library_status_masters`(`id`),
  ...
  -- Deferred FKs: lib_book_purchases_items → lib_book_copies and lib_digital_resources
  -- Added here after both referenced tables are defined
  ALTER TABLE `lib_book_purchases_items`
    ADD CONSTRAINT `fk_bookPurchItems_copyId`   FOREIGN KEY (`book_copy_id`)        REFERENCES `lib_book_copies`(`id`) ON DELETE SET NULL,
    ADD CONSTRAINT `fk_bookPurchItems_digResId`  FOREIGN KEY (`digital_resource_id`) REFERENCES `lib_digital_resources`(`id`) ON DELETE SET NULL;
  ...
  CREATE TABLE IF NOT EXISTS `lib_digital_resource_tags` (
    ...
    `updated_at`          TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`          TIMESTAMP NULL,
    CONSTRAINT `fk_digitalResTags_digResId` FOREIGN KEY (`digital_resource_id`) REFERENCES `lib_digital_resources`(`id`) ON DELETE CASCADE,
...
-- ----------------------------------------------------------------------------
  ...
  -- Deferred FK: lib_fine_slab_config → lib_membership_types
  -- Added here after lib_membership_types is defined (forward reference resolved)
  ALTER TABLE `lib_fine_slab_config`
    ADD CONSTRAINT `fk_fineSlabConfig_membershipType`
  ...
  CREATE TABLE IF NOT EXISTS `lib_members` (
    ...
    CONSTRAINT `fk_members_userId`         FOREIGN KEY (`user_id`)           REFERENCES `sys_users`(`id`),
    CONSTRAINT `fk_members_membershipType` FOREIGN KEY (`membership_type_id`) REFERENCES `lib_membership_types`(`id`),
    CONSTRAINT `fk_members_status`         FOREIGN KEY (`status`)             REFERENCES `lib_library_status_masters`(`id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_digital_resource_access_restrictions` (
    ...
    `updated_at`          TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`          TIMESTAMP NULL,
    CONSTRAINT `chk_drar_at_least_one` CHECK (
    ...
    INDEX `idx_drar_resId_active` (`digital_resource_id`, `is_active`),
    INDEX `idx_drar_roleId`       (`role_id`),
    INDEX `idx_drar_userId`       (`user_id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- ----------------------------------------------------------------------------
  ...
  -- Tab-6.1 : Book Reservations & Renewals
  ...
  CREATE TABLE IF NOT EXISTS `lib_reservations` (
    ...
    `queue_position`          SMALLINT UNSIGNED NOT NULL DEFAULT 1,  -- F-020: Position in reservation queue
    ...
    CONSTRAINT `fk_reservation_bookId`       FOREIGN KEY (`book_id`)              REFERENCES `lib_books_master`(`id`),
    CONSTRAINT `fk_reservation_memberId`     FOREIGN KEY (`member_id`)            REFERENCES `lib_members`(`id`),
    CONSTRAINT `fk_reservation_status`       FOREIGN KEY (`status`)               REFERENCES `lib_library_status_masters`(`id`),
    -- NOTE: FK for transaction_id added via ALTER TABLE below (forward reference to lib_transactions)
    CONSTRAINT `fk_reservation_approvedById` FOREIGN KEY (`renewal_approved_by_id`) REFERENCES `sys_users`(`id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Condition :
  ...
  CREATE TABLE IF NOT EXISTS `lib_transactions` (
    ...
    `updated_at`          TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`          TIMESTAMP NULL,
    CONSTRAINT `fk_trans_bookId`          FOREIGN KEY (`book_id`)             REFERENCES `lib_books_master`(`id`),
    CONSTRAINT `fk_trans_copyId`          FOREIGN KEY (`copy_id`)             REFERENCES `lib_book_copies`(`id`),
    CONSTRAINT `fk_trans_memberId`        FOREIGN KEY (`member_id`)           REFERENCES `lib_members`(`id`),
    CONSTRAINT `fk_trans_issuedById`      FOREIGN KEY (`issued_by_id`)        REFERENCES `sys_users`(`id`),
    CONSTRAINT `fk_trans_receivedById`    FOREIGN KEY (`received_by_id`)      REFERENCES `sys_users`(`id`),
    CONSTRAINT `fk_trans_issueCondId`     FOREIGN KEY (`issue_condition_id`)  REFERENCES `lib_book_conditions`(`id`),
    CONSTRAINT `fk_trans_returnCondId`    FOREIGN KEY (`return_condition_id`) REFERENCES `lib_book_conditions`(`id`),
    CONSTRAINT `fk_trans_status`          FOREIGN KEY (`status`)              REFERENCES `lib_library_status_masters`(`id`),
    INDEX `idx_trans_book`        (`book_id`, `status`),
    ...
    INDEX `idx_trans_issuedById`  (`issued_by_id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Condition :
    -- When Issue -> Check max_books_allowed in lib_membership_types; if limit exceeded, member cannot issue.
    -- When Return -> Check pending requests in lib_reservations for that book; notify members by queue position.
  ...
  -- Deferred FK: lib_reservations.transaction_id → lib_transactions(id)
  -- Added here after lib_transactions is defined
  ALTER TABLE `lib_reservations`
    ADD CONSTRAINT `fk_reservation_transId` FOREIGN KEY (`transaction_id`) REFERENCES `lib_transactions`(`id`);
  ...
  CREATE TABLE IF NOT EXISTS `lib_digital_access_transactions` (
    ...
    -- Indexes
    INDEX `idx_digAccTx_member_status`   (`member_id`, `status`),
    INDEX `idx_digAccTx_book_member`     (`book_id`, `member_id`),
    INDEX `idx_digAccTx_digRes`          (`digital_resource_id`, `status`),
    INDEX `idx_digAccTx_accessReq`       (`access_request_id`),
    INDEX `idx_digAccTx_accessWindow`    (`access_start_at`, `access_expires_at`),
    INDEX `idx_digAccTx_downloaded`      (`is_downloaded`, `download_count`),
    INDEX `idx_digAccTx_lastDownloaded`  (`last_downloaded_at`),
    INDEX `idx_digAccTx_lastDownloadIp`  (`last_download_ip`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Conditions :
  ...
  CREATE TABLE IF NOT EXISTS `lib_fines` (
    ...
    `updated_at`                TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`                TIMESTAMP NULL,
    CONSTRAINT `fk_fines_transId`       FOREIGN KEY (`transaction_id`)    REFERENCES `lib_transactions`(`id`),
    CONSTRAINT `fk_fines_memberId`      FOREIGN KEY (`member_id`)         REFERENCES `lib_members`(`id`),
    CONSTRAINT `fk_fines_waivedById`    FOREIGN KEY (`waived_by_id`)      REFERENCES `sys_users`(`id`),
    CONSTRAINT `fk_fines_fineSlabConf`  FOREIGN KEY (`fine_slab_config_id`) REFERENCES `lib_fine_slab_config`(`id`),
    CONSTRAINT `fk_fines_status`        FOREIGN KEY (`status`)            REFERENCES `lib_library_status_masters`(`id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_fine_payments` (
    ...
    CONSTRAINT `fk_finePayments_fineId`       FOREIGN KEY (`fine_id`)        REFERENCES `lib_fines`(`id`),
    CONSTRAINT `fk_finePayments_receivedById` FOREIGN KEY (`received_by_id`) REFERENCES `sys_users`(`id`),
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_transaction_history` (
    ...
    `updated_at`      TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`      TIMESTAMP NULL,
    CONSTRAINT `fk_txHistory_transId`        FOREIGN KEY (`transaction_id`)  REFERENCES `lib_transactions`(`id`),
    CONSTRAINT `fk_txHistory_performedById`  FOREIGN KEY (`performed_by_id`) REFERENCES `sys_users`(`id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_inventory_audit` (
    ...
    `updated_at`       TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`       TIMESTAMP NULL,
    CONSTRAINT `fk_invAudit_performedById` FOREIGN KEY (`performed_by_id`) REFERENCES `sys_users`(`id`),
    CONSTRAINT `fk_invAudit_status`        FOREIGN KEY (`status`)          REFERENCES `lib_library_status_masters`(`id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_inventory_audit_details` (
    ...
    `updated_at`           TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`           TIMESTAMP NULL,
    CONSTRAINT `fk_invAuditDet_auditId`         FOREIGN KEY (`audit_id`)             REFERENCES `lib_inventory_audit`(`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_invAuditDet_copyId`          FOREIGN KEY (`copy_id`)              REFERENCES `lib_book_copies`(`id`),
    CONSTRAINT `fk_invAuditDet_expLocId`        FOREIGN KEY (`expected_location_id`) REFERENCES `lib_shelf_locations`(`id`),
    CONSTRAINT `fk_invAuditDet_actLocId`        FOREIGN KEY (`actual_location_id`)   REFERENCES `lib_shelf_locations`(`id`),
    CONSTRAINT `fk_invAuditDet_condId`          FOREIGN KEY (`condition_id`)         REFERENCES `lib_book_conditions`(`id`),
    CONSTRAINT `fk_invAuditDet_status`          FOREIGN KEY (`status`)               REFERENCES `lib_library_status_masters`(`id`),
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_reading_behavior_analytics` (
    ...
    CONSTRAINT `fk_readBeh_memberId`       FOREIGN KEY (`member_id`)           REFERENCES `lib_members`(`id`),
    CONSTRAINT `fk_readBeh_academicYearId` FOREIGN KEY (`academic_year_id`)    REFERENCES `academic_years`(`id`),
    CONSTRAINT `fk_readBeh_genreId`        FOREIGN KEY (`preferred_genre_id`)  REFERENCES `lib_genres`(`id`),
    CONSTRAINT `fk_readBeh_categoryId`     FOREIGN KEY (`preferred_category_id`) REFERENCES `lib_categories`(`id`),
    INDEX `idx_reading_behavior_member` (`member_id`, `academic_year_id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_popularity_trends` (
    ...
    `created_at`             TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at`             TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT `fk_bookPopTrend_bookId` FOREIGN KEY (`book_id`) REFERENCES `lib_books_master`(`id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_collection_health_metrics` (
    ...
    `created_at`                   TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at`                   TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT `fk_collHealth_categoryId` FOREIGN KEY (`category_id`) REFERENCES `lib_categories`(`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_collHealth_genreId`    FOREIGN KEY (`genre_id`)    REFERENCES `lib_genres`(`id`) ON DELETE SET NULL,
  ...
  CREATE TABLE IF NOT EXISTS `lib_curricular_alignment` (
    ...
    `created_at`              TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at`              TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT `fk_libCurrAlign_academicYearId` FOREIGN KEY (`academic_year_id`) REFERENCES `academic_years`(`id`),
    CONSTRAINT `fk_libCurrAlign_classId`        FOREIGN KEY (`class_id`)         REFERENCES `sch_classes`(`id`),
    CONSTRAINT `fk_libCurrAlign_subjectId`      FOREIGN KEY (`subject_id`)       REFERENCES `sch_subjects`(`id`),
    CONSTRAINT `fk_libCurrAlign_bookId`         FOREIGN KEY (`book_id`)          REFERENCES `lib_books_master`(`id`),
    UNIQUE KEY `uq_curricular_book` (`academic_year_id`, `class_id`, `subject_id`, `book_id`),
  ...
  CREATE TABLE IF NOT EXISTS `lib_engagement_events` (
    ...
    `filters_used_json`    JSON,                                          -- F-026: was filters_used
    ...
    `created_at`           TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT `fk_engEvent_memberId`   FOREIGN KEY (`member_id`)          REFERENCES `lib_members`(`id`),
    CONSTRAINT `fk_engEvent_bookId`     FOREIGN KEY (`book_id`)            REFERENCES `lib_books_master`(`id`),
    CONSTRAINT `fk_engEvent_digResId`   FOREIGN KEY (`digital_resource_id`) REFERENCES `lib_digital_resources`(`id`),
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_book_reviews_ratings` (
    ...
    `approved_by_id` INT UNSIGNED NULL,                            -- FK to sys_users.id
  ...
  -- NT-005 : Background Services Registry (also fixes F-012)
  -- Required so the seed INSERT for 'Book Condition Update' service works.
  CREATE TABLE IF NOT EXISTS `lib_background_services` (
    `id`               INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
