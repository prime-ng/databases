-- =====================================================================
-- SYLLABUS MANAGEMENT MODULE - ENHANCED VERSION 2
-- =====================================================================


  CREATE TABLE IF NOT EXISTS `slb_topic_level_types` (
    `id`                               INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `level`                            TINYINT UNSIGNED NOT NULL,      -- e.g., 0=Topic, 1=Sub-topic, 2=Mini Topic, 3=Micro Topic, 5=Nano Topic
    `code`                             VARCHAR(3) NOT NULL,            -- e.g., (TOP, SBT, MIN, MIC, NAN)
    `name`                             VARCHAR(150) NOT NULL,          -- e.g., (TOPIC, SUB-TOPIC, MINI TOPIC, MICRO TOPIC, NANO TOPIC)
    ...
    UNIQUE KEY `idx_topic_type_homework_release_flag` (`homework_release_flag`),
    UNIQUE KEY `idx_topic_type_quiz_release_flag` (`quiz_release_flag`),
    UNIQUE KEY `idx_topic_type_quest_release_flag` (`quest_release_flag`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Conditions:
  -- This Table will be set by PG_Team and will not be available for change to School.
  -- 0=Topic, 1=Sub-topic, 2=Mini Topic, 3=Micro Topic, 4=Nano Topic, 5=Ultra Topic,
  ...
  CREATE TABLE IF NOT EXISTS `slb_topics` (
    ...
    `level_id`                    INT UNSIGNED NOT NULL DEFAULT 0, -- FK-slb_topic_level_types.id (0=Topic, 1=Sub-topic, 2=Mini Topice etc.)
  ...
  -- Conditions:
    ...
    -- ✅ Enforce at application/service layer:
      ...
      WHERE
        :new_min <= max_percentage AND :new_min >= min_percentage
        OR :new_max >= min_percentage AND :new_max <= max_percentage
        AND is_active = 1
      LIMIT 1;
  ...
  -- Conditions:
    ...
    -- ✅ Enforce at application/service layer:
      SELECT 1
      FROM slb_grade_division_master
      WHERE
        :new_min <= max_percentage AND :new_min >= min_percentage
        OR :new_max >= min_percentage AND :new_max <= max_percentage
        AND is_active = 1
  ...
  -- This table will capture Rules for Extra Periods allowed on Top of Number of Periods available for a Subject of a Class. e.g. Exam Revision, remedial class, etc
  -- We have to decide whether to keep it at Class Level or Section Level. or Both. 
  
  CREATE TABLE IF NOT EXISTS `slb_syllabus_extra_period_rules` (
    ...
    -- Period settings
    `extra_allocation_warning_type`      ENUM('WARN','BLOCK','NONE') NOT NULL DEFAULT 'WARN', -- Type of warning to be sent
    `is_extra_periods_allowed`           TINYINT(1) NOT NULL DEFAULT 1,        -- Allow Teacher to override the periods
    `max_extra_period_allowed_number`    SMALLINT UNSIGNED NOT NULL DEFAULT 0,   -- Max no. of extra periods allowed
    `max_extra_period_allowed_percetage` DECIMAL(5,2) NOT NULL DEFAULT 0.00,   -- Max no. of extra periods allowed
    `is_default`                         TINYINT(1) NOT NULL DEFAULT 0,        -- System Generated or Not (e.g. Exam Revision, remedial class, )
    `default_flag`                       TINYINT(1) GENERATED ALWAYS AS ((case when (`is_default` = 1) then '1' else NULL end)) STORED,
    -- Control
    `is_active`                          TINYINT(1) NOT NULL DEFAULT 1,
    `created_at`                         TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`                         TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    -- Indexes
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_defult_flag` (`default_flag`),
    UNIQUE KEY `uq_sepa_class_section_subject` (`academic_session_id`, `class_id`, `section_id`, `subject_id`),
    CONSTRAINT `fk_sepa_academic_session` FOREIGN KEY (`academic_session_id`) REFERENCES `glb_academic_sessions` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sepa_class` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sepa_section` FOREIGN KEY (`section_id`) REFERENCES `sch_sections` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sepa_subject` FOREIGN KEY (`subject_id`) REFERENCES `sch_subjects` (`id`) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  CREATE TABLE IF NOT EXISTS `slb_syllabus_sequencing` (
    `id`                         INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `academic_session_id`        INT UNSIGNED NOT NULL,
    `class_id`                   INT UNSIGNED NOT NULL,           -- FK to sch_classes.id. NULL = applies to all classes
    `section_id`                 INT UNSIGNED DEFAULT NULL,       -- FK to sch_sections.id. NULL = applies to all sections
    ...
    -- Lesson Sequencing data ----------------------------------------------------------
    `allocation_start_date`      DATE NOT NULL,
    `allocation_end_date`        DATE NOT NULL,
    `total_available_periods`    SMALLINT UNSIGNED NOT NULL DEFAULT 0,   -- No. of Periods for the Topic
    `total_allocated_periods`    SMALLINT UNSIGNED NOT NULL DEFAULT 0,   -- No. of Periods for the Topic
    `total_period_last_sync_at`  TIMESTAMP NULL DEFAULT NULL,   -- When the total periods was last synced from the timetable
    `section_marker`             INT UNSIGNED GENERATED ALWAYS AS (IFNULL(`section_id`,0)) STORED,
    `sequenced_at`               TIMESTAMP NULL DEFAULT NULL,
    `sequenced_by`               INT UNSIGNED DEFAULT NULL,
    `created_by`                 INT UNSIGNED DEFAULT NULL,
    `created_at`                 TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`                 TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_sylSequencing_class_section_subject` (`academic_session_id`, `class_id`, `section_marker`, `subject_id`),
    KEY `idx_sylSequencing_dates` (`scheduled_start_date`, `scheduled_end_date`),
    KEY `idx_sylSequencing_class_subject` (`class_id`, `subject_id`),
    CONSTRAINT `fk_sylSequencing_session` FOREIGN KEY (`academic_session_id`) REFERENCES `sch_org_academic_sessions_jnt` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylSequencing_class` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylSequencing_section` FOREIGN KEY (`section_id`) REFERENCES `sch_sections` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylSequencing_subject` FOREIGN KEY (`subject_id`) REFERENCES `sch_subjects` (`id`) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  CREATE TABLE IF NOT EXISTS `slb_syllabus_schedule` (
    ...
    `topic_id`                 INT UNSIGNED NULL,         -- FK to slb_topics.id (It can be Topic, Sub-Topic, Mini-Topic, Micro-Topic etc.)
    `topic_level_type_id`      INT UNSIGNED NULL,  -- FK to slb_topic_level_types.id
    ...
    `is_completed`           TINYINT(1) NOT NULL DEFAULT 0,  -- New Filed to Mark Completion Status by Teacher
    ...
    CONSTRAINT `fk_sylsched_teacher` FOREIGN KEY (`assigned_teacher_id`) REFERENCES `sch_employees` (`id`) ON DELETE SET NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



  CREATE TABLE IF NOT EXISTS `slb_syllabus_periods_allocation` (
    `id`                       INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `academic_session_id`      INT UNSIGNED NOT NULL,             -- FK to sch_org_academic_sessions_jnt.id
    `date`                     DATE NOT NULL,
    `is_school_open_for_study` TINYINT(1) NOT NULL DEFAULT 1,
    `class_id`                 INT UNSIGNED NOT NULL,             -- FK to sch_classes.id
    `section_id`               INT UNSIGNED NOT NULL,             -- FK to sch_sections.id
    `subject_id`               INT UNSIGNED NOT NULL,             -- FK to sch_subjects.id
    `subject_study_format_id`  INT UNSIGNED NOT NULL,             -- FK to sch_subject_study_format_jnt.id
    `tot_periods_in_day`       INT UNSIGNED NOT NULL,             -- Total number of periods in a day
    `tot_periods_in_week`      INT UNSIGNED NOT NULL,             -- Total number of periods in a week
    `data_created_by`          ENUM('MANUAL','AUTO') NOT NULL DEFAULT 'MANUAL',
    `notes`                    VARCHAR(500) DEFAULT NULL,
    `created_by`               INT UNSIGNED DEFAULT NULL,         -- FK to sch_teachers.id
    `created_at`               TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`               TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_sylperiods_allocation` (`academic_session_id`, `date`, `class_id`, `section_id`, `subject_id`, `subject_study_format_id`),
    CONSTRAINT `fk_sylperiods_session` FOREIGN KEY (`academic_session_id`) REFERENCES `sch_org_academic_sessions_jnt` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylperiods_class` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylperiods_section` FOREIGN KEY (`section_id`) REFERENCES `sch_sections` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylperiods_subject` FOREIGN KEY (`subject_id`) REFERENCES `sch_subjects` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylperiods_study_format` FOREIGN KEY (`subject_study_format_id`) REFERENCES `sch_subject_study_format_jnt` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_sylperiods_teacher` FOREIGN KEY (`created_by`) REFERENCES `sch_teachers` (`id`) ON DELETE SET NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  -- Removed Fields from Table slb_topic_level_types : Filed Name - `can_be_used_for_homework_release`, `homework_release_flag`, `can_be_used_for_quiz_release`, `quiz_release_flag`, `can_be_used_for_quest_release`, `quest_release_flag`
  -- 


-- =================================================================================================================================================================================================
-- Syllabus Completion Control
-- =================================================================================================================================================================================================
-- Class : {7th  }      Section : { A   }          Subject : { Science                                         }                                                              [Filter] [Search]
-- -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- |Lesson / Topic/ Sub-Topic / Mini Topic                                                                                  | Release Linked on Completion |   Completion Status
-- |                                                                                                                        | Homework |  Quiz   |  Quest  |         
-- -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Lesson -1                                                                                                                |          |         |  Link   |  Completed / Pending
--   Topic-1                                                                                                                |          |         |         |
--     Sub-Topic-1                                                                                                          |          |  Link   |         |
--      Mini-Topic-1                                                                                                        |   Link   |         |         |
--      Mini-Topic-2                                                                                                        |   Link   |         |         | 
