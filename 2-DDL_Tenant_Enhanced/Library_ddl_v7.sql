...
-- ----------------------------------------------------------------------------
  ...
  -- Conditions:
  -- If `fine_amt_calc_type` is 'BookCost', then `max_fine_cap` should be 'BookCost'.
  -- IF `fine_amt_calc_type` is 'BookCost', then in table `lib_fine_slab_details`; `fine_rate` should be 'BookCost' and `rate_type` should be 'Fixed', `calculation_type` should be 'Per_Book'.
  ...
  CREATE TABLE IF NOT EXISTS `lib_fine_slab_details` (
    ...
    `to_day`              TINYINT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `chk_lib_fineSlabDetails_toDay_ge_fromDay` CHECK (`to_day` >= `from_day`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  -- Conditions:
  -- If (`lib_fine_slab_config`.fine_amt_calc_type` = 'BookCost', then in table `lib_fine_slab_details`; `fine_rate` should be 'BookCost' and `rate_type` should be 'Fixed', `calculation_type` should be 'Per_Book'.
  ...
  -- Condition :
  -- Keep `is_system` = 1 for all below status codes.
...
-- -------------------------------------
  ...
  -- New Table: lib_digital_access_request_types
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_members` (
    ...
    `status`                          SMALLINT UNSIGNED NOT NULL,           -- FK to lib_library_status_masters.id
    `is_suspended`                    TINYINT(1) DEFAULT 0,                 -- Whether member is suspended
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_physical_book_requests` (
    ...
    `status`                  SMALLINT UNSIGNED NOT NULL,      -- FK to lib_library_status_masters.id
    `queue_position`          INT UNSIGNED NOT NULL DEFAULT 0, -- Position in the reservation queue
  ...
  -- Condition :
    ...
    -- Check `lib_digital_resources` table for below :
    -- 1. If Member is Student then check `can_student_download`, it should be 1.
    -- 2. If Member is Teacher then check `can_teacher_download`, it should be 1.
    -- 3. if Member is Staff then check `can_staff_download`, it should be 1.
    -- Check `digital_access_days` in `lib_membership_types` master table and calculate the number of days for digital access (Library team can Edit the days).
    -- According to above conditions, member can request for digital access for Read only / Download.
    -- If Request is for Renewal (Renewal Access Request) then Check renewal_allowed, max_renewals, max_books_allowed in lib_membership_types.
    -- Check `can_restricted_members_view_list` in `lib_membership_types` master table and check `is_restricted_member` in `lib_members` table.
    -- Check `is_suspended` in `lib_members` table, If Memeber is suspended then member cannot request for digital access.
    -- Check `lib_digital_resource_access_restrictions` table whether member is allowed to access the selected digital resource.
    -- Whenn REquest is Approved by Library Incharge, then send Notification to member and create `lib_digital_access_transactions` record.
    -- Notification will be sent on below Events :
  ...
  CREATE TABLE IF NOT EXISTS `lib_digital_access_transactions` (
    ...
    -- Access Window
    `access_type`               ENUM('Download', 'Read_Online') NOT NULL DEFAULT 'View_Online',
    ...
    `last_download_device`      ENUM('Desktop', 'Mobile', 'Tablet', 'Other') NULL,
    `last_download_user_agent`  VARCHAR(500)    NULL,                         -- User Agent  (Chrome, Safari, Firefox, IE, Edge, Opera, etc.)
    ...
    `revoked_by_system`        TINYINT(1)      NOT NULL DEFAULT 0,            -- True if revoked by system (Backgroupnd Service)
  ...
  -- Conditions :
    ...
    --   On Access Window (Student Portal / Library Module foe Staff): Wheever Access / Download; update coresponding Fileds like - `last_accessed_at`, Update all Download Tracking Fileds

  -- Tab-6.5 : Fines
  CREATE TABLE IF NOT EXISTS `lib_fines` (
    `id`                        INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    `transaction_type`          ENUM('Digital', 'Physical') NOT NULL,
    `transaction_id`            INT UNSIGNED NULL,                             -- FK to lib_transactions.id
    `digital_transaction_id`    INT UNSIGNED NULL,                             -- FK to lib_digital_access_transactions.id
    `member_id`                 INT UNSIGNED NOT NULL,                         -- FK to lib_members.id
    `fine_type`                 SMALLINT UNSIGNED NOT NULL,                    -- FK to lib_fine_type.id
    `amount`                    DECIMAL(10,2) NOT NULL CHECK (amount >= 0),
    `days_overdue`              INT NULL DEFAULT 0,
    `calculated_from`           DATE NULL,
    `calculated_to`             DATE NULL,
    ...
    CONSTRAINT `fk_lib_fines_digTransId`    FOREIGN KEY (`digital_transaction_id`) REFERENCES `lib_digital_access_transactions`(`id`),
  ...
  -- Conditions :
  -- at Returning a Book - Check below conditions :
  -- Check If Returning date > Due Date + (lib_membership_types.grace_period_days) days then Fine will be applicable as per 'fine_slab_config', If Returning date < Due Date + (lib_membership_types.grace_period_days) days then No Fine will be applicable
  -- Check Book_conditions_json : If `return_condition_id` <> `issue_condition_id` then Fine may applicable (Will be decided by Libreary Staff)
  -- Fine Slab will be used from 'fine_slab_config' & `lib_fine_slab_details`
...
-- ----------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lib_transaction_history` (
    ...
    `transaction_id`            INT UNSIGNED NULL,                             -- FK to lib_transactions.id
    `digital_transaction_id`    INT UNSIGNED NULL,                             -- FK to lib_digital_access_transactions.id
    `physical_book_action_type` ENUM('Issued', 'Returned', 'Renewed', 'Marked_Lost', 'Condition_Updated') NOT NULL,
    `digital_book_action_type`  ENUM('Access_Granted', 'Access_Revoked', 'Access_Expired', 'Access_Extended') NOT NULL,
    ...
    CONSTRAINT `fk_lib_txHistory_digitalTransId` FOREIGN KEY (`digital_transaction_id`)  REFERENCES `lib_digital_access_transactions`(`id`),
    INDEX `idx_lib_txHistory_transaction` (`transaction_id`),
    INDEX `idx_lib_txHistory_performed`   (`updated_at`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  -- Condition : `status` is a FK to lib_library_status_masters.id
  -- Set `actual_location_id` Then Set `condition_id` Then User will Scan all the Book from the Selected Location with Selected Condition
  -- When Scan Book Copy Code, Fetch Information from `lib_book_copies` : `expected_location_id` = lib_book_copies.`shelf_location_id`, `lib_inventory_audit_details`.`status` = 'Found'
  -- Edit can be done only by Library_Head
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
  ...
  -- Tab-8.6 : Engagement Events (Tracks granular user interactions with the library system for detailed behavior analysis.)
...
-- ----------------------------------------------------------------------------
  ...
  -- When any Student provide rating on a Book, Rating will be updated automatically in lib_books_master.student_rating/student_rating_coun.
  -- When any Faculty provide rating on a Book, Rating will be updated automatically in lib_books_master.academic_rating
  ...
  -- Wishlist will be displayed on Book in Student Portal / Library Module for Staff & Faculty.
  -- Student / Staff / Faculty can add a Book to their Wishlist OR can remove a Book from their Wishlist.


  -- In V6 below Table name was `lib_library_settings`; in V7 renamed it to `lib_library_config`
  CREATE TABLE IF NOT EXISTS `lib_library_config` (
    `id`               INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
    `academic_year_id` INT UNSIGNED NULL,                          -- NULL = global default; non-null = year-specific override
    `setting_key`      VARCHAR(100) NOT NULL,                      -- 'LIBRARY_BUDGET'
  ...
  -- CONDITION :
  -- User can only update Setting_value & Description. No other fields can be updated.
  -- User can not add a new Entry, can only update Values of existing Entries.
...
-- Change Log
-- --------------------------------------------------------------------------------------------------------------------------------------------------
-- 2026-06-18 (Changes after Migration Creation by Tarun)
-- Addedd Condition on `lib_fine_slab_config`
-- Added New Fields in Table : `lib_members` (`is_suspended` TINYINT(1) DEFAULT 0,)
-- Edit ENUM Option for `access_type` to ENUM('Download', 'Read_Online') in Table : `lib_digital_access_transactions`
-- Added Filed : `revoked_by_system` TINYINT(1) NOT NULL DEFAULT 0, in Table : `lib_digital_access_transactions`
-- Added New Fields in Table : `lib_fines`
--          - Field `transaction_type`, `digital_transaction_id`
--          - Addedd CONSTRAINT `fk_lib_fines_digTransId`    FOREIGN KEY (`digital_transaction_id`) REFERENCES `lib_digital_access_transactions`(`id`),
--          - Edit `days_overdue` to make it NULL (Previously it was NOT NULL)
--          - Edit `transaction_id` to make it NULL (Previously it was NOT NULL)
--          - Edit `calculated_from` to make it NULL (Previously it was NOT NULL)
--          - Edit `calculated_to` to make it NULL (Previously it was NOT NULL)
