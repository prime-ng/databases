...
The Module Overview is a static informational screen. No database queries are executed on page load.
...
- The system MUST provide a master configuration to define Lessons, Topics, and their hierarchical relationships
- The system MUST support multiple competency frameworks (NEP 2020, NCF, Bloom's Taxonomy) and map them to curriculum content
- The system MUST enable dynamic scheduling of topics to specific dates, sections, and teachers
- The system MUST provide real-time progress tracking comparing planned vs actual teaching delivery
- The system MUST generate compliance audit reports with radar charts and weighted competency analysis
- The system MUST automate content release to students based on prerequisite completion and scheduled dates
- The system MUST enforce role-based access control across all syllabus management screens
- The system MUST support soft-delete and audit logging for all curriculum entities
- The system MUST provide resource readiness audits to ensure lessons have required multimedia assets
...
|-----------|-------------|-------------|
| `slb_lessons` | Core lesson/chapter definitions with textbook mapping | Master |
| `slb_topics` | Hierarchical topic tree with materialized path | Master |
| `slb_topic_level_types` | Topic depth level definitions (topic, sub-topic, mini, micro, nano) | Master |
| `slb_competencies` | Hierarchical competency definitions aligned to NEP/NCF | Master |
| `slb_competency_types` | Competency category definitions (Knowledge, Skill, Attitude) | Master |
| `slb_topic_competency_jnt` | Junction mapping topics to competencies with weightage | Master |
| `slb_bloom_taxonomy` | Bloom's revised taxonomy 6-level hierarchy | Cognitive |
| `slb_cognitive_skill` | Granular cognitive skills mapped to Bloom levels | Cognitive |
| `slb_ques_type_specificity` | Question type specificity linked to cognitive skills | Cognitive |
| `slb_question_types` | Question format definitions (MCQ, Essay, etc.) | Cognitive |
| `slb_complexity_level` | Difficulty level definitions for adaptive testing | Cognitive |
| `slb_performance_categories` | Percentage bands with AI-triggered actions | Master |
| `slb_grade_division_master` | Grade/division scales for report card printing | Master |
| `slb_syllabus_schedule` | Date-bound topic scheduling to sections and teachers | Planning |
| `slb_syllabus_periods_allocation` | Daily period allocation per class-section-subject | Planning |

### External Module Dependencies

| Module | Nature of Dependency |
|--------|---------------------|
| **SchoolSetup** | Required — Provides `Class`, `Subject`, `Section`, `AcademicSession`, `Employee` models |
| **Library / SyllabusBooks** | Optional — Provides textbook data for lesson book linking |
| **LmsExam** | Optional — Exam scopes reference lessons and topics |
| **LmsHomework** | Optional — Homework can be assigned at topic level |
| **LmsQuiz** | Optional — Quizzes scoped to topics with auto-release triggers |
| **LmsQuests** | Optional — Learning quests scoped to curriculum units |
