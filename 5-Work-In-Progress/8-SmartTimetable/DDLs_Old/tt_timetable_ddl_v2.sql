...
CREATE TABLE `tt_class_mode_rule` (
  ...
  CONSTRAINT `fk_cmr_mode` FOREIGN KEY (`mode_id`) REFERENCES `tt_timetable_mode`(`id`),
...
CREATE TABLE `tt_class_subgroup` (
  ...
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_tt_group_code` (`class_group_id`,`code`),
...
CREATE TABLE `tt_generation_run` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_gr_mode` FOREIGN KEY (`mode_id`) REFERENCES `tt_timetable_mode`(`id`),
  CONSTRAINT `fk_gr_period_set` FOREIGN KEY (`period_set_id`) REFERENCES `tt_period_set`(`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
