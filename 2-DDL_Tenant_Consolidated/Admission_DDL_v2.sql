...
CREATE TABLE IF NOT EXISTS `adm_document_checklist` (
  ...
  `max_size_kb`        INT UNSIGNED     NOT NULL DEFAULT 5120,            -- Maximum upload file size in KB; default 5 MB
  `sort_order`         TINYINT UNSIGNED NOT NULL DEFAULT 0,               -- Display order in document checklist UI
  `is_active`          TINYINT(1)       NOT NULL DEFAULT 1,               -- Soft enable/disable
  --`created_by`         BIGINT UNSIGNED  NOT NULL,                       -- sys_users.id — creator
  --`updated_by`         BIGINT UNSIGNED  NOT NULL,                       -- sys_users.id — last editor
  `created_at`         TIMESTAMP        NULL,                             -- Record creation timestamp
...
CREATE TABLE IF NOT EXISTS `adm_quota_config` (
  ...
  `reserved_seats`     SMALLINT UNSIGNED NOT NULL DEFAULT 0, -- RTE mandated minimum (e.g., 25% for RTE)
...
CREATE TABLE IF NOT EXISTS `adm_enquiries` (
  ...
  `lead_source`        ENUM('Website','Walk-in','Campaign','Referral','Social_Media','Phone','Other') NOT NULL DEFAULT 'Walk-in', -- 'How the lead was captured',
  `status`             ENUM('New','Assigned','Contacted','Interested','Not_Interested','Callback','Converted','Duplicate') NOT NULL DEFAULT 'New', -- 'Lead CRM status',
...
CREATE TABLE IF NOT EXISTS `adm_entrance_test_candidates` (
  ...
  `application_id`        BIGINT UNSIGNED     NOT NULL, -- FK → adm_applications
  `roll_no`               VARCHAR(20)         NULL, -- Test hall roll number; auto-generated on candidate list generation
...
CREATE TABLE IF NOT EXISTS `adm_withdrawals` (
  ...
  `refund_status`             ENUM('Not_Eligible','Pending','Approved','Paid')
                                                  NOT NULL DEFAULT 'Not_Eligible', -- Not_Eligible = no fee paid or outside window; Pending → Approved → Paid
...
CREATE TABLE IF NOT EXISTS `adm_promotion_records` (
  ...
  `result`                    ENUM('Promoted','Detained','Transferred','Alumni','Left')
                                                  NOT NULL, -- Promotion outcome; Detained = same class next session; Left = no new record
...
CREATE TABLE IF NOT EXISTS `adm_transfer_certificates` (
  ...
  `conduct`               ENUM('Excellent','Good','Satisfactory','Poor')
                                              NOT NULL DEFAULT 'Good', -- Student conduct grade for TC
...
CREATE TABLE IF NOT EXISTS `adm_behavior_incidents` (
  `id`                        BIGINT UNSIGNED     NOT NULL AUTO_INCREMENT, -- Primary key
  `student_id`                INT UNSIGNED        NOT NULL, -- FK → std_students.id; enrolled student involved
  ...
  `incident_type`             ENUM('Bullying','Cheating','Disruption','Absenteeism','Vandalism','Violence','Misconduct','Other')
                                                  NOT NULL, -- Type of disciplinary incident
  `severity`                  ENUM('Low','Medium','High','Critical')
                                                  NOT NULL, -- Critical = auto NTF dispatched to principal + parent
  ...
  `status`                    ENUM('Open','Action_Taken','Closed','Escalated')
                                                  NOT NULL DEFAULT 'Open', -- Incident resolution status
