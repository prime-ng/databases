...
--   Layer 1: bha_rating_scales, bha_categories, bha_interventions
--            (no deps on other bha_* tables — foundation entities)
--   Layer 2: bha_rating_levels → bha_rating_scales
--            bha_criteria → bha_categories
--   Layer 3: bha_class_category_jnt → bha_categories + sch_classes
--            bha_assessment_periods → sch_org_academic_sessions_jnt
--            bha_config → bha_rating_scales + sch_org_academic_sessions_jnt
--   Layer 4: bha_assessments → bha_assessment_periods + sch_employees + sch_class_section_jnt
--            bha_audit_log (standalone — polymorphic, no FK deps)
--   Layer 5: bha_assessment_ratings → bha_assessments + std_students + bha_criteria + bha_rating_levels
--            bha_student_remarks → bha_assessments + std_students
--            bha_computed_scores → std_students + bha_categories + bha_assessment_periods
--            bha_incidents → std_students + sch_employees + bha_categories + bha_criteria
--   Layer 6: bha_incident_witnesses_jnt → bha_incidents
--            bha_incident_intervention_jnt → bha_incidents + bha_interventions
--
...
--   • bha_class_category_jnt maps to sch_classes directly (NOT sch_class_groups_jnt
...
--   • bha_audit_log is IMMUTABLE — no updated_at, no deleted_at
...
--   bha_assessment_ratings (raw teacher ratings per student per criterion)
...
--     → UPSERT to bha_computed_scores (cached)
--
...
--   • bha_config is NOT seeded — school must explicitly configure
--
-- v2 CHANGES FROM v1:
--   • bha_rating_scales: added `code`, `grade_type`, `min_rating`, `max_rating`
--     columns; removed `grade_boundaries_json` (grade mapping now derived from
--     min/max rating + bha_rating_levels numeric_value ranges)
--
...
-- =========================================================================
-- LAYER 1 — No dependencies on other bha_* tables
-- These are the foundation entities that other bha_* tables reference.
-- They can be created in any order among themselves.
-- =========================================================================


-- =========================================================================
-- TABLE 1: bha_rating_scales
-- =========================================================================
...
-- (stored in bha_rating_levels) with labels and numeric values.
--
-- EXAMPLES:
...
--     academic session (linked via bha_config.rating_scale_id)
...
--     (c) Validate that bha_rating_levels.numeric_value falls within bounds
...
--     via CASCADE on bha_rating_levels.rating_scale_id
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_rating_levels.rating_scale_id → this table (1:N — levels within scale)
--   • bha_config.rating_scale_id → this table (active scale for session)
--
-- SEED DATA: One default scale ("5-Point Behavioural Scale") with code "5_POINT",
--   grade_type "letter", min_rating 1.0, max_rating 5.0
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_rating_scales` (
...
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_criteria.category_id → this table (1:N — criteria within category)
--   • bha_class_category_jnt.category_id → this table (N:M — category ↔ classes)
--   • bha_computed_scores.category_id → this table (scores per category)
--   • bha_incidents.category_id → this table (optional incident categorisation)
--   • self-reference: parent_id → bha_categories.id (hierarchy)
--
-- ON DELETE:
--   • parent_id ON DELETE SET NULL — orphans sub-categories to top-level
--   • bha_criteria uses ON DELETE CASCADE — deleting a category removes its criteria
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_categories` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  KEY `idx_bha_cat_parent` (`parent_id`),
  KEY `idx_bha_cat_polarity` (`polarity`),
  CONSTRAINT `fk_bha_cat_parent_id` FOREIGN KEY (`parent_id`) REFERENCES `bha_categories` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Behavioural categories (Classroom Engagement, Respect, etc.)';


-- =========================================================================
-- TABLE 3: bha_interventions
-- =========================================================================
...
--
-- HOW IT WORKS:
--   • When a teacher logs an incident (bha_incidents), they can select one or
--     more interventions from this master list via bha_incident_intervention_jnt
--   • Teachers can also write free-text intervention notes directly on the
--     incident record (bha_incidents.intervention_notes) in addition to or
...
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_incident_intervention_jnt.intervention_id → this table (N:M junction
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_interventions` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
--     assessment grid (bha_assessment_ratings.rating_level_id → this table)
...
--
-- ON DELETE:
--   • CASCADE from bha_rating_scales — deleting a scale removes all its levels
--   • bha_assessment_ratings.rating_level_id uses ON DELETE SET NULL — if a
--     level is deleted, existing ratings become unlinked (NULL = not rated)
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_rating_levels.rating_scale_id → bha_rating_scales.id (parent)
--   • bha_assessment_ratings.rating_level_id → this table (selected level)
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_rating_levels` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `rating_scale_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_rating_scales.id',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_level` (`rating_scale_id`, `sort_order`),
  KEY `idx_bha_level_scale` (`rating_scale_id`),
  CONSTRAINT `fk_bha_level_rating_scale_id` FOREIGN KEY (`rating_scale_id`) REFERENCES `bha_rating_scales` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Individual levels within a rating scale (e.g., Outstanding=5, Good=3)';


-- =========================================================================
-- TABLE 5: bha_criteria
-- =========================================================================
...
--
-- HOW IT WORKS:
--   • Each criterion belongs to exactly one category (bha_categories)
...
--     score. Weights are proportional (see bha_categories.weight explanation)
...
--   criterion_score = rating.numeric_value (from bha_rating_levels)
...
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_criteria.category_id → bha_categories.id (parent category)
--   • bha_assessment_ratings.criterion_id → this table (rated criterion)
--   • bha_incidents.criterion_id → this table (optional incident linkage)
--
-- ON DELETE:
--   • CASCADE from bha_categories — deleting a category removes all its criteria
--   • bha_assessment_ratings uses ON DELETE RESTRICT — cannot delete a criterion
--     that has existing ratings (must deactivate via is_active instead)
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_criteria` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `category_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_categories.id — parent category',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  KEY `idx_bha_criteria_category` (`category_id`),
  CONSTRAINT `fk_bha_criteria_category_id` FOREIGN KEY (`category_id`) REFERENCES `bha_categories` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- =========================================================================
-- TABLE 6: bha_class_category_jnt
-- =========================================================================
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_class_category_jnt` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `class_id` INT UNSIGNED NOT NULL COMMENT 'FK to sch_classes.id — cross-module',
  `category_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_categories.id',
  `is_active` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Soft enable/disable',
  `created_by` BIGINT UNSIGNED NOT NULL COMMENT 'sys_users.id',
  `updated_by` BIGINT UNSIGNED NOT NULL COMMENT 'sys_users.id',
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_class_cat` (`class_id`, `category_id`),
  KEY `idx_bha_cc_class` (`class_id`),
  KEY `idx_bha_cc_category` (`category_id`),
  CONSTRAINT `fk_bha_cc_class_id` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bha_cc_category_id` FOREIGN KEY (`category_id`) REFERENCES `bha_categories` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Junction: maps which behavioural categories apply to which classes';


-- =========================================================================
-- TABLE 7: bha_assessment_periods
-- =========================================================================
...
--     bha_computed_scores. No further edits allowed. Scores become available
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_assessment_periods` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  KEY `idx_bha_period_session` (`academic_session_id`),
  KEY `idx_bha_period_term` (`academic_term_id`),
  KEY `idx_bha_period_status` (`status`),
  CONSTRAINT `fk_bha_period_session_id` FOREIGN KEY (`academic_session_id`) REFERENCES `sch_org_academic_sessions_jnt` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_period_term_id` FOREIGN KEY (`academic_term_id`) REFERENCES `sch_academic_term` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
--     - 'weighted_average': Weighted by bha_categories.weight (DEFAULT — recommended)
...
--
-- NOT SEEDED:
--   bha_config is NOT seeded during tenant onboarding. When first accessed,
...
--
-- UNIQUE CONSTRAINT:
--   uq_bha_config_session ensures ONE config per academic session.
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_config` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `academic_session_id` SMALLINT UNSIGNED NOT NULL COMMENT 'FK to sch_org_academic_sessions_jnt.id — one config per session',
  `rating_scale_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_rating_scales.id — active rating scale for this session',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_config_session` (`academic_session_id`),
  KEY `idx_bha_config_session` (`academic_session_id`),
  KEY `idx_bha_config_scale` (`rating_scale_id`),
  CONSTRAINT `fk_bha_config_session_id` FOREIGN KEY (`academic_session_id`) REFERENCES `sch_org_academic_sessions_jnt` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_config_scale_id` FOREIGN KEY (`rating_scale_id`) REFERENCES `bha_rating_scales` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- individual rating entries (bha_assessment_ratings) together.
--
...
--     assess ONLY the categories mapped to their class via bha_class_category_jnt.
--
-- ASSESSMENT WORKFLOW (FSM):
--   draft ──submit()──▶ submitted ──approve()──▶ reviewed ──lock()──▶ locked
--     ▲                     │                       │
...
--
-- INDEX STRATEGY:
--   • idx_bha_assess_status (period_id, status) — Powers the Principal's
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_assessments` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `period_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_assessment_periods.id',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_assessment` (`teacher_id`, `class_section_id`, `period_id`),
  KEY `idx_bha_assess_period` (`period_id`),
  KEY `idx_bha_assess_teacher` (`teacher_id`),
  KEY `idx_bha_assess_cs` (`class_section_id`),
  KEY `idx_bha_assess_reviewed_by` (`reviewed_by`),
  KEY `idx_bha_assess_status` (`period_id`, `status`),
  CONSTRAINT `fk_bha_assess_period_id` FOREIGN KEY (`period_id`) REFERENCES `bha_assessment_periods` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_assess_teacher_id` FOREIGN KEY (`teacher_id`) REFERENCES `sch_employees` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_assess_cs_id` FOREIGN KEY (`class_section_id`) REFERENCES `sch_class_section_jnt` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_assess_reviewed_by` FOREIGN KEY (`reviewed_by`) REFERENCES `sch_employees` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Header record for a teachers assessment submission per class-section per period';


-- =========================================================================
-- TABLE 10: bha_audit_log
-- =========================================================================
...
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_audit_log` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_bha_audit_entity` (`entity_type`, `entity_id`),
  KEY `idx_bha_audit_changed_by` (`changed_by`),
  KEY `idx_bha_audit_changed_at` (`changed_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- =========================================================================
-- TABLE 11: bha_assessment_ratings
-- =========================================================================
...
--   • Each cell in the grid corresponds to one bha_assessment_ratings row
...
--
-- UNIQUE CONSTRAINT:
--   uq_bha_rating (assessment_id, student_id, criterion_id) prevents
...
--
-- INDEX STRATEGY:
--   • idx_bha_rating_lookup (student_id, criterion_id, assessment_id) — The
...
--
-- ON DELETE:
--   • CASCADE from bha_assessments — deleting an assessment removes all its ratings
--   • RESTRICT from std_students — cannot delete a student with existing ratings
--   • RESTRICT from bha_criteria — cannot delete a criterion with existing ratings
--   • SET NULL from bha_rating_levels — if a level is deleted, rating becomes unlinked
--
-- AUDIT:
--   Every update to rating_level_id is captured by the Eloquent Observer
--   → written to bha_audit_log with old and new values
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_assessment_ratings` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `assessment_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_assessments.id — parent assessment',
  `student_id` INT UNSIGNED NOT NULL COMMENT 'FK to std_students.id — student being rated (cross-module)',
  `criterion_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_criteria.id — criterion being rated',
  `rating_level_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'FK to bha_rating_levels.id — selected rating level; NULL = not yet rated',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_rating` (`assessment_id`, `student_id`, `criterion_id`),
  KEY `idx_bha_rating_assess` (`assessment_id`),
  KEY `idx_bha_rating_student` (`student_id`),
  KEY `idx_bha_rating_criterion` (`criterion_id`),
  KEY `idx_bha_rating_level` (`rating_level_id`),
  KEY `idx_bha_rating_lookup` (`student_id`, `criterion_id`, `assessment_id`),
  CONSTRAINT `fk_bha_rating_assessment_id` FOREIGN KEY (`assessment_id`) REFERENCES `bha_assessments` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bha_rating_student_id` FOREIGN KEY (`student_id`) REFERENCES `std_students` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_rating_criterion_id` FOREIGN KEY (`criterion_id`) REFERENCES `bha_criteria` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_rating_level_id` FOREIGN KEY (`rating_level_id`) REFERENCES `bha_rating_levels` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Individual rating entries — core fact table (one per student per criterion per assessment)';


-- =========================================================================
-- TABLE 12: bha_student_remarks
-- =========================================================================
-- Overall teacher remarks per student per assessment. This is DIFFERENT from
-- the per-criterion remarks on bha_assessment_ratings.remark.
--
-- PURPOSE:
...
--
-- UNIQUE CONSTRAINT:
--   uq_bha_remark (assessment_id, student_id) — one overall remark per student
--   per assessment. If the teacher updates their remark, it overwrites.
--
-- RELATIONSHIP TO OTHER TABLES:
--   • bha_student_remarks.assessment_id → bha_assessments.id (parent assessment)
--   • bha_student_remarks.student_id → std_students.id (cross-module)
--   • Consumed by BehaviouralReportService for student reports and parent views
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_student_remarks` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `assessment_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_assessments.id — parent assessment',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_remark` (`assessment_id`, `student_id`),
  KEY `idx_bha_remark_assess` (`assessment_id`),
  KEY `idx_bha_remark_student` (`student_id`),
  CONSTRAINT `fk_bha_remark_assessment_id` FOREIGN KEY (`assessment_id`) REFERENCES `bha_assessments` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bha_remark_student_id` FOREIGN KEY (`student_id`) REFERENCES `std_students` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Overall remarks per student per assessment (not per-criterion)';


-- =========================================================================
-- TABLE 13: bha_computed_scores
-- =========================================================================
...
-- bha_assessment_ratings and cached here for fast retrieval.
--
-- WHY CACHE SCORES:
--   Computing scores requires joining bha_assessment_ratings with bha_criteria,
--   bha_categories, bha_rating_levels, and potentially averaging across multiple
...
--   1. SELECT all bha_assessment_ratings for this student + period
...
--   7. Map scores to grades using bha_rating_scales min_rating/max_rating bounds
--   8. UPSERT into bha_computed_scores (uses ON DUPLICATE KEY UPDATE)
--
...
--
-- UNIQUE CONSTRAINT:
--   uq_bha_score (student_id, category_id, period_id) — one score per
--   student per category per period. UPSERT on recomputation.
--
-- INDEX STRATEGY:
--   • idx_bha_score_lookup (student_id, period_id) — Primary query pattern
--     for "get all category scores for this student in this period"
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_computed_scores` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `student_id` INT UNSIGNED NOT NULL COMMENT 'FK to std_students.id (cross-module)',
  `category_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_categories.id — category scored',
  `period_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_assessment_periods.id — assessment period',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_score` (`student_id`, `category_id`, `period_id`),
  KEY `idx_bha_score_student` (`student_id`),
  KEY `idx_bha_score_category` (`category_id`),
  KEY `idx_bha_score_period` (`period_id`),
  KEY `idx_bha_score_lookup` (`student_id`, `period_id`),
  CONSTRAINT `fk_bha_score_student_id` FOREIGN KEY (`student_id`) REFERENCES `std_students` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_score_category_id` FOREIGN KEY (`category_id`) REFERENCES `bha_categories` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_score_period_id` FOREIGN KEY (`period_id`) REFERENCES `bha_assessment_periods` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Cached computed scores per student per category per period (recalculated on demand)';


-- =========================================================================
-- TABLE 14: bha_incidents
-- =========================================================================
...
--                May trigger parent notification (depends on bha_config threshold).
...
--   incident.severity ≥ bha_config.parent_notification_threshold
...
--
-- INDEX STRATEGY:
--   • idx_bha_incident_timeline (student_id, incident_date) — Primary query
--     for "show me all incidents for this student in chronological order"
--   • idx_bha_incident_type and idx_bha_incident_severity — For analytics:
...
--   • category_id → bha_categories.id (OPTIONAL — links incident to a category)
--   • criterion_id → bha_criteria.id (OPTIONAL — links to specific criterion)
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_incidents` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `category_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'FK to bha_categories.id — optional linked category',
  `criterion_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'FK to bha_criteria.id — optional linked criterion',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  KEY `idx_bha_incident_student` (`student_id`),
  KEY `idx_bha_incident_reporter` (`reported_by`),
  KEY `idx_bha_incident_category` (`category_id`),
  KEY `idx_bha_incident_criterion` (`criterion_id`),
  KEY `idx_bha_incident_timeline` (`student_id`, `incident_date`),
  KEY `idx_bha_incident_type` (`incident_type`),
  KEY `idx_bha_incident_severity` (`severity`),
  CONSTRAINT `fk_bha_incident_student_id` FOREIGN KEY (`student_id`) REFERENCES `std_students` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_incident_reported_by` FOREIGN KEY (`reported_by`) REFERENCES `sch_employees` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_bha_incident_category_id` FOREIGN KEY (`category_id`) REFERENCES `bha_categories` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_bha_incident_criterion_id` FOREIGN KEY (`criterion_id`) REFERENCES `bha_criteria` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- =========================================================================
-- TABLE 15: bha_incident_witnesses_jnt
-- =========================================================================
...
--
-- UNIQUE CONSTRAINT:
--   uq_bha_witness (incident_id, witness_type, witness_id) prevents the
--   same person from being listed as a witness twice for the same incident.
--
-- ON DELETE:
--   • CASCADE from bha_incidents — deleting an incident removes its witnesses
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_incident_witnesses_jnt` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `incident_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_incidents.id — parent incident',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_witness` (`incident_id`, `witness_type`, `witness_id`),
  KEY `idx_bha_witness_incident` (`incident_id`),
  CONSTRAINT `fk_bha_witness_incident_id` FOREIGN KEY (`incident_id`) REFERENCES `bha_incidents` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Junction: witnesses to behavioural incidents (polymorphic student/staff)';


-- =========================================================================
-- TABLE 16: bha_incident_intervention_jnt
-- =========================================================================
...
--   from the bha_interventions master list (displayed as checkboxes in the UI).
...
--
-- COMPLEMENTARY TO bha_incidents.intervention_notes:
--   • bha_incidents.intervention_notes = free-text description of what was done
--     (for cases where the teacher doesn't select from the predefined list)
--   • bha_incident_intervention_jnt = structured selection from the master list
...
--   bha_incidents WHERE category = 'Aggressive or Bullying'
--
-- UNIQUE CONSTRAINT:
--   uq_bha_inc_int (incident_id, intervention_id) prevents the same intervention
--   from being linked to the same incident twice.
--
-- ON DELETE:
--   • CASCADE from bha_incidents — deleting an incident removes its intervention links
--   • RESTRICT from bha_interventions — cannot delete an intervention that has been
--     used in incident records (must deactivate via is_active instead)
-- =========================================================================
CREATE TABLE IF NOT EXISTS `bha_incident_intervention_jnt` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `incident_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_incidents.id — incident',
  `intervention_id` BIGINT UNSIGNED NOT NULL COMMENT 'FK to bha_interventions.id — intervention applied',
  ...
  `deleted_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft delete',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_bha_inc_int` (`incident_id`, `intervention_id`),
  KEY `idx_bha_ii_incident` (`incident_id`),
  KEY `idx_bha_ii_intervention` (`intervention_id`),
  CONSTRAINT `fk_bha_ii_incident_id` FOREIGN KEY (`incident_id`) REFERENCES `bha_incidents` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bha_ii_intervention_id` FOREIGN KEY (`intervention_id`) REFERENCES `bha_interventions` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
--  │                                                                        │
--  │  bha_rating_scales ──┬── bha_rating_levels (5 levels per scale)        │
--  │                     └── bha_config (active scale for session)          │
--  │                              │                                         │
--  │  bha_categories ──┬── bha_criteria (58 criteria across 9 categories)   │
--  │                  └── bha_class_category_jnt ── sch_classes             │
--  │                                                                        │
--  │  bha_interventions (9 predefined actions)                              │
--  │  bha_assessment_periods (Term 1, Term 2, Annual, etc.)                 │
--  │                                                                        │
--  └────────────────────────────────────────────────────────────────────────┘
--
--  ┌── ASSESSMENT WORKFLOW (teachers fill, principal reviews) ──────────────┐
--  │                                                                        │
--  │  Teacher selects: Period + Class-Section → creates bha_assessments     │
--  │       │                                                                │
--  │       ├── bha_assessment_ratings (students × criteria grid)            │
--  │       │     Each cell = one row: student_id + criterion_id → level_id  │
--  │       │     UNIQUE (assessment_id, student_id, criterion_id)           │
--  │       │                                                                │
--  │       └── bha_student_remarks (overall remark per student)             │
--  │             UNIQUE (assessment_id, student_id)                         │
--  │                                                                        │
--  │  Status: draft ──▶ submitted ──▶ reviewed ──▶ locked                   │
--  │                                                                        │
--  │  Every rating change ──▶ bha_audit_log (immutable)                     │
--  │                                                                        │
--  └────────────────────────────────────────────────────────────────────────┘
--
--  ┌── SCORE COMPUTATION (triggered on assessment approval) ────────────────┐
--  │                                                                        │
--  │  bha_assessment_ratings (raw teacher ratings)                          │
...
--  │       │                                                                │
--  │       ▼                                                                │
--  │  bha_computed_scores (cached: score + grade per student per category)  │
--  │       │                                                                │
--  │       ▼                                                                │
...
--  │                                                                         │
--  │ Teacher logs: bha_incidents (student, date, type, severity, description │
--  │       │        location, intervention_notes, attachments_json)          │
--  │       │                                                                 │
--  │       ├── bha_incident_witnesses_jnt (polymorphic: student or staff)    │
--  │       │                                                                 │
--  │       ├── bha_incident_intervention_jnt → bha_interventions             │
--  │       │                                                                 │
...
-- ============================================================================
--   Layer 1 (3): bha_rating_scales, bha_categories, bha_interventions
--   Layer 2 (2): bha_rating_levels, bha_criteria
--   Layer 3 (3): bha_class_category_jnt, bha_assessment_periods, bha_config
--   Layer 4 (2): bha_assessments, bha_audit_log
--   Layer 5 (4): bha_assessment_ratings, bha_student_remarks, bha_computed_scores, bha_incidents
--   Layer 6 (2): bha_incident_witnesses_jnt, bha_incident_intervention_jnt
-- ============================================================================
