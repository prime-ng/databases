-- =====================================================================
-- SCHOOL SETUP MODULE - VERSION 3.0 (PRODUCTION-GRADE)
-- SUB-MODULE: CLASS, EMPLOYEE, INFRA SETUP & COMMONCHAT
-- =====================================================================
...
-- 1. Consolidated all 3 Sub-Modules into 1 DDL Schema as structured in Applicaiton
-- =====================================================================

-- ===========================================================================
-- 1 - CLASS SETUP SUB-MODULE (sch)
-- ===========================================================================
  ...
  CREATE TABLE IF NOT EXISTS `sch_class_group_subject_options_jnt` (
    ...
    `subject_option_id` INT unsigned NOT NULL,    -- FK to 'sch_subject_options'
    `ordinal` tinyint DEFAULT NULL,               -- will have sequence order (Auto Update by Drag & Drop)
    `class_id` int unsigned NOT NULL,             -- FK to 'sch_classes'
    `section_id` int unsigned NULL,               -- FK to 'sch_sections' (Optional)
    `subject_id` int unsigned NOT NULL,           -- FK to 'sch_subjects'
    `subject_option_code` varchar(20) NOT NULL,   -- e.g. 'CS','PE','FA' etc. (Added new). (Denormalization for easy access in timetable)
    `subject_option_name` varchar(50) NOT NULL,   -- e.g. 'Computer Science', 'Physical Education', 'Fine Arts' etc. (Denormalization for easy access in timetable)
...
-- ===========================================================================
  ...
  -- we need to create a table to annual session for leave which will start from Jan and will end in Dec. This table will be used to link the leave with the annual session and also to calculate the leave balance for the employees. This table will be linked with the sch_employee_leave_balance table and sch_employee_leave_applications table.
    ...
    CREATE TABLE IF NOT EXISTS `sch_staff_attendance_types` (
      ...
      `is_active`             TINYINT(1) NOT NULL DEFAULT 1,
      `created_by`            INT UNSIGNED DEFAULT NULL,  -- FK to sys_users.id (who created this type)
      `created_at`            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      `updated_at`            TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      `deleted_at`            TIMESTAMP NULL,
      ...
      CONSTRAINT `fk_attendance_created_by` FOREIGN KEY (`created_by`) REFERENCES `sys_users` (`id`) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
  ...
  -- ---------------------------------------------------------------------------
  -- sch_employees_profile   (enhanced in v4 — fixed UNIQUE, added created_by)
  -- ---------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `sch_employee_emergency_contacts` (
    ...
    `is_active`             TINYINT(1)   NOT NULL DEFAULT 1,
    `created_by`            INT UNSIGNED DEFAULT NULL,    -- FK to sys_users.id for who created this emergency contact record
    `created_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`            TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
  ...
  CREATE TABLE IF NOT EXISTS `sch_employee_bank_details` (
    ...
    `notes`                 VARCHAR(255) DEFAULT NULL,
    `is_active`             TINYINT(1)   NOT NULL DEFAULT 1,
    `created_by`            INT UNSIGNED DEFAULT NULL,    -- FK to sys_users.id for who created this bank detail record
    `created_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`            TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
  ...
  CREATE TABLE IF NOT EXISTS `sch_employee_documents` (
    ...
    `is_active`             TINYINT(1)   NOT NULL DEFAULT 1,
    `created_by`            INT UNSIGNED DEFAULT NULL,    -- FK to sys_users.id for who created this document record
    `created_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`            TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`            TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
