...
CREATE TABLE IF NOT EXISTS `msh_source_components` (
  `id`            INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `code`          VARCHAR(30) NOT NULL           COMMENT 'EXAM, HOMEWORK, QUIZ, QUEST, UT1, UT2, HY, etc.',
...
CREATE TABLE IF NOT EXISTS `msh_template_scholastic_components` (
  ...
  `source_component_id` INT UNSIGNED NOT NULL         COMMENT 'FK → msh_source_components.id (e.g. EXAM, HOMEWORK, QUIZ, QUEST)',
...
CREATE TABLE IF NOT EXISTS `msh_template_ia_components` (
  `id`                  INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `config_template_id`  INT UNSIGNED NOT NULL         COMMENT 'FK → msh_config_templates.id',
  `ia_component_type_id` INT UNSIGNED NOT NULL        COMMENT 'FK → msh_ia_component_types.id (Notebook, Enrichment, etc.)',
...
CREATE TABLE IF NOT EXISTS `msh_marksheet_schedules` (
  ...
  -- Status (via sys_dropdown_table)
  `status_id`             INT UNSIGNED NOT NULL        COMMENT 'FK → sys_dropdown_table.id (DRAFT, COMPUTED, REVIEWED, PUBLISHED, LOCKED)',
  ...
  CONSTRAINT `fk_msh_ms_status` FOREIGN KEY (`status_id`) REFERENCES `sys_dropdown_table` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
--   → Seeds sys_dropdown_table entries for msh_marksheet_schedules.status_id
