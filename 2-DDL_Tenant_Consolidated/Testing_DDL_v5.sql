...
CREATE TABLE IF NOT EXISTS `tst_users` (
  ...
  `code`          VARCHAR(20) NOT NULL,              -- User code, e.g. 'admin', 'brijesh', 'shailesh'. This will be used as login user id
  `name`          VARCHAR(50) NOT NULL,
  `email`         VARCHAR(100) NOT NULL,
  `password`      VARCHAR(512) NOT NULL,             -- Hashed Password
  `role`          ENUM('Admin','QA_Lead','Tester','Developer') NULL DEFAULT 'Tester', -- v5: used to filter assignment pick-lists for FR-8 (Tester) / FR-9 (Developer)
...
CREATE TABLE IF NOT EXISTS `tst_test_cases` (
  `id`                    INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  `tab_id`                INT UNSIGNED NOT NULL,             -- FK to tst_tabs
...
-- ----------------------------------------------------------------------------
-- SECTION 9 (NEW v5): FORWARD-REFERENCE COLUMN — tst_test_run_results.bug_id
-- ----------------------------------------------------------------------------

-- Links a failing run result to the bug it raised/belongs to (FR-9). Added via
-- ALTER because tst_bugs (defined in Section 6) itself references
-- tst_test_run_results (defined in Section 3).
ALTER TABLE `tst_test_run_results`
  ADD COLUMN `bug_id` INT UNSIGNED NULL AFTER `test_case_id`,
  ADD INDEX `idx_tst_testRunResults_bug` (`bug_id`),
  ADD CONSTRAINT `fk_tst_testRunResults_bug` FOREIGN KEY (`bug_id`) REFERENCES `tst_bugs`(`id`) ON DELETE SET NULL;
