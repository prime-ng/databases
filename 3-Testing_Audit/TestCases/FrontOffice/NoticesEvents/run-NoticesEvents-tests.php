...
/**
 * Cross-platform Dusk runner for the FrontOffice :: NoticesEvents suite
 * (compound: fof_notices + fof_school_events).
 * Runs natively on Windows and Linux/macOS (no .ps1 / .sh needed).
 *
 * Usage:
 *   php run-NoticesEvents-tests.php [--php=/path/to/php] [--filter=test_noticesevents_23] [--sync-db]
 *
 * Prerequisites (see fof_NoticesEventsValidation_Report.md):
 *   - Copy fof_NoticesEvents_TestCas.php into prime_testing/tests/Browser/ before running.
 *   - FrontOffice must be ENABLED in prime_testing/modules_statuses.json (currently false → routes 404).
...
$filter    = $options['filter'] ?? 'fof_NoticesEvents_TestCas';
...
$proofFile = $proofDir . DIRECTORY_SEPARATOR . "NoticesEvents_dusk_{$timestamp}.log";
...
echo "==================================================================\n";
echo " FrontOffice :: NoticesEvents — Dusk run\n";
echo "  php     : {$phpBinary}\n";
echo "  repo    : {$testRepo}\n";
echo "  filter  : {$filter}\n";
