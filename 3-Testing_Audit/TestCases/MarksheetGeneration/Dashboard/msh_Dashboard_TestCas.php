 ...
 *
 * ONE comprehensive suite per screen (merged foundation + comprehensive — no V1/V2
 * split). The Dashboard aggregates the module's msh_* tables; it owns NO primary
 * table and has NO create/edit/delete matrix. Coverage focuses on: routes/views/
 * aggregation-wiring truth, dashboard render, the six stat widgets + active/inactive
 * breakdown, the 4-pillar combined navigation, the recent-activity tabs, permission
 * gating (D39-MSH), empty-state branches, guest→login redirect, tenancy scope smoke,
 * console/responsive smoke, and the proving tests for the owned audit defects.
 *
 * Owned defects proved here:
 *   - BUG-MSH-001 (P0): routes/api.php apiResource('marksheetgenerations', ...) is DEAD —
 *       RouteServiceProvider::map() maps only web routes AND the controller defines none of
 *       index/store/show/update/destroy. Proven via Route::has(...)===false,
 *       method_exists(...)===false and a getJson() probe accepting {401,403,404,405,500}.
 *   - PERF-MSH-003 (P2): results() eager-loads Student::where('is_active',1)->get() and
 *       Subject::get() with no pagination — documented + proven present in source.
 *   - D39-MSH (P1): tenant.msh-*.view gates are unseeded (super-admin only) — the suite
 *       grants the 5 view gates explicitly to the admin so positive render tests are
 *       deterministic, and proves denial for a fresh limited user.
 *
 * Source of truth (verified 2026-Jul-10):
 *   - Modules/MarksheetGeneration/app/Http/Controllers/MarksheetGenerationController.php
 *   - Modules/MarksheetGeneration/routes/web.php + routes/api.php + Providers/RouteServiceProvider.php
 *   - resources/views/dashboard.blade.php + pages/{configuration,components,scheduling,results}.blade.php
 *   - 2-DDL_Tenant_Consolidated/MarksheetGeneration_DDL_v1.sql (msh_*, Database: tenant_db)
 *
 * DB scope: tenant-side (msh_* prefix, tenant_db) → tenant scaffolding required.
 * Env prereq: MarksheetGeneration ENABLED in modules_statuses.json + APP_ENV=testing.
 *
 * Semantic bands: 01-09 wiring/schema · 10-19 aggregation/render rules ·
 * 40-49 navigation/integration · 50-59 permissions/auth/dead-API ·
 * 60-69 UI/UX · 70-79 edge/empty-state · 90-99 tenancy/console/responsive.
 */
class msh_Dashboard_TestCas extends DuskTestCase
{
    ...
    private const API_ROUTES_FILE  = 'Modules/MarksheetGeneration/routes/api.php';
    private const PROVIDER_FILE     = 'Modules/MarksheetGeneration/app/Providers/RouteServiceProvider.php';
    ...
    /** Stat-card labels rendered by dashboard.blade.php (FR-01). */
    private const STAT_CARD_LABELS = [
        'Marksheet Types', 'Config Templates', 'Schedules',
        'Student Results', 'Schedule Classes', 'Practical Configs',
    ];

    /** 4-pillar combined navigation targets (FR-02). */
    ...
    /** Every msh_* table the dashboard aggregates counts from (verified in DDL v1). */
    ...
    /** REST verbs the dead apiResource would bind to (BUG-MSH-001). */
    private const DEAD_API_METHODS = ['index', 'store', 'show', 'update', 'destroy'];
    ...
    /** BC-DB/BC-BIZ/BC-AUTH — routes, views, aggregation and gate wiring truth. */
    ...
    /** BC-BIZ — dashboard + 4 combined routes registered under the module prefix. */
    ...
    /** TC-P02 — the six primary stat cards render with their labels (FR-01). */
    public function test_dashboard_11_six_primary_stat_cards_render(): void
    {
        $this->browseWithFailureScreenshot('cards', function (Browser $browser): void {
            $this->visitAuthenticated($browser, self::DASHBOARD_PATH);
            foreach (self::STAT_CARD_LABELS as $label) {
                $browser->assertSee($label);
            }
        });
    }

    /** TC-P03 — primary stat values reflect live DB counts (aggregation truth). */
    ...
    /** TC-P04 — the secondary Active/Inactive breakdown row renders (FR-01). */
    ...
    /** TC-P05 — the three recent-activity tab controls are present (FR-03). */
    ...
    /** BC-BIZ — recent results are capped at five and eager-load their relations. */
    public function test_dashboard_16_recent_results_limited_to_five(): void
    {
        ...
        $controller = File::get(base_path(self::CONTROLLER_FILE));
        $this->assertStringContainsString(
            "with(['student', 'classSection.class', 'classSection.section'])",
            $controller
        );
    }

    /** TC-P13 — the today-date badge renders the current year. */
    ...
    /** TC-P06 — the 4-pillar nav links target the combined routes (FR-02). */
    ...
    /** TC-D02 — recent results eager-load the cross-module student + class-section. */
    ...
    /**
     * PERF-MSH-003 — results() eager-loads Student::where('is_active',1)->get() and
     * Subject::get() with no pagination. The page must still render; this proves the
     * current behaviour and documents the unbounded-query risk (P2).
     */
    ...
    /** BC-AUTH — all five gate strings are present verbatim in the controller source. */
    ...
    /**
     * BUG-MSH-001 (P0) — the module apiResource is never registered: routes/api.php
     * declares apiResource('marksheetgenerations', ...) but RouteServiceProvider::map()
     * calls only mapWebRoutes(). Neither the named routes nor the controller methods exist.
     */
    public function test_dashboard_58_api_resource_routes_not_registered(): void
    {
        // (a) None of the apiResource names are registered.
        foreach (self::DEAD_API_METHODS as $action) {
            $this->assertFalse(
        ...
        // (b) The provider maps only web routes (never mapApiRoutes()).
        $provider = File::get(base_path(self::PROVIDER_FILE));
        $this->assertStringContainsString('$this->mapWebRoutes();', $provider);
        $this->assertStringNotContainsString('mapApiRoutes', $provider);

        // (c) api.php still declares the dead apiResource (defect not yet removed).
        $apiRoutes = File::get(base_path(self::API_ROUTES_FILE));
        $this->assertStringContainsString("apiResource('marksheetgenerations'", $apiRoutes);
    }

    /** BUG-MSH-001 (P0) — the controller defines NONE of the REST resource methods. */
    public function test_dashboard_59_controller_missing_rest_methods(): void
    {
        foreach (self::DEAD_API_METHODS as $method) {
            $this->assertFalse(
    ...
    /** TC-P/UX — the Overview tab pane is active by default. */
    ...
    /** TC-P/UX — a scheduling CTA (View All / empty-state) targets the scheduling page. */
    ...
    /** TC-P/UX — a results CTA (View All / empty-state) targets the results page. */
    ...
    /** BUG-MSH-001 (P0) — HTTP probe: the dead API never returns a working 200 resource. */
    ...
    private function assertCombinedPageResolves(string $path, string $context): void
    {
        $this->browseWithFailureScreenshot('combined-' . $context, function (Browser $browser) use ($path, $context): void {
            ...
            $this->assertStringNotContainsString('/login', $current, "{$context} redirected to login (permission/env issue).");
    ...
    {
        $case = 'tab-' . strtolower(str_replace(' ', '-', $tableMarker));
        $this->browseWithFailureScreenshot($case, function (Browser $browser) use ($tableMarker, $emptyMarker): void {
            $this->visitAuthenticated($browser, self::DASHBOARD_PATH);
     ...
     * page (Gate::authorize → 403). Defensive: skip cleanly if a limited user cannot
     * be provisioned or the environment grants a super-admin bypass.
     */
