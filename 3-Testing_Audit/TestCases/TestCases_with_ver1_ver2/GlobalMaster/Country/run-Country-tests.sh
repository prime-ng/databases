#!/usr/bin/env bash
# =============================================================================
# Country (GlobalMaster) — Dusk runner (Linux / WSL / macOS)
# Mirrors the golden reference / committed Prime-Billing runner. Runs the mirrored
# test classes that live under prime_testing:
...
#   - GlobalMaster AND Prime modules ENABLED in modules_statuses.json (both are
#     currently false -> every /global-master/country route 404s).
#   - Central scope: tests run on http://127.0.0.1:8000 (prime_db / global_master_mysql)
#     with NO tenant init.
#
# USAGE
#   ./run-Country-tests.sh [--php <path>] [--v1-only] [--v2-only] [--filter <method>]
# =============================================================================
set -uo pipefail

PHP_BIN="php"
FILTER=""
V1_CLASS="glb_CountryV1_TestCas"
V2_CLASS="glb_CountryV2_TestCas"
RUN_V1=1
RUN_V2=1
...
PROOF_DIR="tests/Browser/Modules/Prime/GlobalMaster/Country/proof"
mkdir -p "$PROOF_DIR"
PROOF_FILE="${PROOF_DIR}/country_run_${TIMESTAMP}.log"

# Clean old screenshots
find "tests/Browser/Modules/Prime/GlobalMaster/Country/screenshots" -type f -name '*.png' -delete 2>/dev/null || true
