...
# Prereqs:
#   - Billing enabled in prime_testing/modules_statuses.json (else 404 on all routes).
#   - Prime/Billing tests run against http://127.0.0.1:8000 (enforced by PrimeDuskTestCase).
#   - bil_tenant_email_schedules migration applied (tests self-skip if table missing).

$projectRoot = Resolve-Path (Join-Path $PSScriptRoot "..\..\..\..\..\..")  # 6 up: Prime/Billing/EmailSchedule
...
if ($Filter) {
    ...
    $duskFilter = "bil_EmailScheduleV1_TestCas"
} elseif ($V2Only) {
    $duskFilter = "bil_EmailScheduleV2_TestCas"
} else {
    $duskFilter = "bil_EmailSchedule"
}
...
try {
    ...
    if ($totalCount -ne 0) {
        ...
        Write-Host "  EmailSchedule RESULTS:" -ForegroundColor Cyan
