...
/**
 * MarksheetGeneration — Scheduling & Lifecycle (V2 comprehensive suite).
 *
 ...
 * Others : msh_schedule_class_jnt, msh_subject_practical_configs, msh_computation_logs
 *
 * Semantic bands: 01-09 schema · 10-19 business rules · 20-29 state machine · 30-39 validation
 *                 40-49 integration/FK · 50-59 permissions · 60-69 UI/UX · 70-79 edge · 90-99 tenancy/security
 *
 * Audit defects proved as tests: BR-MSH-026, BR-MSH-027, BR-MSH-037, BR-MSH-039, BR-MSH-050,
 *   PERF-MSH-001/002, DEP-MSH-001, DOC-MSH-002, SEC-MSH-003/D39-MSH, and BUG-MSH-101 (new).
 *
 * Dusk Browser has no assertStatus(); lifecycle POST endpoints return 302 redirects — this suite
 * asserts DB + activity-log + computation-log side effects (05_ D14). APP_ENV=testing bypasses CSRF.
 */
class msh_SchedulingAndLifecycleV2_TestCas extends DuskTestCase
{
    ...
    {
        $this->assertTrue(Schema::hasTable('msh_marksheet_schedules'));
        $this->assertTrue(Schema::hasColumns('msh_marksheet_schedules', [
            ...
            'unlock_reason', 'unlocked_at', 'unlocked_by', 'is_active', 'created_by', 'updated_by', 'deleted_at',
        ]));
        $this->assertTrue(Schema::hasColumns('msh_schedule_class_jnt', ['schedule_id', 'class_section_id', 'is_active', 'deleted_at']));
        $this->assertTrue(Schema::hasColumns('msh_subject_practical_configs', ['academic_session_id', 'class_id', 'subject_id', 'has_practical', 'theory_max_marks', 'practical_max_marks', 'deleted_at']));
        $this->assertTrue(Schema::hasColumns('msh_computation_logs', ['schedule_id', 'action', 'triggered_by', 'started_at', 'completed_at', 'duration_seconds', 'status', 'remarks']));
        // Immutable audit log — no deleted_at.
        $this->assertFalse(Schema::hasColumn('msh_computation_logs', 'deleted_at'), 'msh_computation_logs is immutable and should have no deleted_at.');
        ...
        $this->assertStringContainsString("->on('sys_dropdowns')", $migration);
    }

    public function test_scheduling_02_status_dropdown_rows_exist_on_sys_dropdowns(): void
    {
        // DOC-MSH-002: the DDL comment says sys_dropdown_table, but the real table is sys_dropdowns.
        $this->assertTrue(Schema::hasTable('sys_dropdowns'));
        $this->assertFalse(Schema::hasTable('sys_dropdown_table'), 'DOC-MSH-002: stale DDL name sys_dropdown_table must not exist.');
        ...
        foreach (['DRAFT', 'COMPUTED', 'REVIEWED', 'PUBLISHED', 'LOCKED'] as $value) {
            ...
            if ($id === null) {
                $this->markTestSkipped("Status dropdown '{$value}' not seeded on sys_dropdowns for key " . self::STATUS_KEY . '.');
            }
    ...
    public function test_scheduling_03_models_configuration_and_relationships(): void
    {
        ...
        $this->assertSame('bool', $casts['is_locked'] ?? null, 'is_locked should cast to bool.');

        $this->assertContains(SoftDeletes::class, class_uses_recursive(SubjectPracticalConfig::class));
        $this->assertNotContains(SoftDeletes::class, class_uses_recursive(ComputationLog::class));
    }

    public function test_scheduling_04_bug_msh_101_scheduleclass_missing_softdeletes(): void
    {
        // Migration declares softDeletes() + column exists ...
        $this->assertStringContainsString('$table->softDeletes()', File::get(base_path(self::MIGRATION_SCJ_FILE)));
        $this->assertTrue(Schema::hasColumn('msh_schedule_class_jnt', 'deleted_at'));
        // ... but the model omits the trait (BUG-MSH-101) ...
        $this->assertNotContains(SoftDeletes::class, class_uses_recursive(ScheduleClass::class));
        // ... while controller + service call soft-delete-only methods → runtime BadMethodCallException.
        $this->assertStringContainsString('onlyTrashed', File::get(base_path(self::SC_CONTROLLER_FILE)));
        $this->assertStringContainsString('ScheduleClass::withTrashed()', File::get(base_path(self::SC_SERVICE_FILE)));
    }
    ...
    public function test_scheduling_10_create_persists_and_logs_stored(): void
    {
        ...
        $this->assertNotNull(MarksheetSchedule::find($schedule->id));
    ...
    public function test_scheduling_12_delete_soft_deletes_and_logs_deleted(): void
    {
        ...
        $this->assertNull(MarksheetSchedule::find($id));
        $this->assertNotNull(MarksheetSchedule::withTrashed()->find($id));
    ...
    public function test_scheduling_13_combined_page_renders_schedule_and_practical_tabs(): void
    {
        $this->browseWithFailureScreenshot('sch-13-tabs', function (Browser $browser): void {
            ...
            $this->assertTrue($this->pageSourceContains($browser, 'ractical') || $this->pageSourceContains($browser, 'heory'), 'Practical configs tab did not render.');
    ...
    public function test_scheduling_15_practical_config_toggle_status_endpoint(): void
    {
        $deps = $this->scheduleDependencies();
        if ($deps['class_id'] === 0 || $deps['subject_id'] === 0) {
            $this->markTestSkipped('Practical config requires a class + subject seed.');
        ...
        $fresh = SubjectPracticalConfig::find($config->id);
    ...
    {
        // The unique key uq_msh_scj_schedule_class exists in the schema even though the model is soft-delete-broken.
        $driver = DB::connection()->getDriverName();
        if ($driver === 'mysql') {
        ...
        // BUG-MSH-101: MarksheetScheduleService::syncClassSections() runs on every create/update with class_section_ids
        // and calls ScheduleClass::withTrashed() → BadMethodCallException on the trait-less model.
        $this->assertStringContainsString('withTrashed()', File::get(base_path(self::SC_SERVICE_FILE)));
        $this->assertStringContainsString('$existing->restore()', File::get(base_path(self::SC_SERVICE_FILE)));
    }
    ...
    public function test_scheduling_17_compute_from_draft_dispatches_and_logs(): void
    {
        ...
        $this->browseWithFailureScreenshot('sch-17-compute', function (Browser $browser) use ($schedule): void {
            $this->authenticate($browser);
    ...
    public function test_scheduling_18_export_endpoint_downloads_or_authorizes(): void
    {
    ...
    public function test_scheduling_20_draft_compute_flips_toward_computed(): void
    {
    ...
    public function test_scheduling_21_computed_review_to_reviewed(): void
    {
        ...
        $this->assertSame($deps['status_ids']['REVIEWED'], (int) MarksheetSchedule::find($schedule->id)->status_id);
        $this->assertTrue(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'REVIEW')->exists());
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_22_reviewed_publish_to_published_and_locks_template(): void
    {
        ...
        $this->assertSame($deps['status_ids']['PUBLISHED'], (int) MarksheetSchedule::find($schedule->id)->status_id);
        $this->assertSame(1, (int) DB::table('msh_config_templates')->where('id', $deps['config_template_id'])->value('is_locked'), 'BR-MSH-037: publish must lock template.');
        $this->assertTrue(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'PUBLISH')->exists());
        $this->forceDeleteSchedule($schedule);
    ...
    public function test_scheduling_23_published_lock_to_locked_sets_is_locked(): void
    {
        ...
        $this->assertSame($deps['status_ids']['LOCKED'], (int) $fresh->status_id);
        $this->assertTrue((bool) $fresh->is_locked);
        $this->assertTrue(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'LOCK')->exists());
        $this->forceDeleteSchedule($schedule);
    }

    public function test_scheduling_24_unlock_with_reason_reverts_to_computed(): void
    {
        $deps = $this->scheduleDependencies();
        ...
        $fresh = MarksheetSchedule::find($schedule->id);
        $this->assertSame($deps['status_ids']['COMPUTED'], (int) $fresh->status_id);
        $this->assertFalse((bool) $fresh->is_locked);
        $this->assertSame($reason, (string) $fresh->unlock_reason);
        $this->assertTrue(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'UNLOCK')->where('remarks', $reason)->exists(), 'BR-MSH-039 audit row.');
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_25_illegal_review_from_draft_rejected(): void
    {
        ...
        $this->postLifecycle($schedule->id, 'review');
        $this->assertSame($deps['status_ids']['DRAFT'], (int) MarksheetSchedule::find($schedule->id)->status_id, 'DRAFT→review must be rejected.');
        $this->assertFalse(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'REVIEW')->exists());
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_27_illegal_lock_from_reviewed_rejected(): void
    {
        ...
        $this->assertFalse(ComputationLog::where('schedule_id', $schedule->id)->where('action', 'LOCK')->exists());
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_28_compute_blocked_when_locked(): void
    {
        ...
        $before = $this->computeDispatchedCount((int) $schedule->id);
        $this->postLifecycle($schedule->id, 'compute');
        $this->assertSame($before, $this->computeDispatchedCount((int) $schedule->id), 'Locked schedule compute must NOT dispatch.');
        $this->forceDeleteSchedule($schedule);
    }

    public function test_scheduling_29_br_msh_026_reviewed_can_be_recomputed_defect(): void
    {
    ...
    public function test_scheduling_31_duplicate_code_same_session_blocked_diff_session_allowed(): void
    {
        ...
        $this->assertTrue($threw, 'Same (session, code) must be rejected.');

        // Different session (if a second session exists) with the same code is allowed.
    ...
    public function test_scheduling_34_foreign_key_exists_rules_present(): void
    {
        ...
        $this->assertStringContainsString('exists:sys_dropdowns,id', $req);
    ...
    public function test_scheduling_35_unlock_reason_min_length_rejected(): void
    {
        ...
        $fresh = MarksheetSchedule::find($schedule->id);
        $this->assertSame($deps['status_ids']['PUBLISHED'], (int) $fresh->status_id, 'A too-short unlock_reason must be rejected (validation) and leave the schedule PUBLISHED.');
    ...
    {
        $req = File::get(base_path('Modules/MarksheetGeneration/app/Http/Requests/SubjectPracticalConfigRequest.php'));
    ...
    public function test_scheduling_37_schedule_date_is_nullable_date(): void
    {
        ...
        // Persisting a null schedule_date is accepted by the schema.
        $schedule = $this->createScheduleSeed($this->uniqueScheduleCode('DT'), 'DRAFT', ['schedule_date' => null]);
        $this->assertNull(MarksheetSchedule::find($schedule->id)->schedule_date);
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_40_config_template_fk_restrict_on_delete(): void
    {
        ...
        $blocked = false;
        try {
            // Attempt to hard-delete the referenced template row → RESTRICT should block it.
    ...
    public function test_scheduling_41_status_fk_references_sys_dropdowns(): void
    {
        $migration = File::get(base_path(self::MIGRATION_SCHEDULE_FILE));
        $this->assertStringContainsString("foreign('status_id', 'fk_msh_ms_status')->references('id')->on('sys_dropdowns')", $migration);
    }

    public function test_scheduling_42_deleting_schedule_removes_schedule_class_rows(): void
    {
        $deps = $this->scheduleDependencies();
        ...
        // Insert a junction row directly (avoiding the BUG-MSH-101 service path).
        ...
        // fk_msh_scj_schedule is ON DELETE CASCADE — hard-deleting the parent removes junction rows.
        MarksheetSchedule::withTrashed()->where('id', $schedule->id)->forceDelete();
        $this->assertFalse(DB::table('msh_schedule_class_jnt')->where('schedule_id', $schedule->id)->exists(), 'schedule_id FK CASCADE should remove junction rows.');
    }

    public function test_scheduling_43_computation_log_fk_schedule_cascade(): void
    {
        $migration = File::get(base_path(self::MIGRATION_LOG_FILE));
        $this->assertTrue(Schema::hasTable('msh_computation_logs'));
        $this->assertStringContainsString('msh_marksheet_schedules', $migration);
    }
    ...
    public function test_scheduling_44_precheck_cross_module_reads_guarded(): void
    {
        ...
        try {
            ...
            $this->forceDeleteSchedule($schedule);
            $this->markTestSkipped('DEP-MSH-001: precheck cross-module dependency unavailable — ' . $e->getMessage());
        }
        $this->forceDeleteSchedule($schedule);
    }
    ...
    public function test_scheduling_52_sec_msh_003_formrequests_authorize_true(): void
    {
        ...
        $this->assertStringContainsString('return true;', File::get(base_path('Modules/MarksheetGeneration/app/Http/Requests/ScheduleClassRequest.php')));
    }
    ...
    public function test_scheduling_54_policy_methods_map_to_permissions(): void
    {
        $policy = File::get(base_path(self::POLICY_FILE));
        foreach (['review', 'publish', 'unlock', 'lock', 'export'] as $ability) {
            $this->assertStringContainsString("tenant.msh-marksheet-schedule.{$ability}", $policy, "Policy must expose ability {$ability}.");
        }
        // D39-MSH is documented as an environment prerequisite (permissions unseeded) in the Validation Report.
    ...
    {
        // Edge: unlock() does not validate the current status — it always sets COMPUTED + clears is_locked.
        $deps = $this->scheduleDependencies();
    ...
    public function test_scheduling_71_br_msh_027_concurrent_compute_not_guarded_defect(): void
    {
        ...
        // Simulate an in-flight computation.
    ...
    {
        // Compute action never calls validateExamWeightageSum / validateScholasticWeightageSum.
        $controller = File::get(base_path(self::SCHEDULE_CONTROLLER_FILE));
        ...
        // precheck() only surfaces a count, not a sum-to-100 check.
        $this->assertStringContainsString("'weightages'      => (\$template?->examWeightages->count()", $controller, 'precheck shows weightage COUNT only.');
    }
    ...
    {
        // The store rule requires a non-empty string code; a whitespace payload must not create a persisted row.
    ...
    public function test_scheduling_90_cross_tenant_direct_id_is_isolated(): void
    {
        ...
        // A schedule id from the current tenant must not resolve on the second tenant host.
    ...
    public function test_scheduling_91_stored_xss_in_unlock_reason_is_escaped(): void
    {
        ...
        $fresh = MarksheetSchedule::find($schedule->id);
        // Reason is stored verbatim (raw) ...
        $this->assertSame($reason, (string) $fresh->unlock_reason);
        // ... but must render escaped on the show page.
        try {
    ...
    /**
     * Issue an authenticated lifecycle POST from an admin browser session and return nothing —
     * callers assert DB/activity side effects. Reuses one browser context per call.
     */
...
JS);
    ...
    private function scheduleDependencies(): array
    {
        ...
        if ($configTemplateId === 0 || $academicSessionId === 0 || $missing !== []) {
            $this->markTestSkipped('Scheduling tests require a config template, an academic session, and DRAFT/COMPUTED/REVIEWED/PUBLISHED/LOCKED dropdown rows for key ' . self::STATUS_KEY . '.');
        }
