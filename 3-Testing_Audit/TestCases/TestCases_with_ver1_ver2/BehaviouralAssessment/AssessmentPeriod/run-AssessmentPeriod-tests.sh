#!/bin/bash
# AssessmentPeriod (BehaviouralAssessment) Dusk runner.
# Assumes this file sits at prime_testing/tests/Browser/Modules/BehaviouralAssessment/AssessmentPeriod/.
...
if $V1; then FC="bha_AssessmentPeriodV1_TestCas"
elif $V2; then FC="bha_AssessmentPeriodV2_TestCas"
else FC="bha_AssessmentPeriod"
fi
FF="${FC}${F:+::${F}}"

echo "=== AssessmentPeriod Dusk Tests (filter: $FF) ==="
...
if grep -qE "Tests:[[:space:]]+[0-9]+" "$L"; then
  ...
  echo "============================================"
  echo "  AssessmentPeriod RESULTS: $LINE"
  echo "============================================"
fi
