<#
    Runner: GlobalMaster / Dropdown Dusk suite (Prime-central).
    ...
    Prereqs (see Validation Report §7):
      - GlobalMaster AND Prime modules ENABLED in prime_testing/modules_statuses.json (else 404 / route absent)
      - Central app served on http://127.0.0.1:8000 with APP_ENV=testing
      - Test file copied to: tests/Browser/Modules/GlobalMaster/Dropdown/sys_Dropdown_TestCas.php

    Usage:
      ./run-Dropdown-tests.ps1 [-Php C:\php\php.exe] [-Filter test_dropdown_30] [-SyncDb]
#>
...
$ProofFile = Join-Path $ProofDir "dropdown_$Ts.log"

Write-Host "== GlobalMaster Dropdown Dusk run =="
Write-Host "Runner root : $RunnerRoot"
Write-Host "PHP         : $Php"
Write-Host "Filter      : $Filter"
Write-Host "Proof       : $ProofFile"

$ShotDir = Join-Path $RunnerRoot "tests/Browser/Modules/GlobalMaster/Dropdown/screenshots"
