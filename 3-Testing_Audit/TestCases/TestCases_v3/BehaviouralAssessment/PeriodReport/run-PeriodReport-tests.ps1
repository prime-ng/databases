<#
    Runner — BehaviouralAssessment / PeriodReport Dusk suite (single comprehensive file).
    Class name = bha_PeriodReport_TestCas (no V1/V2 split).

    Usage:
        ./run-PeriodReport-tests.ps1 [-Php <php-bin>] [-Filter <pattern>] [-SyncDb]
...
param(
    [string]$Php = "php",
    [string]$Filter = "bha_PeriodReport_TestCas",
    [switch]$SyncDb
)
...
try {
    ...
    if ($SummaryLine) {
        ...
        Write-Host ""
        Write-Host "============================================"
        Write-Host "  RESULTS (PeriodReport):"
