...
/**
 * Behavioural Assessment — Category Performance dashboard (Reports Hub → "Category & Criteria Performance").
 *
 ...
 *                      Screen requirement 23 (Category-Performance) AND screen requirement 17 (Category-Summary)
 *                      BOTH map to this single categories() implementation (DOC-BA-002, proven in _73). The one
 *                      shared view is literally titled "Category Performance" (screen-23 wording) — screen 17's
 *                      "Category Summary" has no distinct page.
 ...
 *                      and the immediate CategorySummary sibling (same categories() implementation).
 *
 * Defects proven:
 ...
 *                  SQLSTATE[42S22] "Unknown column 'score'" → the Category Performance page HARD-500s every time it
 *                  is opened. Proven deterministically at the DB layer in _11, at the route in _12, and by source
 *                  scan in _13. Contrast: student() reads numeric_score (_15); byClass() computes std_dev in memory (_78).
 *   - BUG-BA-011 : reports/export is a permanent abort(501) stub on a live, authorized route — the requirement's
 *                  PDF export (annual developmental journal) is therefore unavailable for this screen (_70 / _72).
 ...
 * Requirement-vs-implementation gaps (screen 23 — the advanced statistical dashboard):
 *   - DOC-BA-002 : screen 23 (Category-Performance) and screen 17 (Category-Summary) share one implementation (_73).
 *   - RPT-GAP-11 : requirement Class + Section filters are NOT implemented — only a period_id filter exists (_71).
 *   - RPT-GAP-12 : requirement grid columns + PDF/CSV export are NOT implemented as specified (_72).
 *   - RPT-GAP-21 : the Score Dispersion Curve / Standard-Deviation bell curve (screen-23 core widget) is NOT
 *                  implemented — categories() returns only AVG/MIN/MAX, never a std deviation (_74).
 *   - RPT-GAP-22 : the Demographic Score Split (gender-wise, boarding-vs-day scholar) is NOT implemented (_75).
 *   - RPT-GAP-23 : the Academic Correlation Matrix (behaviour category avg vs academic GPA) is NOT implemented (_76).
 *   - RPT-GAP-24 : the Standardization Threshold warning (SD > 1.20 → "High Grading Dispersal Detected") is NOT
 *                  implemented — no dispersion is computed on this route to threshold against (_77).
 */
class bha_CategoryPerformance_TestCas extends DuskTestCase
{
    ...
    private const SCREENSHOT_DIR    = 'tests/Browser/Modules/BehaviouralAssessment/CategoryPerformance/screenshots';
    ...
    public function test_category_performance_01_computed_scores_schema_and_model_are_correct(): void
    {
    ...
    public function test_category_performance_02_runtime_table_prefix_diverges_from_ddl_doc_ba_001(): void
    {
    ...
    public function test_category_performance_03_report_controller_method_and_route_are_registered(): void
    {
        foreach (['index', 'categories', 'student', 'byClass', 'export'] as $method) {
            $this->assertTrue(
        ...
        // The Category Performance route is a static GET with NO route-model param (filtered via ?period_id=).
    ...
    public function test_category_performance_04_categories_view_titled_category_performance(): void
    {
        ...
        if ($view !== null) {
            // Screen-23 wording IS the shared view's title (breadcrumb + section heading).
            $this->assertStringContainsString('Category Performance', $view, 'The shared report view is titled "Category Performance".');
            $this->assertStringContainsString('School-Wide Category', $view, 'categories view renders the school-wide category grid.');
        }
        if ($index !== null) {
            $this->assertStringContainsString("route('behavioural-assessment.reports.categories')", $index, 'Reports Hub must link to the Category Performance report.');
        }
    }
    ...
    public function test_category_performance_10_reports_hub_renders_for_authorized_admin(): void
    {
        $this->browseWithFailureScreenshot('hub-render', function (Browser $browser): void {
            ...
            $this->assertStringContainsString('Category & Criteria Performance', $source, 'Reports Hub must advertise the Category Performance report card.');
        });
    }

    public function test_category_performance_11_category_aggregate_raw_sql_uses_nonexistent_score_column_bug_ba_013(): void
    {
    ...
    public function test_category_performance_12_category_performance_page_hard_500s_due_to_bug_ba_013(): void
    {
        // Route manifestation: an authorized admin opening the Category Performance page gets a server error (500)
        ...
        if (in_array($status, [0, 404], true)) {
            $this->markTestSkipped('Category Performance route not reachable (module disabled / 404) — cannot exercise the route-level 500.');
        }
        if ($status === 302) {
            $this->markTestSkipped('Category Performance route redirected (auth/login) — route-level 500 not exercised.');
        }

        $this->assertSame(
            500,
            $status,
            'BUG-BA-013: the Category Performance page must currently 500 (Unknown column `score`). '
                . 'When the controller is fixed to aggregate numeric_score, update this expectation to 200.'
        );
    }

    public function test_category_performance_13_categories_controller_aggregates_on_score_not_numeric_score_bug_ba_013(): void
    {
    ...
    public function test_category_performance_14_seeded_score_has_numeric_score_but_no_score_attribute(): void
    {
        ...
        $this->assertNull($seed->score, 'BUG-BA-013: the model exposes no `score` attribute — the dashboard would read null.');
        $this->deleteComputedScore($seed);
    }

    public function test_category_performance_15_student_report_correctly_reads_numeric_score_contrast(): void
    {
    ...
    public function test_category_performance_16_criterion_performance_reads_rating_levels_numeric_value(): void
    {
    ...
    public function test_category_performance_17_dashboard_is_anonymized_no_student_identity_columns(): void
    {
        // Screen-23 "Anonymity Constraints": the dashboard must contain NO student-level identities — only cohort
        // aggregates. Confirm via source (the page itself cannot render — BUG-BA-013).
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        if ($view === null) {
            $this->markTestSkipped('categories view not readable to confirm anonymization.');
        }
        $this->assertStringNotContainsString('student->name', $view, 'Anonymized dashboard must not print student names.');
        $this->assertStringNotContainsString('roll_no', $view, 'Anonymized dashboard must not print roll numbers.');
        $this->assertStringNotContainsString('admission_no', $view, 'Anonymized dashboard must not print admission numbers.');
    ...
    public function test_category_performance_30_period_filter_does_not_change_the_bug_ba_013_outcome(): void
    {
    ...
    public function test_category_performance_31_unknown_period_filter_still_reaches_the_bug(): void
    {
    ...
    public function test_category_performance_32_garbage_query_params_do_not_introduce_a_new_error(): void
    {
        $response = $this->getFromAdminPage(self::REPORT_CATEGORIES . '?period_id=not-a-number&category_id=%3Cb%3E&foo=1');
        $this->assertContains(
            (int) ($response['status'] ?? 0),
            [500, 404, 302, 0],
    ...
    public function test_category_performance_40_computed_scores_fks_restrict_on_delete(): void
    {
        if (DB::connection()->getDriverName() !== 'mysql') {
    ...
    public function test_category_performance_41_categories_report_dependency_tables_exist(): void
    {
        ...
        foreach (['ba_computed_scores', 'ba_categories', 'ba_criteria', 'ba_rating_levels', 'ba_class_category_jnt'] as $table) {
            $this->assertTrue(Schema::hasTable($table), "Dependency table {$table} must exist for the Category Performance report.");
        }
    ...
    public function test_category_performance_50_guest_is_redirected_to_login(): void
    {
    ...
    public function test_category_performance_51_limited_user_gets_403_on_category_performance(): void
    {
    ...
    public function test_category_performance_52_policy_maps_to_permission_strings(): void
    {
    ...
    public function test_category_performance_53_export_gate_diverges_from_policy_val_ba_003(): void
    {
    ...
    public function test_category_performance_60_categories_view_declares_an_empty_state(): void
    {
    ...
    public function test_category_performance_61_categories_view_exposes_only_a_period_filter(): void
    {
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        if ($view === null) {
    ...
    public function test_category_performance_62_reports_hub_card_links_to_category_report(): void
    {
        $this->browseWithFailureScreenshot('hub-link', function (Browser $browser): void {
            ...
            $source = $browser->driver->getPageSource();
            $this->assertStringContainsString('reports/categories', $source, 'Reports Hub must link to the Category Performance report.');
        });
    }

    // =====================================================================
    // Band 70–79 — Edge cases / requirement-vs-implementation gaps (BC-EDG)
    //   Screen 23 is the "advanced statistical" dashboard — most of its
    //   promised widgets are entirely unbuilt (RPT-GAP-21..24).
    // =====================================================================

    public function test_category_performance_70_export_is_live_abort_501_stub_bug_ba_011(): void
    {
        // BUG-BA-011: an authorized user hitting the live export route gets HTTP 501 (unimplemented stub). Screen-23
        // promises a "PDF export for the annual developmental journal" — it is unavailable.
    ...
    public function test_category_performance_71_requirement_class_and_section_filters_not_implemented_rpt_gap_11(): void
    {
        // Screen-23 workflow selects Category + Period + Class (e.g. "Grade 8"). The implementation has ONLY a
        // period_id filter — no Class or Section filter — RPT-GAP-11.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        if ($view === null) {
    ...
    public function test_category_performance_72_requirement_columns_and_pdf_export_not_implemented_rpt_gap_12(): void
    {
        // Screen-23 promises a "Calculate Statistics" action and a PDF export of the correlation insights. Neither a
        // statistics-recompute control nor a streamed PDF/CSV writer exists — RPT-GAP-12.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        ...
        if ($view !== null) {
            $this->assertStringNotContainsString('Calculate Statistics', $view, 'RPT-GAP-12 changed: a Calculate-Statistics control now present.');
        }
        if ($controller !== null) {
            $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
            $this->assertStringNotContainsString('StreamedResponse', $categoriesBody, 'RPT-GAP-12: no streamed PDF/CSV export implemented on this route.');
            $this->assertStringNotContainsString('fputcsv', $categoriesBody, 'RPT-GAP-12: no CSV writer implemented.');
        }
    }

    public function test_category_performance_73_screen_23_and_17_share_one_implementation_doc_ba_002(): void
    {
        // DOC-BA-002: requirement screen 23 (Category-Performance) and screen 17 (Category-Summary) both resolve to
        // the single categories() route/view — there is no separate advanced Category-Performance implementation.
        $this->assertTrue(Route::has('behavioural-assessment.reports.categories'), 'The shared categories route must exist.');
        $this->assertFalse(Route::has('behavioural-assessment.reports.category-performance'), 'DOC-BA-002: no distinct category-performance route exists.');
        $this->assertFalse(Route::has('behavioural-assessment.reports.category-summary'), 'DOC-BA-002: no distinct category-summary route exists.');
        $this->assertFalse(method_exists(BaReportController::class, 'categoryPerformance'), 'DOC-BA-002: no dedicated categoryPerformance() controller method exists.');

        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        if ($view === null) {
            $this->markTestSkipped('categories view not readable to confirm DOC-BA-002.');
        }
        // The one shared view carries screen 23's "Category Performance" label — proving screen 23 is the canonical
        // title and screen 17's "Category Summary" collapses into it.
        $this->assertStringContainsString('Category Performance', $view, 'The single shared view is labelled Category Performance (screen-23 wording).');
    }

    public function test_category_performance_74_standard_deviation_dispersion_curve_not_implemented_rpt_gap_21(): void
    {
        // Screen-23 core widget: the Score Dispersion Curve / Standard-Deviation bell curve. categories() returns
        // only AVG/MIN/MAX — it never computes a standard deviation, and the view has no dispersion chart. RPT-GAP-21.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        if ($view === null && $controller === null) {
            $this->markTestSkipped('Sources not readable to confirm RPT-GAP-21.');
        }
        if ($controller !== null) {
            $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
            $this->assertStringNotContainsString('std_dev', $categoriesBody, 'RPT-GAP-21 changed: categories() now computes a std deviation.');
            $this->assertStringNotContainsString('STDDEV', strtoupper($categoriesBody), 'RPT-GAP-21 changed: categories() now aggregates STDDEV.');
        }
        if ($view !== null) {
            $this->assertStringNotContainsStringIgnoringCase('Standard Deviation', $view, 'RPT-GAP-21 changed: a Standard-Deviation widget now present.');
            $this->assertStringNotContainsStringIgnoringCase('Dispersion', $view, 'RPT-GAP-21 changed: a Dispersion curve now present.');
            $this->assertStringNotContainsStringIgnoringCase('Bell Curve', $view, 'RPT-GAP-21 changed: a Bell-Curve chart now present.');
        }
    }

    public function test_category_performance_75_demographic_gender_split_not_implemented_rpt_gap_22(): void
    {
        // Screen-23 Demographic Score Split (gender-wise + boarding vs day scholar). No gender/enrolment split is
        // computed or rendered — categories() groups only by category_id. RPT-GAP-22.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        if ($view === null && $controller === null) {
            $this->markTestSkipped('Sources not readable to confirm RPT-GAP-22.');
        }
        if ($controller !== null) {
            $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
            $this->assertStringNotContainsStringIgnoringCase('gender', $categoriesBody, 'RPT-GAP-22 changed: categories() now splits by gender.');
            $this->assertStringNotContainsStringIgnoringCase('boarding', $categoriesBody, 'RPT-GAP-22 changed: categories() now splits boarding vs day scholar.');
        }
        if ($view !== null) {
            $this->assertStringNotContainsStringIgnoringCase('gender', $view, 'RPT-GAP-22 changed: a gender split now rendered.');
        }
    }

    public function test_category_performance_76_academic_correlation_matrix_not_implemented_rpt_gap_23(): void
    {
        // Screen-23 Academic Correlation Matrix (behaviour category avg vs academic GPA scatter). No correlation
        // computation or academic-score join exists in categories(). RPT-GAP-23.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        if ($view === null && $controller === null) {
            $this->markTestSkipped('Sources not readable to confirm RPT-GAP-23.');
        }
        if ($controller !== null) {
            $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
            $this->assertStringNotContainsStringIgnoringCase('correlation', $categoriesBody, 'RPT-GAP-23 changed: categories() now computes a correlation index.');
            $this->assertStringNotContainsStringIgnoringCase('gpa', $categoriesBody, 'RPT-GAP-23 changed: categories() now joins academic GPA.');
        }
        if ($view !== null) {
            $this->assertStringNotContainsStringIgnoringCase('correlation', $view, 'RPT-GAP-23 changed: a correlation matrix now rendered.');
        }
    }

    public function test_category_performance_77_standardization_threshold_warning_not_implemented_rpt_gap_24(): void
    {
        // Screen-23 business rule: SD > 1.20 on a 5-point scale → "High Grading Dispersal Detected" warning icon.
        // Since no dispersion is computed on this route (RPT-GAP-21), the threshold warning is also absent. RPT-GAP-24.
        $view = $this->readAppFile($this->moduleRootPath('resources/views/reports/categories.blade.php'));
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        if ($view === null && $controller === null) {
            $this->markTestSkipped('Sources not readable to confirm RPT-GAP-24.');
        }
        if ($controller !== null) {
            $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
            $this->assertStringNotContainsString('1.20', $categoriesBody, 'RPT-GAP-24 changed: a 1.20 dispersal threshold now checked.');
        }
        if ($view !== null) {
            $this->assertStringNotContainsStringIgnoringCase('High Grading Dispersal', $view, 'RPT-GAP-24 changed: the dispersal warning banner now present.');
            $this->assertStringNotContainsStringIgnoringCase('Polarized Grading', $view, 'RPT-GAP-24 changed: a polarized-grading alert now present.');
        }
    }

    public function test_category_performance_78_std_dev_exists_in_byclass_not_in_categories_contrast(): void
    {
        // Contrast / partial capability: the std-deviation statistic the requirement wants IS computed — but only in
        // byClass() (the Class Analysis report), in memory via sqrt(...). categories() (this screen's route) never
        // computes it. So the building block exists, it is simply not wired into Category Performance.
        $controller = $this->readAppFile($this->moduleRootPath('app/Http/Controllers/BaReportController.php'));
        if ($controller === null) {
            $this->markTestSkipped('BaReportController source not readable for the std_dev contrast.');
        }
        $byClassBody = $this->extractMethodBody($controller, 'function byClass', 'function incidents');
        $categoriesBody = $this->extractMethodBody($controller, 'function categories', 'function export');
        $this->assertStringContainsString('std_dev', $byClassBody, 'byClass() computes a std_dev — the statistic screen-23 wants.');
        $this->assertStringContainsString('sqrt(', $byClassBody, 'byClass() computes std_dev in memory via sqrt().');
        $this->assertStringNotContainsString('std_dev', $categoriesBody, 'categories() (Category Performance route) does NOT compute std_dev — RPT-GAP-21.');
    }
    ...
    public function test_category_performance_90_tenant_context_is_initialized(): void
    {
        $this->assertTrue(
    ...
    public function test_category_performance_91_api_resource_lacks_tenancy_and_is_dead_dead_ba_001(): void
    {
    ...
    public function test_category_performance_92_web_report_routes_carry_full_tenancy_stack(): void
    {
        $rsp = $this->readAppFile($this->moduleRootPath('app/Providers/RouteServiceProvider.php'));
        if ($rsp === null) {
    ...
    public function test_category_performance_93_categories_view_escapes_output(): void
    {
    ...
    private function captureScreenshot(Browser $browser, string $kind, string $caseName): void
    {
        ...
        $rawName = 'category-performance-' . $kind . '-' . $caseName . '-' . now()->format('Ymd_His');
        $safeName = preg_replace('/[^A-Za-z0-9_-]+/', '-', $rawName);
        $safeName = is_string($safeName) && $safeName !== '' ? $safeName : 'category-performance-' . $kind . '-' . now()->format('Ymd_His');
        try {
    ...
    private function sendJsonRequestFromBrowser(
        ...
        $browser->script(<<<JS
window.__cpApiDone = false;
window.__cpApiError = '';
window.__cpApiResult = null;
...
JS);
    ...
    private function makeLimitedUser(): User
    {
        try {
            ...
            $attributes = [
                'name'              => 'CP Limited ' . $this->uniqueSuffix(),
                'email'             => 'cp_limited_' . strtolower($this->uniqueSuffix()) . '@tenant.test',
                'password'          => 'password',
                'is_active'         => 1,
            ...
            if (Schema::hasColumn('sys_users', 'emp_code')) {
                $attributes['emp_code'] = 'CP' . substr($this->uniqueSuffix(), -8);
            }
