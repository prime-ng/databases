...
#
# Usage:
#   ./run-Incident-tests.sh [--php <php-bin>] [--filter <method>] [--sync-db]
#
# Examples:
#   ./run-Incident-tests.sh
#   ./run-Incident-tests.sh --filter test_incident_92
#   ./run-Incident-tests.sh --php /usr/bin/php --sync-db
#
set -uo pipefail

# --- Config -----------------------------------------------------------------
APP_REPO="${APP_REPO:-/Users/bkwork/Herd/prime_testing}"
TEST_CLASS="bha_Incident_TestCas"
PHP_BIN="php"
FILTER=""
SYNC_DB=0
...
PROOF_FILE="$PROOF_DIR/Incident_${STAMP}.log"

SHOT_DIR="$APP_REPO/tests/Browser/Modules/BehaviouralAssessment/Incident/screenshots"
if [[ -d "$SHOT_DIR" ]]; then
