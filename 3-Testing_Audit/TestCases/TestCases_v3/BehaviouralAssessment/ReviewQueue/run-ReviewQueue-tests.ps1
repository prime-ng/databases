...
# Class name = bha_ReviewQueue_TestCas. Default filter targets that class.
# Prereq: BehaviouralAssessment enabled in modules_statuses.json; APP_ENV=testing (set below).
# Endpoint state-machine tests markTestSkipped when cross-module seed rows are absent.
...
if ($Filter) {
    ...
    $duskFilter = "bha_ReviewQueue_TestCas"
}
...
try {
    ...
    if ($totalCount -ne 0) {
        ...
        Write-Host "  RESULTS (ReviewQueue):" -ForegroundColor Cyan
