#!/usr/bin/env bash
#
# Runner — BehaviouralAssessment / Review Queue Dusk suite (single comprehensive file).
# Class name = bha_ReviewQueue_TestCas (no V1/V2 split).
#
# Usage:
#   ./run-ReviewQueue-tests.sh [--php <php-bin>] [--filter <pattern>] [--sync-db]
#
# Prerequisites:
...
#   - Endpoint state-machine tests need cross-module seed rows (ba_assessment_periods, sch_employees,
#     sch_class_section_jnt); they markTestSkipped when those are absent (partial env stays green).

set -uo pipefail

PHP_BIN="php"
FILTER="bha_ReviewQueue_TestCas"
SYNC_DB=0
