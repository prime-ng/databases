<#
    Runner — BehaviouralAssessment / IncidentReport Dusk suite (single comprehensive file).
    Class name = bha_IncidentReport_TestCas (no V1/V2 split).

    Usage:
        ./run-IncidentReport-tests.ps1 [-Php <php-bin>] [-Filter <pattern>] [-SyncDb]
...
param(
    [string]$Php = "php",
    [string]$Filter = "bha_IncidentReport_TestCas",
    [switch]$SyncDb
)
...
try {
    ...
    if ($SummaryLine) {
        ...
        Write-Host ""
        Write-Host "============================================"
        Write-Host "  RESULTS (IncidentReport):"
