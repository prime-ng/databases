#!/usr/bin/env bash
#
# run-StudentRemark-tests.sh — BehaviouralAssessment / StudentRemark Dusk suite runner (bash / Linux / WSL / macOS)
#
# Runs the single comprehensive test file bha_StudentRemark_TestCas.php (41 methods).
#
# Usage:
#   ./run-StudentRemark-tests.sh [--php <php-bin>] [--filter <method-or-pattern>] [--sync-db]
#
# Options:
#   --php <path>       PHP binary to use (default: php)
#   --filter <pattern> Dusk --filter value (default: whole class bha_StudentRemark_TestCas)
...
PHP_BIN="php"
FILTER="bha_StudentRemark_TestCas"
SYNC_DB=0
...
PROOF_DIR="proof"
mkdir -p "$PROOF_DIR"
PROOF_FILE="${PROOF_DIR}/StudentRemark_dusk_${TIMESTAMP}.log"

echo "======================================================================"
echo " BehaviouralAssessment / StudentRemark — Dusk suite"
echo " PHP     : ${PHP_BIN}"
echo " Filter  : ${FILTER}"
...
SHOT_DIR="tests/Browser/Modules/BehaviouralAssessment/StudentRemark/screenshots"
if [[ -d "$SHOT_DIR" ]]; then
