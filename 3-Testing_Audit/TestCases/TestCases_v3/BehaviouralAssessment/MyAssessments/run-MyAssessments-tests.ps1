<#
.SYNOPSIS
    run-MyAssessments-tests.ps1 — BehaviouralAssessment / MyAssessments Dusk suite runner (Windows PowerShell).
...
.PARAMETER Filter
    Dusk --filter value (default: the whole class bha_MyAssessments_TestCas).
...
.EXAMPLE
    ./run-MyAssessments-tests.ps1
    ./run-MyAssessments-tests.ps1 -Filter test_my_assessments_20_submit_transitions_draft_to_submitted_and_logs
    ./run-MyAssessments-tests.ps1 -SyncDb
#>

param(
    [string]$Php = "php",
    [string]$Filter = "bha_MyAssessments_TestCas",
    [switch]$SyncDb
)
...
$proofFile = Join-Path $proofDir "MyAssessments_dusk_$timestamp.log"

Write-Host "======================================================================"
Write-Host " BehaviouralAssessment / MyAssessments - Dusk suite"
Write-Host " PHP     : $Php"
...
$shotDir = "tests/Browser/Modules/BehaviouralAssessment/MyAssessments/screenshots"
