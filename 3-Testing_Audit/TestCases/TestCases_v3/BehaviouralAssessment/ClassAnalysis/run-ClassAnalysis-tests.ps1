<#
    Runner — BehaviouralAssessment / ClassAnalysis Dusk suite (single comprehensive file).
    Class name = bha_ClassAnalysis_TestCas (no V1/V2 split).

    Usage:
        ./run-ClassAnalysis-tests.ps1 [-Php <php-bin>] [-Filter <pattern>] [-SyncDb]
...
param(
    [string]$Php = "php",
    [string]$Filter = "bha_ClassAnalysis_TestCas",
    [switch]$SyncDb
)
...
try {
    ...
    if ($SummaryLine) {
        ...
        Write-Host ""
        Write-Host "============================================"
        Write-Host "  RESULTS (ClassAnalysis):"
