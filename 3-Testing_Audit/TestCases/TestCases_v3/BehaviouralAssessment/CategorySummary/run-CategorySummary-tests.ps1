<#
    Runner — BehaviouralAssessment / CategorySummary Dusk suite (single comprehensive file).
    Class name = bha_CategorySummary_TestCas (no V1/V2 split).

    Usage:
      ./run-CategorySummary-tests.ps1 [-Php <php-bin>] [-Filter <pattern>] [-SyncDb]
    ...
    NOTE: test _12 expects the Category Summary page to currently return HTTP 500 (BUG-BA-013:
...
param(
    [string]$Php = "php",
    [string]$Filter = "bha_CategorySummary_TestCas",
    [switch]$SyncDb
)
...
try {
    ...
    if ($summary) {
        ...
        Write-Host ""
        Write-Host "============================================"
        Write-Host "  RESULTS (CategorySummary):"
