<#
.SYNOPSIS
    run-Intervention-tests.ps1 — BehaviouralAssessment / Intervention Dusk suite runner (Windows PowerShell).

.DESCRIPTION
    Runs the single comprehensive test file bha_Intervention_TestCas.php (48 methods):
...
.PARAMETER Filter
    Dusk --filter value (default: the whole class bha_Intervention_TestCas).
...
.EXAMPLE
    ./run-Intervention-tests.ps1
    ./run-Intervention-tests.ps1 -Filter test_intervention_10_create_persists_and_redirects_with_success_flash
    ./run-Intervention-tests.ps1 -SyncDb
#>

param(
    [string]$Php = "php",
    [string]$Filter = "bha_Intervention_TestCas",
    [switch]$SyncDb
)
...
$proofFile = Join-Path $proofDir "Intervention_dusk_$timestamp.log"

Write-Host "======================================================================"
Write-Host " BehaviouralAssessment / Intervention - Dusk suite"
Write-Host " PHP     : $Php"
...
$shotDir = "tests/Browser/Modules/BehaviouralAssessment/Intervention/screenshots"
