#!/usr/bin/env bash
#
# run-Intervention-tests.sh — BehaviouralAssessment / Intervention Dusk suite runner (bash / Linux / WSL / macOS)
#
# Runs the single comprehensive test file bha_Intervention_TestCas.php (48 methods).
#
# Usage:
#   ./run-Intervention-tests.sh [--php <php-bin>] [--filter <method-or-pattern>] [--sync-db]
#
# Options:
#   --php <path>       PHP binary to use (default: php)
#   --filter <pattern> Dusk --filter value (default: whole class bha_Intervention_TestCas)
...
PHP_BIN="php"
FILTER="bha_Intervention_TestCas"
SYNC_DB=0
...
PROOF_DIR="proof"
mkdir -p "$PROOF_DIR"
PROOF_FILE="${PROOF_DIR}/Intervention_dusk_${TIMESTAMP}.log"

echo "======================================================================"
echo " BehaviouralAssessment / Intervention — Dusk suite"
echo " PHP     : ${PHP_BIN}"
echo " Filter  : ${FILTER}"
...
SHOT_DIR="tests/Browser/Modules/BehaviouralAssessment/Intervention/screenshots"
if [[ -d "$SHOT_DIR" ]]; then
