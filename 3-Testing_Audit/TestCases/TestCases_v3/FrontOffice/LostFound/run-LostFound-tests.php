...
/**
 * Cross-platform Dusk runner for FrontOffice → LostFound.
 * Runs natively on Windows and Linux/macOS (pure PHP, no shell dialect).
 *
 * Usage:
 *   php run-LostFound-tests.php [--php=/path/to/php] [--filter=test_name] [--sync-db]
 *
 * Prerequisites (see fof_LostFoundValidation_Report.md):
 *   - Copy fof_LostFound_TestCas.php into prime_testing/tests/Browser/ (namespace Tests\Browser).
...
$filter    = $options['filter'] ?? 'fof_LostFound_TestCas';
...
$proofFile = $proofDir . DIRECTORY_SEPARATOR . "LostFound_{$timestamp}.log";
...
echo "\n=====================================================\n";
echo "LostFound Dusk run complete.\n";
