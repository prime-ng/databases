...
/**
 * Cross-platform Dusk runner for the FrontOffice :: Communication suite.
 * Runs natively on Windows and Linux/macOS (no .ps1 / .sh needed).
 *
 * Usage:
 *   php run-Communication-tests.php [--php=/path/to/php] [--filter=test_communication_10] [--sync-db]
 *
 * Prerequisites (see fof_CommunicationValidation_Report.md):
 *   - Copy fof_Communication_TestCas.php into prime_testing/tests/Browser/ before running.
 *   - FrontOffice must be ENABLED in prime_testing/modules_statuses.json.
...
$filter    = $options['filter'] ?? 'fof_Communication_TestCas';
...
$proofFile = $proofDir . DIRECTORY_SEPARATOR . "Communication_dusk_{$timestamp}.log";
...
echo "==================================================================\n";
echo " FrontOffice :: Communication — Dusk run\n";
echo "  php     : {$phpBinary}\n";
echo "  repo    : {$testRepo}\n";
echo "  filter  : {$filter}\n";
