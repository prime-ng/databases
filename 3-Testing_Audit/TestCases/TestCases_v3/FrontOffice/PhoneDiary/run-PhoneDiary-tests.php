...
/**
 * Cross-platform Dusk runner for the FrontOffice :: PhoneDiary suite.
 * Runs natively on Windows and Linux/macOS (no .ps1 / .sh needed).
 *
 * Usage:
 *   php run-PhoneDiary-tests.php [--php=/path/to/php] [--filter=test_phonediary_20] [--sync-db]
 *
 * Prerequisites (see fof_PhoneDiaryValidation_Report.md):
 *   - Copy fof_PhoneDiary_TestCas.php into prime_testing/tests/Browser/ before running.
...
$filter    = $options['filter'] ?? 'fof_PhoneDiary_TestCas';
...
$proofFile = $proofDir . DIRECTORY_SEPARATOR . "PhoneDiary_dusk_{$timestamp}.log";
...
echo "==================================================================\n";
echo " FrontOffice :: PhoneDiary — Dusk run\n";
echo "  php     : {$phpBinary}\n";
echo "  repo    : {$testRepo}\n";
echo "  filter  : {$filter}\n";
