...
use Modules\MarksheetGeneration\Http\Requests\ClassGroupRequest;
use Modules\MarksheetGeneration\Http\Requests\ConfigTemplateRequest;
use Modules\MarksheetGeneration\Http\Requests\ExamGroupRequest;
use Modules\MarksheetGeneration\Http\Requests\IaComponentTypeRequest;
use Modules\MarksheetGeneration\Http\Requests\MarksheetTypeRequest;
use Modules\MarksheetGeneration\Models\ClassGroup;
...
/**
 * MarksheetGeneration — Configuration Templates (single comprehensive Dusk suite).
 *
 * ONE test file per screen (no V1/V2 split). Merges the former foundation + comprehensive
 * suites, re-verified against the real source on 2026-07-10.
 *
 * Primary table  : msh_config_templates  (tenant_db, prefix msh_ — verified vs DDL CREATE TABLE)
 * Screen         : Configuration composite tab
 *                  route('marksheet-generation.configuration.combined', ['tab' => 'config-templates'])
 *                  → GET /marksheet-generation/configuration
 * Create page    : GET  /marksheet-generation/config-template/create   (full page, NOT a modal)
 * Store/Update   : POST/PUT /marksheet-generation/config-template[/{id}]  (REDIRECT — no expectsJson branch)
 ...
 *                  combined page gate tenant.msh-configuration.view
 *
 * Semantic numbering bands (WP-G):
 *   01-09 schema/config · 10-19 business rules · 20-29 state (is_locked / toggle) · 30-39 validation
 *   40-49 integration/FK · 50-59 permissions · 60-69 UI/UX · 70-79 edge/security · 90-99 tenancy
 *
 * Proven / documented defects:
 *   BUG-MSH-003 (P2) — ExamGroupController::edit() has no model binding → redirects, never 404 (test_56).
 *   SEC-MSH-003 (P1) — all 5 FormRequests here authorize()=true; gate enforced only in controller (test_53, test_55).
 *   D39-MSH     (P1) — tenant.msh-* permissions unseeded → super-admin-only (documented; permissions granted explicitly).
 *   DEV-MSH-CT-01 (candidate) — BR-MSG-027 is_locked immutability guard not implemented (test_21, verify-in-source).
 *
 * Constraints obeyed (05_Known_Test_Failure_Constraints.md): tenant-side initializeTenantContext() via
 * Modules\Prime\Models\Domain + guarded tearDown; App\Models\User + factory; Dusk has no assertStatus()
 * (status via sendJsonRequestFromBrowser); MySQL8 COLUMN_TYPE variance → assertStringContainsString;
 * forceDelete wrapped in try/catch; emp_code ≤20 via '_'.uniqid(); typed props init to null.
 * ENV prereq: MarksheetGeneration enabled in modules_statuses.json + APP_ENV=testing.
 */
class msh_ConfigurationTemplates_TestCas extends DuskTestCase
{
    ...
    private const MT_TOGGLE_PATH  = '/marksheet-generation/marksheet-type';
    ...
    private const CONFIG_PERMISSIONS = [
        ...
        // master entities on the same composite screen
        'tenant.msh-marksheet-type.viewAny',
        'tenant.msh-marksheet-type.view',
        'tenant.msh-marksheet-type.create',
        'tenant.msh-marksheet-type.update',
        'tenant.msh-marksheet-type.delete',
        'tenant.msh-class-group.viewAny',
        'tenant.msh-class-group.view',
        'tenant.msh-class-group.create',
        'tenant.msh-class-group.update',
        'tenant.msh-class-group.delete',
        'tenant.msh-exam-group.viewAny',
        'tenant.msh-exam-group.view',
        'tenant.msh-exam-group.create',
        'tenant.msh-exam-group.update',
        'tenant.msh-exam-group.delete',
        'tenant.msh-ia-component-type.view',
        'tenant.msh-ia-component-type.create',
        'tenant.msh-ia-component-type.update',
        'tenant.msh-ia-component-type.delete',
    ];
    ...
    public function test_config_template_01_schema_model_and_fillable_are_correct(): void
    {
        ...
        $this->assertTrue(
            Schema::hasColumns('msh_config_templates', [
                'id', 'academic_session_id', 'marksheet_type_id', 'exam_group_id', 'grading_schema_id',
                'code', 'name', 'description', 'board_code', 'passing_percentage',
                'compartment_max_failures', 'is_best_of_n_enabled', 'best_of_n_count',
                'is_locked', 'is_active', 'created_by', 'updated_by',
    ...
    public function test_config_template_02_column_types_and_indexes_match_ddl(): void
    {
        ...
        // MySQL 8 COLUMN_TYPE variance — contains, never equals (constraint D17).
    ...
    public function test_config_template_03_request_rules_and_controller_events_are_correct(): void
    {
        ...
        $this->assertTrue(File::exists($requestPath), 'Request file missing: ' . self::REQUEST_FILE);
        ...
        $this->assertStringContainsString("'description' => ['nullable', 'string', 'max:500']", $rc);
        $this->assertStringContainsString("'board_code' => ['nullable', 'string', 'max:50']", $rc);
        ...
        $this->assertStringContainsString("'best_of_n_count' => ['nullable', 'integer', 'min:1', 'max:255']", $rc);
        $this->assertStringContainsString("Rule::unique('msh_config_templates', 'code')", $rc);
        $this->assertStringContainsString('prepareForValidation', $rc);
        // SEC-MSH-003: request self-authorizes true
        $this->assertStringContainsString('public function authorize(): bool', $rc);
        $this->assertStringContainsString('return true;', $rc);

        $controllerPath = base_path(self::CONTROLLER_FILE);
        $this->assertTrue(File::exists($controllerPath), 'Controller file missing: ' . self::CONTROLLER_FILE);
    ...
    public function test_config_template_04_master_tables_and_models_are_correct(): void
    {
        // Composite screen: the 4 configuration masters live on the same page.
        $masters = [
            ['msh_marksheet_types',   MarksheetType::class],
            ['msh_class_groups',      ClassGroup::class],
            ['msh_exam_groups',       ExamGroup::class],
            ['msh_ia_component_types', IaComponentType::class],
        ];

        foreach ($masters as [$table, $modelClass]) {
            $this->assertTrue(Schema::hasTable($table), "Master table {$table} does not exist.");
            $this->assertTrue(Schema::hasColumns($table, ['id', 'code', 'name', 'is_active', 'created_by', 'deleted_at']), "Master table {$table} missing standard columns.");

            /** @var \Illuminate\Database\Eloquent\Model $model */
            $model = new $modelClass();
            $this->assertSame($table, $model->getTable(), "{$modelClass}::getTable() should be {$table}.");
            $this->assertContains(SoftDeletes::class, class_uses_recursive($modelClass), "{$modelClass} should use SoftDeletes.");
            foreach (['code', 'name', 'is_active', 'created_by'] as $col) {
                $this->assertContains($col, $model->getFillable(), "{$modelClass} should have {$col} fillable.");
            }
        }

        // Per-session unique scope for exam groups mirrors config templates.
        $this->assertInstanceOf(BelongsTo::class, (new ExamGroup())->academicSession());
        $this->assertInstanceOf(HasMany::class, (new MarksheetType())->configTemplates());
    }

    public function test_config_template_05_ddl_file_defines_config_template_constraints(): void
    {
        $ddlPath = base_path(self::DDL_FILE);
        if (!File::exists($ddlPath)) {
            // DDL lives in the requirements repo, not the app repo; count and skip when not co-located.
            $this->addToAssertionCount(1);
            $this->markTestSkipped('DDL file not co-located with the app repo (' . self::DDL_FILE . ').');
        }
        ...
        $this->assertStringContainsString('REFERENCES `msh_marksheet_types` (`id`) ON DELETE RESTRICT', $ddl);
        $this->assertStringContainsString('REFERENCES `msh_exam_groups` (`id`) ON DELETE RESTRICT', $ddl);
    ...
    public function test_config_template_10_create_persists_with_activity_stored(): void
    {
        ...
        $this->browseWithFailureScreenshot('ct-10-create', function (Browser $browser) use ($deps, $code, $name): void {
            ...
            $this->assertContains((int) ($response['status'] ?? 0), [200, 302], 'Create did not complete (store redirects; fetch follows to 200).');
        });
    ...
    public function test_config_template_21_br_msg_027_update_not_blocked_when_locked_verify_in_source(): void
    {
        ...
        // guard, so the update SUCCEEDS on a locked record. This test documents the CURRENT behaviour
        // and flags the gap (candidate DEV-MSH-CT-01 — verify in source before asserting a bug).
        $deps = $this->configDependencies();
        ...
        // this assertion is the canary that flips and forces the doc to be updated.
    ...
    public function test_config_template_22_toggle_status_endpoint_updates_is_active(): void
    {
        $deps = $this->configDependencies();
        $template = $this->createConfigTemplateSeed($deps, ['is_active' => true]);

        $this->browseWithFailureScreenshot('ct-22-toggle', function (Browser $browser) use ($template): void {
            $this->authenticate($browser);
            $this->visitAuthenticated($browser, self::COMBINED_PATH . '?tab=config-templates', 900);

            $response = $this->sendJsonRequestFromBrowser($browser, 'POST', self::STORE_PATH . '/' . $template->id . '/toggleStatus', []);
            $this->assertSame(200, (int) ($response['status'] ?? 0), 'Toggle did not return HTTP 200 JSON.');
    ...
    public function test_config_template_25_marksheet_type_master_create_and_toggle(): void
    {
        // Composite screen: prove a configuration master (marksheet-type) toggles via its own endpoint
        // with the same activity idiom (Toggled) as config-template.
        $type = MarksheetType::create([
            'code'          => $this->uniqueCode('MTG'),
            'name'          => $this->uniqueName('MTGGL'),
            'display_order' => 1,
            'is_active'     => true,
            'created_by'    => (int) $this->adminUser->id,
        ]);

        $this->browseWithFailureScreenshot('ct-25-mt-toggle', function (Browser $browser) use ($type): void {
            $this->authenticate($browser);
            $this->visitAuthenticated($browser, self::COMBINED_PATH . '?tab=marksheet-types', 900);

            $response = $this->sendJsonRequestFromBrowser($browser, 'POST', self::MT_TOGGLE_PATH . '/' . $type->id . '/toggleStatus', []);
            $this->assertSame(200, (int) ($response['status'] ?? 0), 'Marksheet-type toggle did not return HTTP 200 JSON.');
            $json = is_array($response['json'] ?? null) ? $response['json'] : [];
            $this->assertTrue((bool) ($json['success'] ?? false), 'Marksheet-type toggle success was not true.');
        });

        $type->refresh();
        $this->assertFalse((bool) $type->is_active, 'Marksheet type was not toggled to inactive.');
        $this->assertActivityIssuedByAdmin((int) $type->id, 'Toggled', MarksheetType::class);
    ...
    public function test_config_template_44_marksheet_type_delete_restricted_while_referenced(): void
    {
        ...
        $this->assertTrue($blocked, 'FK RESTRICT: deleting a referenced marksheet type must be blocked (fk_msh_ct_type).');
        $this->forceDeleteConfigTemplate($template);
    }
    ...
    public function test_config_template_51_user_without_create_permission_gets_403(): void
    {
        ...
        if ($status === 200 || $status === 302) {
            $this->markTestSkipped('Limited user still authorized (super-admin bypass / D39-MSH broad seed) — gate 403 not observable here.');
        }
    ...
    public function test_config_template_53_sec_msh_003_config_template_request_self_authorizes_true(): void
    {
        // SEC-MSH-003: the FormRequest never gates — authorize() returns true unconditionally.
        $request = new ConfigTemplateRequest();
        $this->assertTrue($request->authorize(), 'ConfigTemplateRequest::authorize() must return true (gate enforced in controller only).');
    ...
    public function test_config_template_55_sec_msh_003_all_master_form_requests_self_authorize_true(): void
    {
        // SEC-MSH-003 (systemic): every FormRequest on this composite screen returns authorize()=true.
        foreach ([
            new ConfigTemplateRequest(),
            new MarksheetTypeRequest(),
            new ClassGroupRequest(),
            new ExamGroupRequest(),
            new IaComponentTypeRequest(),
        ] as $request) {
            $this->assertTrue(
                $request->authorize(),
                get_class($request) . '::authorize() must return true — SEC-MSH-003 (no gate in the request layer).'
            );
        }
    }
    ...
    public function test_config_template_56_bug_msh_003_exam_group_edit_redirects_without_binding(): void
    {
        $this->browseWithFailureScreenshot('ct-56-bug-msh-003', function (Browser $browser): void {
            $this->authenticate($browser);
            // ExamGroupController::edit() takes NO ExamGroup param → no implicit route-model binding →
            // no 404 for a bogus id, and it redirects (302) to the combined page. fetch follows to 200.
    ...
    public function test_config_template_63_create_page_shows_breadcrumb_and_form_fields(): void
    {
        $this->configDependencies();

        $this->browseWithFailureScreenshot('ct-63-create-page', function (Browser $browser): void {
            $this->authenticate($browser);
...
JS);
    ...
    private function assertActivityIssuedByAdmin(int $subjectId, string $event, string $subjectType = ConfigTemplate::class): void
    {
        ...
        $this->assertNotNull($log, "Activity log '{$event}' for {$subjectType} #{$subjectId} was not recorded.");
