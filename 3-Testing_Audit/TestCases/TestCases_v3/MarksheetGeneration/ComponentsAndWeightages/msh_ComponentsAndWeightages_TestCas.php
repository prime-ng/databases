...
/**
 * MarksheetGeneration — Components & Weightages (single comprehensive suite).
 *
 * Screen: Components & Weightages — combined tabbed page hosting four child
 *   entities of a ConfigTemplate (screen file: MarksheetGeneration_V2/
 *   03-Components-and-Weightages.md).
 *
 *   Combined page : GET /marksheet-generation/components
 *                   (route marksheet-generation.components.combined,
 *                    MarksheetGenerationController::components(),
 *                    tabs: sc_page / ew_page / ia_page / cc_page).
 *   Entities (all tenant-side, prefix msh_):
 *     - template-scholastic-component  (PRIMARY, msh_template_scholastic_components,
 *                                        uq_msh_tsc_template_component)
 *     - template-exam-weightage        (msh_template_exam_weightages,
 *                                        uq_msh_tew_template_exam)
 *     - template-ia-component          (msh_template_ia_components,
 *                                        uq_msh_tiac_template_type)
 *     - template-coscholastic-component(msh_template_coscholastic_components,
 *                                        uq_msh_tcsc_template_code)
 *
 * Activity-log events (verbatim from controllers): Stored / Updated / Deleted /
 *   Toggled / Restored. Permissions: tenant.msh-{entity}.{ability}.
 *
 * OWNED audit defects proven here (config side of the weightage-sum finding):
 *   BR-MSH-050 / BR-MSH-009 / BR-MSH-012 — the sum of weightage_percent across
 *     scholastic components (and across exam-type weightages) MUST equal 100,
 *     but it is NOT validated at create/compute time. store() creates directly
 *     and never calls MarksheetConfigService::validateScholasticWeightageSum();
 *     the exam validator validateExamWeightageSum() has NO caller (dead code).
 *     A template with sum 40+40=80 is currently accepted — data-integrity gap.
 *   SEC-MSH-003 — all four FormRequests authorize() return true (no request-layer
 *     gating; authorization lives only in the controller Gate::authorize calls).
 *   D39-MSH — component permissions are unseeded (documented env prerequisite;
 *     the suite seeds/grants them defensively in resolveAdminUser()).
 *
 * Discovered (cross-reference scan, verify-in-source, proven current behaviour):
 *   DEV-MSH-C03 — a sum-breaking scholastic UPDATE (routed through the service)
 *     surfaces as HTTP 500 (uncaught DomainException), not a 422; rolls back.
 *   DEV-MSH-C04 — coscholastic grading_scale has no enum (`in:`) rule, so an
 *     out-of-spec value is accepted and persisted.
 *
 * Style: browser Dusk (golden Class reference). Obeys 05_Known_Test_Failure_Constraints.md
 * (tenant init via Domain, App\Models\User factory, SoftDeletes guarded,
 * forceDelete wrapped, typed props initialised, no Dusk assertStatus).
 */
class msh_ComponentsAndWeightages_TestCas extends DuskTestCase
{
    ...
    private const COMBINED_CONTROLLER_FILE   = 'Modules/MarksheetGeneration/app/Http/Controllers/MarksheetGenerationController.php';
    ...
    public function test_components_01_scholastic_migration_model_and_request_configuration_are_correct(): void
    {
        $this->assertTrue(
        ...
        $this->assertTrue(
            Schema::hasColumns('msh_template_scholastic_components', [
                ...
                'max_marks', 'is_active', 'created_by', 'updated_by',
        ...
        if (DB::connection()->getDriverName() === 'mysql') {
            $this->assertNotEmpty(
                ...
                'Composite unique index uq_msh_tsc_template_component missing.'
            );
        }
    ...
    public function test_components_02_exam_weightage_schema_model_and_unique_index(): void
    {
        ...
        $this->assertTrue(Schema::hasColumns('msh_template_exam_weightages', [
            'id', 'config_template_id', 'exam_type_id', 'weightage_percent', 'max_marks',
    ...
    public function test_components_03_ia_component_schema_model_and_unique_index(): void
    {
        $this->assertTrue(Schema::hasTable('msh_template_ia_components'));
        $this->assertTrue(Schema::hasColumns('msh_template_ia_components', [
            'id', 'config_template_id', 'ia_component_type_id', 'max_marks', 'display_order',
    ...
    public function test_components_04_coscholastic_schema_model_and_unique_index(): void
    {
        ...
        $this->assertTrue(Schema::hasColumns('msh_template_coscholastic_components', [
            'id', 'config_template_id', 'name', 'code', 'grading_scale', 'is_ba_linked',
    ...
    public function test_components_05_model_casts_are_correct(): void
    {
        ...
        $this->assertSame('decimal:2', (new TemplateScholasticComponent())->getCasts()['max_marks'] ?? null);
        $this->assertSame('bool', (new TemplateScholasticComponent())->getCasts()['is_active'] ?? null);
        $this->assertSame('decimal:2', (new TemplateExamWeightage())->getCasts()['weightage_percent'] ?? null);
        ...
        $this->assertSame('decimal:2', (new TemplateIaComponent())->getCasts()['max_marks'] ?? null);
        $this->assertSame('bool', (new TemplateCoscholasticComponent())->getCasts()['is_ba_linked'] ?? null);
        $this->assertSame('integer', (new TemplateCoscholasticComponent())->getCasts()['display_order'] ?? null);
    }

    public function test_components_06_request_rules_config_truth_and_controller_gates(): void
    {
        ...
        $this->assertStringContainsString("'regex:/^\\d+(\\.\\d{1,2})?$/'", $sch);
        ...
        $this->assertStringContainsString('msh_template_ia_components', $ia);
        ...
        $this->assertStringContainsString("'max:30'", $cosc);
        // DEV-MSH-C04: grading_scale is only string|max:50 (no `in:` enum rule).
        $this->assertStringContainsString("'grading_scale' => ['sometimes', 'string', 'max:50']", $cosc);

        // Controller gate strings (authorization is enforced only in controllers).
        $controller = File::get(base_path(self::SCHOLASTIC_CONTROLLER_FILE));
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-scholastic-component.create')", $controller);
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-scholastic-component.update')", $controller);
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-scholastic-component.delete')", $controller);
    }

    // ════════════════════════════════════════════════════════════════════════
    // Band 10-19 — business rules (BC-BIZ): create + activity + weightage sum
    // ════════════════════════════════════════════════════════════════════════
    ...
    public function test_components_10_create_scholastic_persists_and_logs_stored(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $sourceId, &$created): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Scholastic store did not return HTTP 200.');
    ...
    public function test_components_11_create_exam_weightage_persists_and_logs_stored(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $examTypeId, &$created): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Exam weightage store did not return HTTP 200.');
    ...
    public function test_components_12_create_ia_component_persists_and_logs_stored(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $iaTypeId, &$created): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'IA component store did not return HTTP 200.');
    ...
    public function test_components_13_create_coscholastic_persists_grading_and_logs_stored(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $code, &$created): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Co-scholastic store did not return HTTP 200.');
    ...
    public function test_components_16_scholastic_weightage_sum_not_validated_on_create_defect(): void
    {
        // OWNED: BR-MSH-050 / BR-MSH-009 / BR-MSH-012.
        // store() creates the row directly and NEVER calls
        // MarksheetConfigService::validateScholasticWeightageSum(), so two
        // components summing to 80 (!= 100) are both accepted — the config-side
        // data-integrity gap. (Sum enforcement only runs on update(), via service.)
        ...
        $this->run(function (Browser $browser) use ($templateId, $sourceA, $sourceB): void {
            ...
            $this->assertSame(200, (int) ($r2['status'] ?? 0), 'BR-MSH-050: create must not validate the 100% sum (defect proof).');
        }, 'c16');
    ...
    public function test_components_17_exam_weightage_sum_never_enforced_defect(): void
    {
        // OWNED: BR-MSH-009 / BR-MSH-012 (exam side). Exam store() and the exam
        // service update() never call validateExamWeightageSum(); a non-100 exam
        // weightage sum is accepted on create.
        $templateId = $this->createConfigTemplateSeed()->id;
        $examTypeId = $this->resolveExamTypeId();
        ...
        $this->run(function (Browser $browser) use ($templateId, $examTypeId, $second): void {
            ...
            if ($second !== null) {
                ...
                $this->assertSame(200, (int) ($r2['status'] ?? 0), 'BR-MSH-009: exam weightage sum must not be validated (defect proof).');
            }
        }, 'c17', 'exam-weightages');
    ...
    public function test_components_18_exam_weightage_sum_validator_is_dead_code_defect(): void
    {
        // Static proof of BR-MSH-009/012 (exam side): the validator exists on
        // MarksheetConfigService but is wired into neither the exam service nor
        // the exam controller — it is dead code.
        $config = File::get(base_path(self::CONFIG_SERVICE_FILE));
        $this->assertStringContainsString('function validateExamWeightageSum', $config, 'Expected the exam-sum validator to be defined.');
    ...
    public function test_components_19_scholastic_update_keeping_sum_logs_updated(): void
    {
        $templateId = $this->createConfigTemplateSeed()->id;
        $sourceId = $this->createSourceComponentSeed()->id;
        ...
        $this->run(function (Browser $browser) use ($row): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Valid update (sum stays 100) should return HTTP 200.');
        }, 'c19');
    ...
    public function test_components_30_scholastic_required_fields_rejected(): void
    {
        $this->run(function (Browser $browser): void {
            $r = $this->sendJsonRequestFromBrowser($browser, 'POST', self::SCHOLASTIC_BASE, []);
            $this->assertSame(422, (int) ($r['status'] ?? 0), 'Empty scholastic store should return HTTP 422.');
    ...
    public function test_components_31_scholastic_weightage_over_100_rejected(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $sourceId): void {
            ...
            $this->assertSame(422, (int) ($r['status'] ?? 0), 'weightage_percent 150 should be rejected (max:100).');
    ...
    public function test_components_35_scholastic_duplicate_component_rejected_with_message(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $sourceId): void {
            ...
            $this->assertSame(422, (int) ($r['status'] ?? 0), 'Duplicate (template, source) should return HTTP 422.');
            $this->assertTrue(
    ...
    public function test_components_37_ia_component_required_and_display_order_min(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId, $iaTypeId): void {
            ...
            // display_order: prepareForValidation coerces falsy to 1, so send explicit -1.
    ...
    public function test_components_40_scholastic_invalid_config_template_rejected(): void
    {
        ...
        $this->run(function (Browser $browser) use ($sourceId): void {
            ...
            $this->assertSame(422, (int) ($r['status'] ?? 0), 'Invalid config_template_id should return HTTP 422 (exists rule).');
    ...
    public function test_components_41_scholastic_invalid_source_component_rejected(): void
    {
        ...
        $this->run(function (Browser $browser) use ($templateId): void {
            ...
            $this->assertSame(422, (int) ($r['status'] ?? 0), 'Invalid source_component_id should return HTTP 422 (exists rule).');
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Band 50-59 — permissions / authorization (BC-AUTH) + SEC-MSH-003
    // ════════════════════════════════════════════════════════════════════════
    ...
    {
        // SEC-MSH-003: every FormRequest authorize() returns true — no request-layer
        // authorization. (Gating is only in the controllers; see test 06/52.)
        foreach ([
    ...
    public function test_components_52_controller_enforces_gates_across_entities(): void
    {
        $scholastic = File::get(base_path(self::SCHOLASTIC_CONTROLLER_FILE));
        $exam = File::get(base_path(self::EXAM_CONTROLLER_FILE));
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-scholastic-component.create')", $scholastic);
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-scholastic-component.viewAny')", $scholastic);
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-exam-weightage.create')", $exam);
        $this->assertStringContainsString("Gate::authorize('tenant.msh-template-exam-weightage.delete')", $exam);
    }

    // ════════════════════════════════════════════════════════════════════════
    // Band 60-69 — UI / UX (render, listing, search, pagination)
    // ════════════════════════════════════════════════════════════════════════
    ...
    {
        // The combined components() paginates each tab with a distinct page name.
        $controller = File::get(base_path(self::COMBINED_CONTROLLER_FILE));
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Band 70-79 — edge cases / security (BC-EDG + TC-S)
    // ════════════════════════════════════════════════════════════════════════
    ...
    {
        // DEV-MSH-C04 (verify-in-source): grading_scale has no `in:` enum rule
        // (only string|max:50), so an out-of-spec value is accepted and persisted,
        ...
        $this->run(function (Browser $browser) use ($templateId, $code): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'DEV-MSH-C04: arbitrary grading_scale should currently be accepted.');
    ...
    public function test_components_76_scholastic_show_valid_id_returns_200(): void
    {
        $templateId = $this->createConfigTemplateSeed()->id;
        $sourceId = $this->createSourceComponentSeed()->id;
        $row = $this->createScholasticSeed($templateId, $sourceId, ['weightage_percent' => 100]);
        ...
        $this->run(function (Browser $browser) use ($row): void {
            $r = $this->sendJsonRequestFromBrowser($browser, 'GET', self::SCHOLASTIC_BASE . '/' . $row->id, []);
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Show endpoint for a valid id should return HTTP 200.');
        }, 'c76');
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Band 80-89 — configuration rule (weightage sum) discovered behaviour
    // ════════════════════════════════════════════════════════════════════════

    public function test_components_80_scholastic_update_breaking_sum_returns_500_and_rolls_back(): void
    {
        // DEV-MSH-C03 (verify-in-source): the update path routes through
        // TemplateScholasticComponentService, which DOES enforce the 100% sum —
        // but the resulting DomainException is uncaught, so a sum-breaking update
        // surfaces as HTTP 500 (not a 422 validation error) and rolls back.
        $templateId = $this->createConfigTemplateSeed()->id;
        $sourceId = $this->createSourceComponentSeed()->id;
        $row = $this->createScholasticSeed($templateId, $sourceId, ['weightage_percent' => 100]);
        ...
        $this->run(function (Browser $browser) use ($row): void {
            ...
            $this->assertSame(500, (int) ($r['status'] ?? 0), 'DEV-MSH-C03: sum violation on update should surface as HTTP 500, not 422.');
        }, 'c80');
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Band 90-99 — lifecycle & activity log (BC-BIZ toggle/delete/restore)
    // ════════════════════════════════════════════════════════════════════════
    ...
    public function test_components_90_scholastic_toggle_logs_toggled(): void
    {
        ...
        $this->run(function (Browser $browser) use ($row): void {
            ...
            $this->assertSame(200, (int) ($r['status'] ?? 0), 'Toggle endpoint should return HTTP 200.');
            $json = is_array($r['json'] ?? null) ? $r['json'] : [];
...
JS);
    ...
    {
        // D39-MSH: component permissions are unseeded in the tenant DB. Seed and
        // grant them defensively so authenticated flows are not blocked by a 403.
