# Run the Prime User Dusk suite (central / 127.0.0.1:8000).
# Usage: .\run-User-tests.ps1 [-Php php] [-Filter test_name] [-SyncDb]
param(
    [string]$Php = "php",
    [string]$Filter = "sys_User_TestCas",
    [switch]$SyncDb
)
...
$ProofDir = "tests\Browser\Modules\Prime\User\proof"
New-Item -ItemType Directory -Force -Path $ProofDir | Out-Null
$ProofFile = Join-Path $ProofDir "user_$Ts.log"

# Clean stale screenshots.
Get-ChildItem -Path "tests\Browser\Modules\Prime\User" -Recurse -Directory -Filter "screenshots" -ErrorAction SilentlyContinue |
