...
# Usage: ./run-User-tests.sh [--php /path/to/php] [--filter test_name] [--sync-db]
set -uo pipefail

PHP_BIN="php"
FILTER="sys_User_TestCas"
SYNC_DB=0
...
PROOF_DIR="tests/Browser/Modules/Prime/User/proof"
mkdir -p "$PROOF_DIR"
PROOF_FILE="$PROOF_DIR/user_${TS}.log"

# Clean stale screenshots.
find tests/Browser/Modules/Prime/User -type d -name screenshots -exec rm -f {}/*.png \; 2>/dev/null || true
