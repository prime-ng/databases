#!/usr/bin/env bash
# =============================================================================
# Language (Prime / PRM — CENTRAL) Dusk test runner
...
PHP_BIN="${PHP_BIN:-php}"
FILTER="${1:-}"
TEST_CLASS="glb_Language_TestCas"
TEST_PATH="tests/Browser/Modules/Prime/Language/${TEST_CLASS}.php"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROOF_DIR="${SCRIPT_DIR}/proof"
mkdir -p "${PROOF_DIR}"
STAMP="$(date +%Y%m%d_%H%M%S)"
PROOF_FILE="${PROOF_DIR}/language_dusk_${STAMP}.log"

# Clean stale screenshots for this feature (best-effort).
SHOT_DIR="tests/Browser/Modules/Prime/Language/screenshots"
[ -d "${SHOT_DIR}" ] && rm -f "${SHOT_DIR}"/*.png 2>/dev/null

echo "==============================================================="
echo " Language (PRM central) — Dusk suite"
echo " File   : ${TEST_PATH}"
