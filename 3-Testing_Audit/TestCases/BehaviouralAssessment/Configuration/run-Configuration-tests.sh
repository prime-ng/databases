#!/bin/bash
# Configuration (BehaviouralAssessment) Dusk runner.
# Assumes this file sits at prime_testing/tests/Browser/Modules/BehaviouralAssessment/Configuration/.
...
if $V1; then FC="bha_ConfigurationV1_TestCas"
elif $V2; then FC="bha_ConfigurationV2_TestCas"
else FC="bha_Configuration"
fi
FF="${FC}${F:+::${F}}"

echo "=== Configuration Dusk Tests (filter: $FF) ==="
...
if grep -qE "Tests:[[:space:]]+[0-9]+" "$L"; then
  ...
  echo "============================================"
  echo "  Configuration RESULTS: $LINE"
  echo "============================================"
fi
