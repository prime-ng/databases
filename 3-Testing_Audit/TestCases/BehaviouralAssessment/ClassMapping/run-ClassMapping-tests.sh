#!/bin/bash
# ClassMapping (BehaviouralAssessment) Dusk runner.
# Assumes this file sits at prime_testing/tests/Browser/Modules/BehaviouralAssessment/ClassMapping/.
...
if $V1; then FC="bha_ClassMappingV1_TestCas"
elif $V2; then FC="bha_ClassMappingV2_TestCas"
else FC="bha_ClassMapping"
fi
FF="${FC}${F:+::${F}}"

echo "=== ClassMapping Dusk Tests (filter: $FF) ==="
...
if grep -qE "Tests:[[:space:]]+[0-9]+" "$L"; then
  ...
  echo "============================================"
  echo "  ClassMapping RESULTS: $LINE"
  echo "============================================"
fi
