...
## 1. Generate one feature's full 7-artifact suite  (`feature` mode)

> **Use the testcase-creator agent — mode=feature, module=Accounting, feature=TaxRate.**
> Generate the complete 7-artifact test suite. Read the real source, detect the module's test style, verify the prefix against the DDL, and write to the TestCases folder.
...
> Discover every feature, confirm the Feature Inventory, then generate the 7 artifacts for each (parents → children → junctions → reports).
...
> Generate the suite, then run the Dusk test via the runner, capture the proof file, and report pass/fail with any failures classified as flake / real defect (DEV-###) / test bug.
...
- **Only the Dusk/feature test:**
  > Use testcase-creator to regenerate **only the Dusk test file** for HrStaff / LeaveType and refresh the Gap Analysis + Validation Report to match.
...
> Use the testcase-creator agent to **add the Tenancy (`TC-T`) and Security (`TC-S`) test packs** to the existing Accounting / TaxRate suite — cross-tenant isolation, IDOR, mass-assignment, CSRF, XSS on every free-text field — then update the Gap Analysis and Validation Report.
...
> Use the testcase-creator agent to **run the Validation Report checklist and `php -l`** on the existing HrStaff / LeaveType artifacts — file existence, naming, structure, coverage gates (Negative 100%, Positive ≥90%, Dependency ≥90%, Tenancy 100% P0/P1), TC↔method traceability — and report PASS / PASS-WITH-NOTES.
...
- **Module coverage dashboard:**
  > **Use the testcase-creator agent — mode=report, module=HrStaff.** Generate `_HrStaff_Coverage_Dashboard.md` (coverage % by category, verdict, last run, open DEV-###).
