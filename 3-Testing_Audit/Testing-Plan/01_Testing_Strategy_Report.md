...
The proven pattern already exists: the **`Class&SubjectMgmt/Classes`** feature (golden reference) and the **23 `HrStaff` features** demonstrate the 7-artifact model achieving **~96% traceable coverage** (78 automated methods against 81 manual test cases for a single feature). This report formalises that pattern into a repeatable strategy and extends it with additional test dimensions and reporting.
...
- Unit-level testing of pure helpers (covered opportunistically inside the config tests, not a separate suite).
...
```
          ┌─────────────────────────────┐
          │  E2E Browser (Dusk)         │  ← PRIMARY investment: full UX + integration + cross-module
          ├─────────────────────────────┤
          │  Integration (Dusk)         │  ← schema/model/request config + endpoint + core CRUD
          ├─────────────────────────────┤
          │  Contract/API assertions    │  ← JSON endpoints, status codes, payload shape
          ├─────────────────────────────┤
          │  Static/config assertions   │  ← migration content, fillable, soft-deletes, indexes (test_01)
          └─────────────────────────────┘
```

Because the app is UI-heavy (modal + AJAX CRUD), **the browser layer is where behaviour actually lives**, so it is where we invest. A single comprehensive Dusk file per screen carries both the fast foundational guard (schema truth + core flows) and the exhaustive behavioural suite.

---

## 4. Coverage Model — a Single Comprehensive Test File

Each screen ships **one** comprehensive Dusk test file (schema/config truth + full positive/negative/dependency/state-machine/permissions/tenancy/security/edge matrix). Internally it groups into two content blocks:

| Block | Role | Typical size | Content |
|------|------|-------------|---------|
| **Foundation** | Config truth + core flows | 15–20 methods | `test_01` = migration + model + FormRequest config truth; then core create/edit/delete/toggle/view, key validations, JSON endpoints, breadcrumbs, activity-log issued-by checks |
| **Comprehensive** | Full behavioural matrix | commonly 70+ | Every `TC-P##`, `TC-N##`, `TC-D##`; full negative matrix; all FK integrity paths; cross-module auto-updates; lifecycle; concurrency/race; button-visibility per permission; guest redirect; XSS/whitespace |

**Why one file with both blocks:** the foundation block gives a cheap, fast confidence gate suitable for pre-commit / CI-lite (reached via a category filter); the comprehensive block is the release gate. The Gap Analysis document reconciles the file against the manual test list and reports a coverage percentage per category. Coverage is gated by category — Negative = 100%, Positive ≥ 90%, Dependency ≥ 90%, Tenancy = 100% on P0/P1 — not by any method-count ratio between blocks.
...
Where a feature exposes JSON endpoints (`show`, reorder, toggle, dropdown data), assert **status code + payload shape + required keys** explicitly, so a controller refactor that changes the contract fails a test. (Already partially present in the `show` tests; make it a consistent block.)
...
- A **module-level smoke suite** (foundation-category filter) for fast CI; the **full suite** (whole file) for release.
...
- **Module Coverage Dashboard** — per feature: #methods, coverage %, pass/fail, open DEV-### defects.
...
**Per feature (in the 7 artifacts):** coverage % by category (Gap Analysis), pass/fail (runner proof file), self-validation verdict (Validation Report).
...
- Features with full 7-artifact set / total features.
- Automated methods count; every TC-ID maps to ≥1 test method and every method maps back to a TC/BC.
...
- **`Testcase_Creator` agent** (see `03_`): the primary generator. Input = a module/feature + the knowledge-base paths; output = the 7 artifacts (+ roll-ups in report mode).
...
- **CI hook (recommended):** foundation-category filter on every PR (fast gate); the full file nightly per module; roll-up dashboards published as build artifacts.
...
1. All 7 artifacts exist and pass the Validation Report checklist.
2. `php -l` passes on the PHP file; it extends `DuskTestCase`, namespace `Tests\Browser`.
3. Every TC-ID maps to ≥1 test method and every method maps back to a TC/BC.
