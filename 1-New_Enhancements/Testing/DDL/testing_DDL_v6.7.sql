...
-- Version  : v6.7 (6.6 was enhanced version of 6.5 and then further Refined it to 6.7)
-- MySQL    : 8.0
...
-- User & Machine Master must be created Centrally by one person only. All user will use same user-code and machine-code for their work. 
CREATE TABLE IF NOT EXISTS `tst_users` (
    ...
    `password`      VARCHAR(512) NOT NULL,  -- User Password
    ...
    `is_superuser`  TINYINT(1) NOT NULL DEFAULT 0, -- Super User Flag
    `is_system`     TINYINT(1) NOT NULL DEFAULT 0, -- System User Flag - System Generated Users will be marked as 1 and others as 0
    `is_active`     TINYINT(1) NOT NULL DEFAULT 1, -- Active User Flag - Active Users will be marked as 1 and others as 0
    `created_by`    VARCHAR(10) NULL, -- User who created the record
    `updated_by`    VARCHAR(10) NULL, -- User who updated the record
    `deleted_by`    VARCHAR(10) NULL, -- User who deleted the record
...
VALUES
('S1','Super User','super@prime-testing.local','$2y$12$placeholder_super_hash','System',1,1,1,'S1','S1'),
('S2','System','system@prime-testing.local','$2y$12$placeholder_sys_hash','System',0,1,1,'S1','S1'),
('A1','Brijesh','brijesh@prime-testing.local','$2y$12$placeholder_brij_hash','Architect',0,0,1,'S1','S1'),
('D2','Tarun','tarun@prime-testing.local','$2y$12$placeholder_tarun_hash','Developer',0,0,1,'S1','S1'),
('D3','Shailesh','shailesh@prime-testing.local','$2y$12$placeholder_shail_hash','Developer',0,0,1,'S1','S1'),
('T1','Sameer','sameer@prime-testing.local','$2y$12$placeholder_samer_hash','Tester',0,0,1,'S1','S1'),
('T2','Gaurav','gaurav@prime-testing.local','$2y$12$placeholder_gaurav_hash','Developer',0,0,1,'S1','S1')
ON DUPLICATE KEY UPDATE
    `name` = VALUES(`name`),
    `email` = VALUES(`email`),
    `role` = VALUES(`role`),
...
CREATE TABLE IF NOT EXISTS `tst_machines` (
    ...
    `user_code`         VARCHAR(3) NOT NULL,   -- User Code of the user whom that machine belongs to.
    `machine_code`      VARCHAR(5) NOT NULL,  -- The code format will be User_code+'-'+1 digit number (e.g., A01-1, D02-2,..D99-1 etc.)
...
CREATE TABLE IF NOT EXISTS `tst_test_cases` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `ts_code` VARCHAR(20) NOT NULL,
    `test_case_code` SMALLINT UNSIGNED NOT NULL,  -- Unique Test Case Code (Incremental Number for ts_code)
