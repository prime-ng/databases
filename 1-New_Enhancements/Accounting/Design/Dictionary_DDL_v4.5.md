...
**Document ID:** ACC-DD-V4.5
**Version:** 1.0
**Date:** 2026-09-09
**Describes:** `Accounting_DDL_v4.5.sql` — 75 tables, 22 views
...
### 2. `del_marker` — soft delete that still enforces uniqueness

```sql
`deleted_at`  TIMESTAMP NULL,
`del_marker`  BIGINT UNSIGNED GENERATED ALWAYS AS (IFNULL(UNIX_TIMESTAMP(`deleted_at`),0)) STORED,
UNIQUE KEY (`code`, `del_marker`)
```

**The problem it solves.** An earlier version used `UNIQUE(code, deleted_at)`. That does not work: MySQL treats every `NULL` as distinct, so **any number of live rows could share the same code**. The constraint existed and enforced nothing.

**How this works.** A live row has `del_marker = 0`; a deleted row has its deletion timestamp. So there is exactly **one live row per code**, and deleted rows keep their history.

> Two rows soft-deleted in the same second with the same code would collide. That is the correct outcome, not a defect — you should not be deleting two identically-coded ledgers in the same second.
...
> **`del_marker` and the `*_marker` columns are described here once and generally not repeated in each table below**, the same way the audit columns are. Where a table's unique key depends on one, the key definition in that table's *Keys & rules* names it. They are all `GENERATED … STORED`: **never write to them.**
...
| `code` | VARCHAR(20) | — | **UK** (with `del_marker`). Short identifier | `MAIN` |
...
| `del_marker` | BIGINT UNSIGNED | — | **GEN.** See the pattern above | `0` |
...
| `code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `SUNDRY_DEBTORS` |
| `name` | VARCHAR(120) | — | **UK** (with `del_marker`) | `Sundry Debtors` |
...
| `code` | VARCHAR(30) | ✔ | **UK** (with `del_marker`). Optional short code | `HDFC_CUR` |
| `name` | VARCHAR(150) | — | **UK** (with `del_marker`). What appears in every dropdown | `Bank — HDFC Current` |
...
| **— party links —** | | | | |
| `student_id` | INT UNSIGNED | ✔ | **UK** (with `del_marker`). FK → `std_students`. One ledger per student | `4471` |
...
- Five unique keys, all soft-delete aware: `name`, `code`, `student_id`, `employee_id`, `vendor_id`. **One student cannot have two ledgers.**
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `1` |
| `code` | VARCHAR(20) | — | **UK** (with `del_marker`) | `SECTION` |
| `name` | VARCHAR(100) | — | **UK** (with `del_marker`) | `Academic Section` |
...
| `code` | VARCHAR(20) | — | **UK** (with `del_marker`) | `PRIM` |
| `name` | VARCHAR(120) | — | **UK** (with category and `del_marker`) | `Primary Section` |
...
| `code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `CSR-LIB-2026` |
| `name` | VARCHAR(150) | — | **UK** (with `del_marker`) | `TechCorp CSR — Library` |
...
| `code` | VARCHAR(40) | — | **UK** (with `del_marker`) | `LIBRARY_FINE` |
...
| `code` | VARCHAR(20) | — | **UK** (with `del_marker`) | `RECEIPT` |
| `name` | VARCHAR(80) | — | **UK** (with `del_marker`) | `Receipt` |
...
| **— numbering —** | | | | |
| `prefix` / `suffix` | VARCHAR(10) | ✔ | **UK** on prefix (with `del_marker`) | `RCP` / NULL |
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `1` |
| `code` | VARCHAR(15) | — | **UK** (with `del_marker`) | `CGST` |
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `2` |
| `code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `TDS_194C_CONTRACTOR` |
...
### Keys & rules
`UNIQUE (ledger_id, reference_no, financial_year_id, del_marker)` · `CHECK (original_amount >= 0)` · `CHECK (written_off_amount BETWEEN 0 AND original_amount)`.
...
| `del_marker` | BIGINT UNSIGNED | — | **GEN.** Soft-delete key component | `0` |
| `campus_marker` | SMALLINT UNSIGNED | — | **GEN.** `IFNULL(campus_id, 0)` | `0` |
| `fund_marker` | INT UNSIGNED | — | **GEN.** `IFNULL(fund_id, 0)` | `0` |

### Keys & rules
`UNIQUE (ledger_id, financial_year_id, campus_marker, fund_marker, del_marker)` · `CHECK (amount >= 0)` · `CHECK (exchange_rate > 0)` · `CHECK` — a correction requires a reason.
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `2` |
| `code` | VARCHAR(20) | — | **UK** (with `del_marker`) | `COMP` |
...
| `asset_code` | VARCHAR(50) | — | **UK** (with `del_marker`). The tag number on the asset | `COMP-2026-014` |
...
| `claim_number` | VARCHAR(50) | — | **UK** (with `del_marker`) | `EXP-2026-0088` |
...
| `code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `BUD-2026-27-REV1` |
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `3` |
| `code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `LATE_FEE_18` |
...
| `concession_no` | VARCHAR(50) | — | **UK** (with `del_marker`) | `CON-2026-0142` |
...
| `receipt_no` | VARCHAR(50) | — | **UK** (with `del_marker`). **The 80G receipt number** | `80G/2026-27/0212` |
...
|---|---|:---:|---|---|
| `id` | BIGINT UNSIGNED | — | **PK** | `18` |
| `grant_code` | VARCHAR(30) | — | **UK** (with `del_marker`) | `CSR-TECH-2026` |
...
| `certificate_no` | VARCHAR(50) | — | **UK** (with `del_marker`) | `LDC/2026/8812` |
...
|---|---|:---:|---|---|
| `id` | SMALLINT UNSIGNED | — | **PK** | `4` |
| `code` | VARCHAR(50) | — | **UK** (with `del_marker`) | `AGED_SUSPENSE` |
...
| # | Trap | The rule |
|---|---|---|
| 1 | Writing to `del_marker` or any `*_marker` column | **They are GENERATED.** MySQL maintains them |
