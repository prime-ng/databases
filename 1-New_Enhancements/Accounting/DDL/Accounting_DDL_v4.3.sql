...
CREATE TABLE IF NOT EXISTS `acc_accounting_status_masters` (
	`id`            MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT,
	`status_type`   ENUM('VoucherStatus', 'BankReconciliationStatus', 'ExpenseClaimStatus', 'TallyExportStatus', 'CrossModuleDataStatus') NOT NULL,
...
-- Condition:
	-- Tenant can not Add/Edit/Delete in this Table, he can only View it. This table will be managed(Add/Edit/Del) by Super Admin.
-- Data seed :
	--   Status Type                    				Code
	-- --------------------------------------   ---------------------------------------------------------------
	...
	-- 'Cross-Module Data Status'               - 'Pending','Processed','Failed','Skipped'
...
CREATE TABLE IF NOT EXISTS `acc_cost_centers` (
	...
	`category`      INT UNSIGNED NOT NULL, -- fk to sys_dropdown_table (Cost Center Category) : Department, Activity, Project etc.
	`is_active`     TINYINT(1) NOT NULL DEFAULT 1, -- Soft active flag
	`created_at`    TIMESTAMP NULL DEFAULT NULL,
	`updated_at`    TIMESTAMP NULL DEFAULT NULL,
	`deleted_at`    TIMESTAMP NULL DEFAULT NULL,
	...
	CONSTRAINT `fk_acc_cc_parent` FOREIGN KEY (`parent_id`) REFERENCES `acc_cost_centers` (`id`) ON DELETE SET NUL,
	CONSTRAINT `fk_acc_cc_category` FOREIGN KEY (`category`) REFERENCES `sys_dropdown_table`(`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
-- Conditions:
-- Ceate DropDown Data in `sys_dropdown_table` for the field 'category'
	--  Key 							 Category
	-- ------------------------ -----------------------------------
	-- Cost_Center_Category		 Department
	-- Cost_Center_Category		 Activity
	-- Cost_Center_Category		 Project
...
CREATE TABLE IF NOT EXISTS `acc_recurring_templates` (
	...
	`day_of_month_week` TINYINT NULL, -- Day to post for monthly / Weekly frequency
...
-- Condition:
-- We need to have a button to Post Voucher into Voucher table.
...
--Brij - Need to create Voucher Entry Detail for Assets and Depriciation voucher posting.
...
-- Voucher lines will be created in `acc_voucher`, `acc_voucher_items` & `acc_voucher_item_cost_centers` table as per below Instructions:
...
-- Entry in `acc_voucher`:
	`acc_voucher.id`						= {NEW_VOUCHER_ID} [AUTOGENERATED ID]
	...
	`acc_voucher.source_category_id` = "acc_expense_claims.source_category_id"
	`acc_voucher.source_type`        = "acc_expense_claims.source_type"
	`acc_voucher.source_id`          = "acc_expense_claims.source_id"
	`acc_voucher.status`             = "acc_expense_claims.status"

-- Entry in `acc_voucher_items`: Debit Entries (This may have multipal lines)
	`acc_voucher_items.voucher_id`     = {NEW_VOUCHER_ID}
	`acc_voucher_items.ledger_id`      = "acc_expense_claims_lines.ledger_id"
	...
	`acc_voucher_items.reference_date` = "acc_expense_claims_lines.expense_date"

-- Entry in `acc_voucher_items`: Credit Entry (This will be a single line Entry)
	`acc_voucher_items.voucher_id`     = {NEW_VOUCHER_ID}
	`acc_voucher_items.ledger_id`      = "acc_ledgers.id" -- (Ledger ID of Employee ID = `acc_expense_claims.employee_id`)
	`acc_voucher_items.type`           = "Cr"
	`acc_voucher_items.amount`         = "acc_expense_claims.total_amount"
	`acc_voucher_items.narration`      = "acc_expense_claims.description"
	`acc_voucher_items.bill_reference` = "acc_expense_claims.reference_number"
	`acc_voucher_items.reference_date` = "acc_expense_claims.claim_date"

-- Entry in `acc_voucher_item_cost_centers`:
   `voucher_item_id` = `acc_voucher_items.id`
   `cost_center_id`  = "acc_expense_claims_lines.cost_center_id"
   `amount`          = "acc_expense_claims_lines.total_amount"
   `percentage`      = "NULL"
   `narration`       = "acc_expense_claims_lines.description"
