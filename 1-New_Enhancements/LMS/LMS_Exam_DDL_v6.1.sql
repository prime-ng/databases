...
-- --------------------------------------------------------------------------------------
  ...
  CREATE TABLE IF NOT EXISTS `lms_exam_student_groups` (
    ...
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_esg_code` (`class_id`, `section_id`, `code`),
  ...
  CREATE TABLE IF NOT EXISTS `lms_exam_blueprints` (
    ...
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_blueprint_section` (`exam_paper_id`, `section_name`),
  ...
  CREATE TABLE IF NOT EXISTS `lms_exam_allocations` (
    ...
    `class_section_jnt_id` INT UNSIGNED NULL,  -- FK to sch_class_section_jnt (Class+Section)
    ...
    `conducted_in_school` TINYINT(1) NOT NULL DEFAULT 1,  -- Conducted in School Premises (If this will be 1 then location will be school address otherwise it can be home or any other place)
    `room_id` INT UNSIGNED DEFAULT NULL,        -- FK to sch_rooms.id (If conducted_in_school is 1 then room_id will be mandatory)
    ...
    CONSTRAINT `fk_alloc_classSection` FOREIGN KEY (`class_section_jnt_id`) REFERENCES `sch_class_section_jnt` (`id`),
