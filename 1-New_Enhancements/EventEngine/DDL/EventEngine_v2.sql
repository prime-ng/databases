-- ==================================================================================================
-- EVENT BASED RULE ENGINE (ebe) – PHASE 1 (CORE + AUDIT + MULTI ACTION)
...
-- ------------------------------------------------------------------
-- MASTER DATA (OPTIONAL SEED)
-- ------------------------------------------------------------------

-- Trigger Events
INSERT INTO lms_trigger_event (code, name, description, event_logic, is_active, created_at)
VALUES
('ON_QUIZ_COMPLETION','On Quiz Completion','Triggered when quiz is evaluated','{"event":"quiz_completed"}',1,NOW()),
('ON_HOMEWORK_OVERDUE','On Homework Overdue','Triggered when homework crosses due date','{"event":"homework_overdue"}',1,NOW());

/* Action Types */
INSERT INTO lms_action_type (code, name, description, action_logic, is_active, created_at)
VALUES
('AUTO_ASSIGN_REMEDIAL','Auto Assign Remedial','Assign remedial lesson automatically','{"handler":"assign_remedial"}',1,NOW()),
('NOTIFY_PARENT','Notify Parent','Send notification to parent','{"handler":"notify_parent"}',1,NOW());
