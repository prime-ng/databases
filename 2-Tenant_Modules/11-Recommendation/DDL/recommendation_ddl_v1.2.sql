...
CREATE TABLE IF NOT EXISTS `rec_recommendation_materials` (
  ...
  `material_type` ENUM('TEXT','VIDEO','PDF','AUDIO','QUIZ','ASSIGNMENT','LINK','INTERACTIVE') NOT NULL,
  `purpose` ENUM('REVISION','PRACTICE','REMEDIAL','ADVANCED','ENRICHMENT','CONCEPT_BUILDING') NOT NULL DEFAULT 'PRACTICE',
  `complexity_level` ENUM('EASY','MEDIUM','HARD','CHALLENGING') DEFAULT 'MEDIUM',
  -- Content Source
  `content_source` ENUM('INTERNAL_EDITOR','UPLOADED_FILE','EXTERNAL_LINK','LMS_MODULE','QUESTION_BANK') NOT NULL DEFAULT 'INTERNAL_EDITOR',
  ...
  `media_id` BIGINT UNSIGNED DEFAULT NULL,        -- Link to Central Media Store (sys_media or qns_media_store)
  ...
  CONSTRAINT `fk_recMat_school` FOREIGN KEY (`school_id`) REFERENCES `sch_organizations` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_recMat_class` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_recMat_subject` FOREIGN KEY (`subject_id`) REFERENCES `sch_subjects` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_recMat_topic` FOREIGN KEY (`topic_id`) REFERENCES `slb_topics` (`id`) ON DELETE SET NULL
