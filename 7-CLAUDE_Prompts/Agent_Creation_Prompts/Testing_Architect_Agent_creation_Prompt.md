# Agent Creation Prompt — Testing Architect
...
Please create the **Testing Architect** agent for the Prime-AI project by executing all steps below.
...
Create the file at:
`/Users/bkwork/WorkFolder/1-Old_PrimeDB/old_db/AI_Brain/agents/testing-architect.md`
...
**RULE: Never use `RefreshDatabase` at the top level** — it wipes global_db reference data (boards, states, languages, sys_dropdown entries). Always use `TenantTestCase` for tenant tests.
...
- Append `| CODE | **TEST WRITTEN** | \`tests/Feature/...\`` to that row in known-issues.md
...
## STEP 3 — Verify `testing-strategy.md` exists

Check if this file exists:
`/Users/bkwork/WorkFolder/1-Old_PrimeDB/old_db/AI_Brain/memory/testing-strategy.md`
...
## Stack
- Framework: Pest 4.x on Laravel 12
- DB driver: MySQL 8.0
- Parallelism: `--parallel` flag (process-based isolation)
- Coverage tool: Xdebug (enforced minimum 60%)

## 3-Database Architecture
- global_db: never refresh in tests — use read-only seeders
- prime_db: central tests use Tests\TestCase + RefreshDatabase
- tenant_db: tenant tests use Tests\TenantTestCase — only tenant DB is refreshed

## Test Pyramid Targets
- 70% Feature (HTTP layer)
- 15% Integration (service + model, real DB)
- 10% Unit (pure logic, mocked)
- 5% Browser (Dusk, critical paths only)

## Priority Order Per Module
1. Route smoke (no 500s)
...
- Runs on: push to main/Brijesh/develop; PRs to main
- Security group tests run on every PR
- Performance group tests run nightly
```
...
3. Whether `testing-strategy.md` existed or was created
4. Reply with: `Active role system updated. Say "act as Testing Architect" in any future session to activate this agent.`
