...
---
Evaluate Completness in Below Categories :
...
After discovering more P0/P1 issues than were previously recorded, I judged that the number should go **down** from 30%. The specific amount of reduction (10 percentage points, landing at 20%) was intuition, not calculation.
...
> A module is 100% complete when: (a) it can be deployed to any new tenant without manual intervention, (b) every registered route works without errors, (c) every route requires appropriate authorization, (d) all stated features produce correct output (no stubs, no broken data), and (e) there are no blocking performance issues.
...
Security carries the highest weight (30%) because an unsecured module is worse than an incomplete one — it can cause active data breaches. Route integrity (25%) is second because broken routes mean the module simply does not function for users.
...
5. **Which formula approach do you prefer?**
   ...
   - **Option C — Checklist-Based Score**: define a fixed checklist of 20-30 items per module (routes, auth, migrations, etc.), score each pass/fail, divide sum by total items — very objective and simple but less nuanced
...
Even before agreeing on a formula, the progress.md entries for all modules should have a **"how this % was computed"** footnote. Currently, numbers like "~50% StudentPortal", "~80% SchoolSetup", "~82% Transport" are in the file with no derivation, making them unmaintainable.
