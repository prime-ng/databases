...
{
    protected array $occupied = []; // Tracks which class has which period booked
    protected array $teacherOccupied = []; // Tracks which teacher has which period booked
    protected array $entries = []; // Final timetable entries (what gets saved to DB
    protected array $activityPlacements = []; // Tracks where each activity was placed
    protected ConstraintManager $constraintManager; // Manages all rules/constraints
    protected array $activitiesById = []; // Cache activities by ID for quick lookup

    // Cache teaching periods indices
    protected array $teachingIndices = []; // Stores [1, 2, 4, 5] = indices of teaching per day
    protected array $teachingPeriods = [];  // The actual period objects (without breaks)

    // Stats tracking - monitors performance
    protected array $stats = [
        'activities' => 0,                   // How many activities total
        'total_periods_needed' => 0,         // How many teaching periods needed
        'periods_placed' => 0,               // How many periods actually scheduled
        'slot_evaluations' => 0,             // How many slots we checked
        'constraint_checks' => 0,            // How many rule checks we performed
        'generation_time' => 0,              // How long it took
        'max_attempts_exceeded' => 0,        // How many times we failed after many tries
        'activities_with_no_slots' => 0,     // Activities we couldn't schedule at all
        'activities_partially_placed' => 0,  // Activities we partially scheduled
        'activities_fully_placed' => 0,      // Activities we fully scheduled
    ];

    public function __construct(
        protected Collection $days, // Monday-Friday objects 
        protected Collection $periods,  // 8:00-8:45, 8:45-9:30, etc.
        ConstraintManager $constraintManager = null
    ) {
        $this->days = $days->values(); // Reset array keys to 0,1,2,3...
        $this->periods = $periods->values(); // Reset array keys

        // Pre-calculate teaching periods (skip breaks)
        $this->calculateTeachingPeriods(); // Fill $teachingIndices and $teachingPeriods

        // If no constraint manager provided, create empty one
    ...
    {
        $this->teachingIndices = [];  // Reset
        $this->teachingPeriods = [];  // Reset

        // Loop through all periods
        foreach ($this->periods as $index => $period) {
            // If NOT a break (SBREAK or LUNCH), add to teaching periods
            if (!in_array($period->code, ['SBREAK', 'LUNCH'])) {
                $this->teachingIndices[] = $index;     // Store position [0,1,3,4...]
                $this->teachingPeriods[] = $period;    // Store period object
            }
        }
    ...
    {
        $periodsNeeded = $activity->weekly_periods;    // e.g., Math needs 5 periods/week
        $periodsPlaced = 0;                            // How many we've placed so far
        $maxAttempts = 2000;                           // Stop trying after 2000 attempts
        $attempts = 0;                                 // Current attempt count -> we can set this in system config or other settings
        $backoffCounter = 0;                           // Tracks consecutive failures
    ...
    protected function findCandidateSlot(Activity $activity): ?Slot
    {
        ...
        // Calculate maximum starting position for this activity
        // e.g., if 6 teaching periods total and activity needs 2 periods:
        // maxTeachingStart = 6 - 2 = 4 (can start at positions 0,1,2,3,4)
        ...
        // Get days sorted by current load (least busy first)
        ...
        foreach ($days as $day) {
            ...
            foreach ($teachingStartPositions as $teachingStartIndex) {
                ...
                $this->stats['slot_evaluations']++; // Track that we checked a slot
            }
        }
