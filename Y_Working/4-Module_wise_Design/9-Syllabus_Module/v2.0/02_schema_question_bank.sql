...
CREATE TABLE IF NOT EXISTS `qb_questions` (
  ...
  -- Classification
  `topic_id` INT UNSIGNED NOT NULL,            -- FK to syl_topics
  `competency_id` INT UNSIGNED DEFAULT NULL,   -- FK to syl_competencies
  `lesson_id` INT UNSIGNED DEFAULT NULL,       -- Denormalized
  `class_id` INT UNSIGNED NOT NULL,               -- Denormalized
  `subject_id` INT UNSIGNED NOT NULL,          -- Denormalized
  ...
  `parent_question_id` INT UNSIGNED DEFAULT NULL, -- For variations
