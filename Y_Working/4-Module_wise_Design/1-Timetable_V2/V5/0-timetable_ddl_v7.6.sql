...
-- -------------------------------------------------
    ...
    `term_total_teaching_days` TINYINT UNSIGNED DEFAULT 5,
    `term_total_exam_days` TINYINT UNSIGNED DEFAULT 2,
    `term_week_start_day` TINYINT UNSIGNED NOT NULL COMMENT '1=Monday, 7=Sunday',
    `term_total_periods_per_day` TINYINT UNSIGNED NOT NULL,
    `term_total_teaching_periods_per_day` TINYINT UNSIGNED NOT NULL,
    `term_min_resting_periods_per_day` TINYINT UNSIGNED NOT NULL,
    `term_max_resting_periods_per_day` TINYINT UNSIGNED NOT NULL,
    `term_travel_minutes_between_classes` TINYINT UNSIGNED NOT NULL,
  ...
  -- Conditions:
    ...
    `validation_rules` JSON DEFAULT NULL,                 -- Added New for value validation
    `additional_info` JSON DEFAULT NULL,
    `tenant_can_modify` TINYINT(1) NOT NULL DEFAULT 0,
    `mandatory` TINYINT(1) NOT NULL DEFAULT 1,
    `used_by_app` TINYINT(1) NOT NULL DEFAULT 1,
    `version` SMALLINT UNSIGNED NOT NULL DEFAULT 1,       -- Added New for versioning
  ...
  -- Data Seed for tt_config
    ...
    `algorithm_version` VARCHAR(20) DEFAULT '1.0',      -- Added New
    ...
    `tabu_tenure` INT UNSIGNED DEFAULT 10,              -- Added New
    -- Simulated annealing params
    `initial_temperature` DECIMAL(10,2) DEFAULT 100.00, -- Added New
    `cooling_rate` DECIMAL(5,2) DEFAULT 0.95,
    `min_temperature` DECIMAL(10,2) DEFAULT 1.00,       -- Added New
...
-- -------------------------------------------------
    ...
    `workload_factor` DECIMAL(5,2) DEFAULT 1.00,
    `max_concurrent_classes` TINYINT UNSIGNED DEFAULT 1,
    `ordinal` TINYINT UNSIGNED DEFAULT 1,
    ...
    `day_of_week` TINYINT UNSIGNED NOT NULL COMMENT '1=Monday, 7=Sunday (ISO)',
    `iso_weekday` TINYINT UNSIGNED GENERATED ALWAYS AS (day_of_week) STORED,
    `ordinal` TINYINT UNSIGNED NOT NULL,
    `is_school_day` TINYINT(1) NOT NULL DEFAULT 1,
    `is_weekend` TINYINT(1) GENERATED ALWAYS AS (CASE WHEN day_of_week IN (6,7) THEN 1 ELSE 0 END) STORED,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    ...
    `period_set_id` INT UNSIGNED DEFAULT NULL COMMENT 'Override period set for this day',
    `is_school_day` TINYINT(1) NOT NULL DEFAULT 1,
    ...
    `period_set_id` INT UNSIGNED DEFAULT NULL COMMENT 'Override period set for this class on this day',
    ...
    `total_duration_minutes` INT UNSIGNED GENERATED ALWAYS AS (TIMESTAMPDIFF(MINUTE, day_start_time, day_end_time)) STORED,
    `is_default` TINYINT(1) DEFAULT 0,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    `is_consecutive_allowed` TINYINT(1) DEFAULT 1,
    `max_consecutive` TINYINT UNSIGNED DEFAULT 1,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    ...
    `max_weekly_periods_teacher` TINYINT UNSIGNED DEFAULT 48,
    `min_weekly_periods_teacher` TINYINT UNSIGNED DEFAULT 15,
...
-- -------------------------------------------------
    ...
    `daily_slots_distribution_json` JSON DEFAULT NULL COMMENT 'Distribution pattern across days',
    ...
    `priority_score` TINYINT UNSIGNED DEFAULT 50,
    `difficulty_score` TINYINT UNSIGNED DEFAULT 50,
    `resource_scarcity_index` DECIMAL(5,2) DEFAULT NULL,
    `teacher_scarcity_index` DECIMAL(5,2) DEFAULT NULL,
    -- Status
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- -------------------------------------------------
	...
	CREATE TABLE IF NOT EXISTS `tt_constraint_type` (
		`id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
		`code` VARCHAR(60) NOT NULL,                     -- e.g., 'TEACHER_MAX_DAILY', 'CLASS_MIN_PERIODS', 'ACTIVITY_CONSECUTIVE'
		`name` VARCHAR(150) NOT NULL,                    -- e.g., 'Teacher Maximum Daily Periods', 'Class Minimum Periods Per Day'
		`description` VARCHAR(500) DEFAULT NULL,
		`category_id` TINYINT UNSIGNED NOT NULL,         -- FK to tt_constraint_category.id
		`scope_id` TINYINT UNSIGNED NOT NULL,            -- FK to tt_constraint_scope.id
		`constraint_level` ENUM('HARD', 'STRONG', 'MEDIUM', 'SOFT', 'OPTIMIZATION') NOT NULL DEFAULT 'MEDIUM',
		`default_weight` TINYINT UNSIGNED NOT NULL DEFAULT 50,  -- 1-100
		`is_hard_capable` TINYINT(1) NOT NULL DEFAULT 1,        -- Can this be set as hard constraint?
		`is_soft_capable` TINYINT(1) NOT NULL DEFAULT 1,        -- Can this be set as soft constraint?
		`parameter_schema` JSON NOT NULL,                -- JSON schema for parameters validation
		`validation_logic` TEXT DEFAULT NULL,            -- Custom validation logic (pseudo-code or SQL)
		`conflict_detection_logic` TEXT DEFAULT NULL,    -- Logic to detect conflicts
		`resolution_priority` TINYINT UNSIGNED DEFAULT 50, -- Priority in conflict resolution
		`applicable_target_types` JSON NOT NULL,         -- Array of target_type_ids this constraint applies to
		`is_active` TINYINT(1) NOT NULL DEFAULT 1,
		`created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
		`updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		`deleted_at` TIMESTAMP NULL DEFAULT NULL,
	...
	CREATE TABLE IF NOT EXISTS `tt_constraint` (
		...
		`name` VARCHAR(200) DEFAULT NULL,                 -- Optional custom name
		`description` VARCHAR(500) DEFAULT NULL,
		`academic_term_id` INT UNSIGNED DEFAULT NULL,     -- FK to tt_academic_term.id (NULL = applies to all terms)
		`timetable_type_id` INT UNSIGNED DEFAULT NULL,    -- FK to tt_timetable_type.id (NULL = applies to all types)
		-- Target specification (polymorphic)
		`target_type_id` TINYINT UNSIGNED NOT NULL,       -- FK to tt_constraint_target_type.id
		`target_id` INT UNSIGNED NOT NULL,                -- ID of the target (teacher_id, class_id, etc.)
		-- Constraint parameters
		`is_hard` TINYINT(1) NOT NULL DEFAULT 0,          -- Override level to hard
		`weight` TINYINT UNSIGNED NOT NULL DEFAULT 50,     -- 1-100, overrides default_weight
		`params_json` JSON NOT NULL,                       -- Actual constraint parameters
		-- Temporal validity
		`effective_from_date` DATE DEFAULT NULL,
		`effective_to_date` DATE DEFAULT NULL,
		`apply_for_all_days` TINYINT(1) NOT NULL DEFAULT 1,
		`applicable_days_json` JSON DEFAULT NULL,          -- Array of day_of_week (1-7)
		`applicable_periods_json` JSON DEFAULT NULL,       -- Array of period_ord (1-n)
		-- Additional metadata
		`impact_score` TINYINT UNSIGNED DEFAULT 50,        -- Estimated difficulty impact (1-100)
		`constraint_hash` VARCHAR(64) GENERATED ALWAYS AS (SHA2(CONCAT_WS('|', 
				constraint_type_id, target_type_id, target_id, 
				COALESCE(params_json, ''), is_hard, weight), 256)) STORED,
		-- Audit
		`is_active` TINYINT(1) NOT NULL DEFAULT 1,
	...
	CREATE TABLE IF NOT EXISTS `tt_teacher_unavailable` (
		...
		`constraint_id` INT UNSIGNED DEFAULT NULL,        -- Link to parent constraint
		`unavailable_for_all_days` TINYINT(1) NOT NULL DEFAULT 0,
		`unavailable_for_all_periods` TINYINT(1) NOT NULL DEFAULT 0,
		`day_of_week` TINYINT UNSIGNED DEFAULT NULL,      -- 1-7 (Monday=1, ISO)
		`period_ord` TINYINT UNSIGNED DEFAULT NULL,       -- Period number
	...
	CREATE TABLE IF NOT EXISTS `tt_constraint_violation` (
		...
		`severity` TINYINT UNSIGNED NOT NULL DEFAULT 100, -- 1-100
		`violation_count` INT UNSIGNED NOT NULL DEFAULT 1,
		`affected_entity_type` TINYINT UNSIGNED DEFAULT NULL, -- FK to tt_constraint_target_type
		`affected_entity_id` INT UNSIGNED DEFAULT NULL,
		`day_of_week` TINYINT UNSIGNED DEFAULT NULL,
		`period_ord` TINYINT UNSIGNED DEFAULT NULL,
		`violation_details_json` JSON DEFAULT NULL,
		`suggested_resolution_json` JSON DEFAULT NULL,
...
-- -------------------------------------------------
    ...
    `current_allocated_periods` TINYINT UNSIGNED DEFAULT 0,
    ...
    `competency_level` ENUM('Facilitator', 'Basic', 'Intermediate', 'Advanced', 'Expert') DEFAULT 'Basic',
    ...
    `available_for_full_timetable` TINYINT(1) GENERATED ALWAYS AS 
        (IF(`teacher_available_from_date` <= `timetable_start_date`, 1, 0)) STORED,
    `days_not_available` INT GENERATED ALWAYS AS 
        (GREATEST(0, DATEDIFF(`teacher_available_from_date`, `timetable_start_date`))) STORED,
    `min_availability_score` DECIMAL(7,2) DEFAULT NULL,
    `max_availability_score` DECIMAL(7,2) DEFAULT NULL,
    -- Activity link
    `activity_id` INT UNSIGNED NULL,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    `availability_status` ENUM('Available', 'Unavailable', 'Assigned', 'Free Period', 'Break') NOT NULL DEFAULT 'Available',
    ...
    `assigned_activity_id` INT UNSIGNED DEFAULT NULL,
    `constraint_id` INT UNSIGNED DEFAULT NULL,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    `overall_status` ENUM('Available', 'Unavailable', 'Partially Available', 'Assigned') NOT NULL DEFAULT 'Available',
    `available_for_full_timetable` TINYINT(1) NOT NULL DEFAULT 1,
    ...
    `day_name` VARCHAR(10) NOT NULL,
    `period_number` TINYINT UNSIGNED NOT NULL,
    `availability_status` ENUM('Available', 'Unavailable', 'Assigned') NOT NULL DEFAULT 'Available',
    `assigned_class_id` INT UNSIGNED DEFAULT NULL,
    `assigned_section_id` INT UNSIGNED DEFAULT NULL,
    `assigned_subject_study_format_id` INT UNSIGNED DEFAULT NULL,
    `assigned_activity_id` INT UNSIGNED DEFAULT NULL,
...
-- -------------------------------------------------
    ...
    `average_teacher_availability` DECIMAL(7,2) DEFAULT 1.00,
    ...
    `historical_success_rate` DECIMAL(5,2) DEFAULT NULL,
    -- Calculated priority
    `calculated_priority` DECIMAL(8,3) DEFAULT NULL,
    `manual_override_priority` DECIMAL(8,3) DEFAULT NULL,
    `final_priority` DECIMAL(8,3) GENERATED ALWAYS AS (COALESCE(`manual_override_priority`, `calculated_priority`)) STORED,
    -- Component weights (configurable)
    `weight_teacher_scarcity` TINYINT UNSIGNED DEFAULT 25,
    `weight_weekly_load` TINYINT UNSIGNED DEFAULT 15,
    `weight_teacher_availability` TINYINT UNSIGNED DEFAULT 15,
    `weight_rigidity` TINYINT UNSIGNED DEFAULT 20,
    `weight_resource_scarcity` TINYINT UNSIGNED DEFAULT 15,
    `weight_subject_difficulty` TINYINT UNSIGNED DEFAULT 10,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    ...
    `eligible_room_count` INT UNSIGNED DEFAULT NULL,
    `room_availability_score` DECIMAL(7,2) DEFAULT 1.00,
    -- Activity timing
    ...
    `placement_complete` TINYINT(1) DEFAULT 0,
    `placed_periods_count` TINYINT UNSIGNED DEFAULT 0,
    -- Audit
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_by` INT UNSIGNED DEFAULT NULL,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    `min_gap_from_previous` TINYINT UNSIGNED DEFAULT 0,
    `max_gap_from_previous` TINYINT UNSIGNED DEFAULT NULL,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    ...
    `ordinal` TINYINT UNSIGNED DEFAULT 1,
    `preference_score` TINYINT UNSIGNED DEFAULT 50,
    `allocation_status` ENUM('PENDING', 'ALLOCATED', 'CONFLICT', 'SUBSTITUTED') DEFAULT 'PENDING',
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
...
-- -------------------------------------------------
    ...
    `duration_seconds` INT UNSIGNED GENERATED ALWAYS AS 
        (TIMESTAMPDIFF(SECOND, started_at, finished_at)) STORED,
    -- Status
    ...
    -- Detailed stats
    `placement_attempts` INT UNSIGNED DEFAULT 0,
    `swaps_performed` INT UNSIGNED DEFAULT 0,
    `backtracks_performed` INT UNSIGNED DEFAULT 0,
    `stats_json` JSON DEFAULT NULL,
    -- Error handling
    `error_message` TEXT DEFAULT NULL,
    ...
    `class_id` INT UNSIGNED NOT NULL,
    `section_id` INT UNSIGNED DEFAULT NULL,
    `subject_study_format_id` INT UNSIGNED DEFAULT NULL,
    ...
    `validation_status` ENUM('VALID', 'WARNING', 'VIOLATION') DEFAULT 'VALID',
    -- Audit
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    `substitution_log_id` INT UNSIGNED DEFAULT NULL,
    `is_active` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL DEFAULT NULL,
    ...
    `start_time` TIME,
    `end_time` TIME,
    `duration_minutes` INT UNSIGNED GENERATED ALWAYS AS (TIMESTAMPDIFF(MINUTE, start_time, end_time)) STORED,
    `booked_for_type` ENUM('ACTIVITY', 'EXAM', 'EVENT', 'MAINTENANCE', 'MEETING') NOT NULL,
    `booked_for_id` INT UNSIGNED NOT NULL,
    `timetable_cell_id` INT UNSIGNED DEFAULT NULL,
    `purpose` VARCHAR(500),
...
-- -------------------------------------------------
    ...
    `preference_satisfaction_rate` DECIMAL(5,2) DEFAULT NULL,
    `requested_changes_count` SMALLINT UNSIGNED DEFAULT 0,
    -- Audit
...
-- -------------------------------------------------
    ...
    `change_type` ENUM('CREATE', 'UPDATE', 'DELETE', 'LOCK', 'UNLOCK', 'SWAP', 'SUBSTITUTE', 'BULK_UPDATE') NOT NUL,
...
-- -------------------------------------------------
    ...
    `is_full_day` TINYINT(1) GENERATED ALWAYS AS (CASE WHEN start_period IS NULL AND end_period IS NULL THEN 1 ELSE 0 END) STORED,
    -- Details
    `reason` VARCHAR(500) DEFAULT NULL,
    `document_proof` VARCHAR(255) DEFAULT NULL,
    `contact_during_absence` VARCHAR(100) DEFAULT NULL,
    -- Status
    ...
    `substitution_deadline` TIMESTAMP NULL,
    -- Notification
    ...
    `effectiveness_rating` TINYINT UNSIGNED DEFAULT NULL COMMENT '1-5',
    -- Audit
