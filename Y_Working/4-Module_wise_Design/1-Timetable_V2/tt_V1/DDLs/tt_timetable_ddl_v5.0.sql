...
-- -----------------------------------------------------
-- Check - it should Capture Sub-Group Requirement as well
...
CREATE TABLE IF NOT EXISTS `tt_activity` (
  ...
  -- Check if class_group_id or class_subgroup_id is set but both should not be set
  ...
  -- Check - Why we are capturing Subject & Study_format saperate when we have combination of those?
  `subject_id` INT UNSIGNED DEFAULT NULL, -- FK to sch_subjects
  `study_format_id` INT UNSIGNED DEFAULT NULL, -- FK to sch_study_formats
  -- Duration
  -- Check - What is `duration_periods`?
  -- Check - What is the Use of total_periods?
  ...
  -- Scheduling preferences
    -- Check - What is `difficulty_score` & `priority`?
...
-- -----------------------------------------------------
-- Check - What is `duration_periods`? Is it Number of Periods or Duration(Minutes) of the Period?
...
CREATE TABLE IF NOT EXISTS `tt_constraint` (
  ...
  -- Check - we can use dropdown_table for target_type with target_table_name in additional_info in dropdown_tables
...
-- Check - we need to keep complete detail for what all constraints are violated. Which we can do by adding a new table tt_constraint_violation
