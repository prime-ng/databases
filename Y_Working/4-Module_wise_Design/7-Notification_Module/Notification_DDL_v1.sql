...
CREATE TABLE IF NOT EXISTS ntf_notifications (
    ...
    `template_id` INT UNSIGNED NULL,        -- Template ID
    `priority_id` INT UNSIGNED NOT NULL,    -- 'LOW, NORMAL, HIGH, URGENT'
    `confidentiality_level_id` INT UNSIGNED NOT NULL, -- 'PUBLIC, RESTRICTED, CONFIDENTIAL'
    `scheduled_at` DATETIME NULL,
    `recurring` TINYINT(1) DEFAULT 0,             -- 0: One Time, 1: Recurring
    `recurring_interval_id` INT UNSIGNED NULL, -- fk to sys_dropdown_table e.g. 'HOURLY, DAILY, WEEKLY, MONTHLY, QUARTERLY, YEARLY'
...
CREATE TABLE IF NOT EXISTS ntf_notification_channels (
    ...
    `channel_id` INT UNSIGNED NOT NULL,    -- fk to sys_dropdown_table e.g. 'APP, SMS, WHATSAPP, EMAIL'
    `provider_id` INT UNSIGNED NULL,       -- fk to sys_dropdown_table e.g. 'MSG91, Twilio, AWS SES, Meta API'
    `status_id` INT UNSIGNED NOT NULL,     -- fk to sys_dropdown_table e.g. 'PENDING, SENT, FAILED, RETRIED'
    `scheduled_at` DATETIME NULL,
...
CREATE TABLE IF NOT EXISTS ntf_notification_targets (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `notification_id` INT UNSIGNED NOT NULL,
    `target_type_id` INT UNSIGNED NOT NULL, -- FK to sys_dropdown_table e.g. USER, ROLE, DEPARTMENT, DESIGNATION, CLASS, SECTION, SUBJECT, ENTITY_GROUP, ENTIRE_SCHOOL
    ...
    `target_selected_id` INT UNSIGNED NULL, -- Reference ID based on target type e.g. user_id, role_id, designation_id, etc.
    `resolved_user_id` INT UNSIGNED NULL, -- Final resolved recipient
    `is_active` TINYINT(1) DEFAULT 1,
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL,
...
CREATE TABLE IF NOT EXISTS ntf_user_preferences (
    ...
    `channel_id` INT UNSIGNED NOT NULL, -- 'APP, SMS, WHATSAPP, EMAIL'
...
CREATE TABLE IF NOT EXISTS ntf_delivery_logs (
    ...
    `channel_id` INT UNSIGNED NOT NULL,
    `user_id` INT UNSIGNED NOT NULL,
    `delivery_status_id` INT UNSIGNED NOT NULL, -- 'SENT, FAILED, READ, CLICKED'
...
/* =========================================================
   7. REQUIRED DROPDOWN SEEDS
   ========================================================= */

-- Notification Channels

INSERT INTO sys_dropdown_table (type, value) VALUES
...
('NOTIFICATION_TARGET', 'ROLE'),
('NOTIFICATION_TARGET', 'DESIGNATION'),
('NOTIFICATION_TARGET', 'CLASS'),
('NOTIFICATION_TARGET', 'SECTION'),
('NOTIFICATION_TARGET', 'SUBJECT'),
('NOTIFICATION_TARGET', 'ENTIRE_SCHOOL');
