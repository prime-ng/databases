...
CREATE TABLE IF NOT EXISTS `cmp_sla_configs` (
  ...
  `category_id` INT UNSIGNED DEFAULT NULL, -- Specific category (Optional)
...
CREATE TABLE IF NOT EXISTS `cmp_complaints` (
  ...
  -- Classification
  `category_id` INT UNSIGNED NOT NULL, -- Main Category
  `subcategory_id` INT UNSIGNED DEFAULT NULL, -- Sub-Category (Optional)
  ...
  `route_id` INT UNSIGNED DEFAULT NULL, -- FK to tpt_route (if transport)
  `vehicle_id` INT UNSIGNED DEFAULT NULL, -- FK to tpt_vehicle (if transport)
  ...
  `assigned_to_role` INT UNSIGNED DEFAULT NULL, -- FK to sys_roles (Current Role handling it)
  `assigned_to_user` INT UNSIGNED DEFAULT NULL, -- FK to sys_users (Specific Officer)
...
CREATE TABLE IF NOT EXISTS `cmp_complaint_actions` (
  ...
  -- Action Details
  `performed_by_user` INT UNSIGNED DEFAULT NULL, -- FK to sys_users (NULL for System)
  `performed_by_role` VARCHAR(50) DEFAULT NULL, -- Role Name at time of action
  `assigned_to_user` INT UNSIGNED DEFAULT NULL, -- If reassigned
...
CREATE TABLE IF NOT EXISTS `cmp_attachments` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `complaint_id` INT UNSIGNED NOT NULL,
  `action_id` INT UNSIGNED DEFAULT NULL, -- If attached to a specific comment/action
