...
-- ---------------------------------------------------------------------
-- ENHANCEMENTS IN V2.0 (vs v1):
-- 1. Adopted PrimeDB conventions: InnoDB / utf8mb4_unicode_ci, snake_case plural
--    table names, `_jnt` for junctions, audit columns (is_active, created_at,
--    updated_at, deleted_at, created_by), `_json` suffix for JSON columns.
-- 2. Replaced loose `class_section_id INT` with proper FK to
--    `sch_class_section_jnt(id)` — re-uses School Setup master data.
-- 3. Replaced loose `teacher_id INT` with FK to `sys_users(id)` so any staff
--    user (not only class teacher) can be assigned (Req §3 / Teacher
--    Constraints — Default Class Teacher, Multi-Teacher Assignment).
-- 4. Split slot allocation into two clear modes:
--    `allocation_mode = 'SCHOOL_ALLOCATED' | 'PARENT_PICK'` on
--    `ptm_assignments` (Req §2 — Slot Allocation System).
-- 5. New `ptm_event_settings` table for booking window, cancellation lead
--    time, and other event-wide policy switches (Req §2 — Booking & Deadlines).
-- 6. New `ptm_assignment_teacher_jnt` to support multiple teachers running
--    the same class+section in parallel sub-batches (Req §3 — Multi-Teacher
--    Assignment).
-- 7. New `ptm_blockouts` for global teacher block-out periods (lunch / staff
--    meeting) that are honoured before any slot can be generated or booked
--    (Req §3 — Mandatory Breaks).
-- 8. New `ptm_slot_bookings` separated from `ptm_slots` so that a single
--    "Group Slot" (capacity > 1) can hold N bookings, while still enforcing
--    "one booking per student per teacher per event" via a generated unique
--    key (Req §2 — Limit per Student, Slot Capacity).
-- 9. `ptm_event_class_section_jnt` decides which classes/sections actually
--    participate in a given PTM event — this is the master scope from which
--    ptm_assignments draw.
-- 10. Buffer minutes captured at batch level AND at assignment level
--     (assignment overrides batch) — supports school-wide minimums while
--     still letting a specific class run tighter or looser.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. ptm_event_settings
-- Event-wide policy switches (booking window, cancellation rules, etc.).
-- Kept as a 1-to-1 sibling of ptm_events so admins can update policy without
-- touching the event header — and so that this table can grow new flags
-- without ALTERing ptm_events.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `ptm_event_settings` (
  `id`                          int unsigned NOT NULL AUTO_INCREMENT,
  `ptm_event_id`                int unsigned NOT NULL,        -- FK ptm_events.id (1:1)
  ...
  `deleted_at`                  timestamp    NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_ptmEventSettings_eventId` (`ptm_event_id`),  -- 1 settings row per event
  CONSTRAINT `fk_ptmEventSettings_eventId`   FOREIGN KEY (`ptm_event_id`) REFERENCES `ptm_events` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_ptmEventSettings_createdBy` FOREIGN KEY (`created_by`)   REFERENCES `sys_users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
-- Sample row:
--   (1, 1, '2026-05-01 09:00:00','2026-05-09 23:59:59', 24, 1, 10, 2, 1, 1, 1, ...)


-- ---------------------------------------------------------------------------
-- 3. ptm_event_class_section_jnt
...
-- The *time-grid* of the batch is stored in ptm_batch_slot_definitions.
-- A single batch template can be applied to many class+sections (Req §3 —
-- Template Reusability).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `ptm_batches` (
  `id`                       int unsigned NOT NULL AUTO_INCREMENT,
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `ptm_batch_slot_definitions` (
  `id`               int unsigned NOT NULL AUTO_INCREMENT,
  `batch_id`         int unsigned NOT NULL,            -- FK ptm_batches.id
  ...
  `deleted_at`       timestamp    NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_ptmBatchSlotDef_batch_ordinal` (`batch_id`,`ordinal`),
  UNIQUE KEY `uq_ptmBatchSlotDef_batch_startTime` (`batch_id`,`slot_start_time`),
  CONSTRAINT `fk_ptmBatchSlotDef_batchId` FOREIGN KEY (`batch_id`) REFERENCES `ptm_batches` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
-- Sample rows for batch_id=1 (12 × 10-min slots, 9-11 AM):
...
CREATE TABLE IF NOT EXISTS `ptm_assignments` (
  ...
  `batch_id`                    int unsigned NOT NULL,             -- FK ptm_batches.id (the template applied)
  ...
  `override_buffer_min`         tinyint unsigned DEFAULT NULL,     -- if set, overrides ptm_batches.buffer_min for THIS assignment
  `override_max_participants`   tinyint unsigned DEFAULT NULL,     -- if set, overrides ptm_batches.max_participants_per_slot
  ...
  KEY `idx_ptmAssign_batch` (`batch_id`),
  ...
  CONSTRAINT `fk_ptmAssign_batchId`     FOREIGN KEY (`batch_id`)               REFERENCES `ptm_batches` (`id`),
...
--   * If override_* is NULL, application falls back to the parent ptm_batches values.


-- ---------------------------------------------------------------------------
-- 7. ptm_assignment_teacher_jnt
...
CREATE TABLE IF NOT EXISTS `ptm_slots` (
  ...
  `batch_slot_def_id`   int unsigned DEFAULT NULL,              -- FK ptm_batch_slot_definitions.id (the template row this came from; NULL if hand-created)
  ...
  CONSTRAINT `fk_ptmSlots_batchSlotDefId`      FOREIGN KEY (`batch_slot_def_id`)     REFERENCES `ptm_batch_slot_definitions` (`id`),
...
--    For each ptm_batch_slot_definitions row of the assignment's batch,
