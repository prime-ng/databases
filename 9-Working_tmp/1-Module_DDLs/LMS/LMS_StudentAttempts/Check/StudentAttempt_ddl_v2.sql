...
-- =========================================================================
-- File    : StudentAttempt_ddl_v2.sql
-- Database: tenant_db
-- Prefix  : lms_
...
-- Version : v2 — Enhanced from v1
--           Fixes: deduplicated tables, missing standard columns, FK refs,
...
-- lms_quizzes           → Modules/LmsQuiz          (assessment_id when QUIZ)
-- lms_quests            → Modules/LmsQuests        (assessment_id when QUEST)
-- lms_quiz_allocations  → Modules/LmsQuiz          (allocation_id when QUIZ — polymorphic)
-- lms_quest_allocations → Modules/LmsQuests        (allocation_id when QUEST — polymorphic)
...
CREATE TABLE IF NOT EXISTS `lms_quiz_quest_attempts` (
  ...
  `assessment_id`         INT UNSIGNED NOT NULL,                -- Polymorphic: lms_quizzes.id | lms_quests.id
  `allocation_id`         INT UNSIGNED DEFAULT NULL,            -- Polymorphic: lms_quiz_allocations.id | lms_quest_allocations.id
  -- Attempt Tracking
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_qqat_student_assessment_attempt`(`student_id`, `assessment_type`, `assessment_id`, `attempt_number`),
  KEY `idx_qqat_student`              (`student_id`),
  KEY `idx_qqat_assessment`           (`assessment_type`, `assessment_id`),
  KEY `idx_qqat_allocation`           (`assessment_type`, `allocation_id`),
  ...
  -- Note: assessment_id FK omitted — polymorphic (QUIZ or QUEST)
  -- Note: allocation_id FK omitted — polymorphic (quiz or quest allocation)
...
CREATE TABLE IF NOT EXISTS `lms_attempt_activity_logs` (
  ...
  `attempt_id`      INT UNSIGNED NOT NULL,
  -- Event
  `event_type`      ENUM('FOCUS_LOST','FULLSCREEN_EXIT','BROWSER_RESIZE','KEY_PRESS_BLOCKED','MOUSE_LEAVE','IP_CHANGE','TAB_SWITCH','COPY_PASTE_DETECTED','CONTEXT_MENU_OPENED','DEVTOOLS_DETECTED','WINDOW_BLUR','NETWORK_DISCONNECT') NOT NULL,
...
--          Allows a student to resume a quiz/quest/exam after browser crash,
--          accidental tab close, or network disconnect.
-- Design:  UPSERT pattern — one row per active attempt (UNIQUE on attempt_type+id).
--          Overwritten on each auto-save tick.
--          checkpoint_data holds full answer state snapshot for restore.
-- -------------------------------------------------------------------------
...
-- 9  | lms_attempt_activity_logs         | Proctoring/behavioral event log
-- 10 | lms_attempt_checkpoints           | Session save-state for resume
-- =========================================================================
