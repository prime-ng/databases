...
-- ========================================================================================================
  ...
  CREATE TABLE IF NOT EXISTS `rec_assessment_types` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `type_name` VARCHAR(50) NOT NULL,  -- ALL, QUIZ, QUEST, WEEKLY_TEST, TERM_EXAM, FINAL_EXAM
    `description` VARCHAR(255) DEFAULT NULL,
    `is_active` TINYINT(1) DEFAULT 1,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  CREATE TABLE IF NOT EXISTS `rec_recommendation_materials` (
    ...
    `material_type` INT UNSIGNED DEFAULT NULL,      -- fk to rec_dynamic_material_types (e.g. 'TEXT','VIDEO','PDF','AUDIO','QUIZ','ASSIGNMENT','LINK','INTERACTIVE')
    `purpose` INT UNSIGNED DEFAULT NULL,            -- fk to rec_dynamic_purposes (e.g. 'REVISION','PRACTICE','REMEDIAL','ADVANCED','ENRICHMENT','CONCEPT_BUILDING') NOT NULL DEFAULT 'PRACTICE',
    ...
    -- Academic Mapping
    `subject_id` INT UNSIGNED DEFAULT NOT NULL,     -- FK to sch_subjects
    `class_id` INT UNSIGNED DEFAULT NOT NULL,       -- FK to sch_classes (Target Class)
    `section_id` INT UNSIGNED DEFAULT NULL,         -- FK to sch_sections (optional, If Null - then applicable to all sections of the class)
    `topic_id` INT UNSIGNED DEFAULT NOT NULL,       -- FK to slb_topics
    `competency_id` INT UNSIGNED DEFAULT NULL,      -- FK to slb_competency_types (Optional link to Competency Framework)
    -- Metadata
    ...
    CONSTRAINT `fk_recMat_material_type` FOREIGN KEY (`material_type`) REFERENCES `rec_dynamic_material_types` (`id`),
    CONSTRAINT `fk_recMat_purpose` FOREIGN KEY (`purpose`) REFERENCES `rec_dynamic_purposes` (`id`),
  ...
  CREATE TABLE IF NOT EXISTS `rec_recommendation_rules` (
    ...
    -- Narrowing Scope
    `class_id` INT UNSIGNED DEFAULT NOT NULL,  -- FK to sch_classes
    `section_id` INT UNSIGNED DEFAULT NULL,  -- FK to sch_sections. -- If NULL, then applies to all sections of the class
    `subject_id` INT UNSIGNED DEFAULT NOT NULL,  -- FK to sch_subjects
    `topic_id` INT UNSIGNED DEFAULT NOT NULL,  -- FK to slb_topics
    ...
    `recommendation_mode_id` INT UNSIGNED NOT NULL,  -- FK to rec_recommendation_modes (SPECIFIC_MATERIAL, SPECIFIC_BUNDLE, DYNAMIC_BY_TOPIC, DYNAMIC_BY_COMPETENCY
  ...
  CREATE TABLE IF NOT EXISTS `rec_student_recommendations` (
    ...
    `assigned_date` DATE NOT NULL,               --  (*) Date when the recommendation was assigned
    -- Source of Recommendation
    `rule_id` INT UNSIGNED DEFAULT NULL,         -- fk to rec_recommendation_rules. Which rule generated this?
    `triggered_by_quiz_id` INT UNSIGNED DEFAULT NULL,         -- FK to lms_quizzes Optional: Link to the Quiz/Exam that triggered this recommendation
    `triggered_by_quest_id` INT UNSIGNED DEFAULT NULL,         -- FK to lms_quests Optional: Link to the Quest that triggered this recommendation
  -- `triggered_by_result_id` INT UNSIGNED DEFAULT NULL, -- Optional: Link to the Exam Result ID in Exam Module
    `manual_assigned_by` INT UNSIGNED DEFAULT NULL,     --  (*) FK to sys_users (Optional: Link to the User who manually assigned this recommendation)
    -- The Content
    `material_id` INT UNSIGNED DEFAULT NULL,  -- FK to rec_recommendation_materials. If rule recommended a specific material.  (*)
    `bundle_id` INT UNSIGNED DEFAULT NULL,    -- FK to rec_material_bundles. If rule recommended a bundle.  (*)
    -- Context
    `recommendation_reason` VARCHAR(255) DEFAULT NULL, --  (*) e.g. "Scored Low in Algebra Quiz"
    `priority` ENUM('LOW','MEDIUM','HIGH','CRITICAL') DEFAULT 'MEDIUM',  --  (*) Priority assigned based on rule or manual override. Can be used for sorting and urgency indication.
    `due_date` DATE DEFAULT NULL,  --  (*) Need to create a Key in sch_config, to be used here.
    -- Status Tracking
    `status` ENUM('PENDING','VIEWED','IN_PROGRESS','COMPLETED','SKIPPED','EXPIRED','CANCELLED') DEFAULT 'PENDING',  --  (*) Teacher can mannually Cancel the recommendation if deemed not relevant. System can mark as EXPIRED if past due date.
    `assigned_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  (*) When the recommendation was created/assigned
    `first_viewed_at` TIMESTAMP NULL DEFAULT NULL,
    `completed_at` TIMESTAMP NULL DEFAULT NULL,
    -- Outcomes
    `score_achieved` DECIMAL(5,2) DEFAULT NULL,     -- If the recommendation was a Quiz/Quest
    `student_rating` TINYINT UNSIGNED DEFAULT NULL, --  (*) (1-5 Stars) Teacher will provide feedback on whether the recommendation was relevant/helpful after completion. This can be used to refine rules and materials.
    `reassigned_quiz` TINYINT(1) DEFAULT 0,         -- If the student was reassigned a different quiz/material based on performance after initial recommendation
    `reassigned_quiz_id` INT UNSIGNED DEFAULT NULL, -- FK to lms_quizzes. The new quiz assigned if reassigned_quiz=1
    `student_feedback` VARCHAR(255) DEFAULT NULL,   --  (*) Optional free-text feedback from student on the recommendation
    `is_active` TINYINT(1) DEFAULT 1,  --  (*) Soft delete flag. If 0, then recommendation is considered deleted/archived and won't appear in active lists.
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    ...
    CONSTRAINT `fk_recStud_quiz` FOREIGN KEY (`triggered_by_quiz_id`) REFERENCES `lms_quizzes` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_recStud_quest` FOREIGN KEY (`triggered_by_quest_id`) REFERENCES `lms_quests` (`id`) ON DELETE SET NULL,
    ...
    CONSTRAINT `fk_recStud_assessmentType` FOREIGN KEY (`reassigned_quiz_id`) REFERENCES `lms_quizzes` (`id`) ON DELETE SET NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
