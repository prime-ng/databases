...
-- Main Quest Table
  CREATE TABLE IF NOT EXISTS `lms_quests` (
    ...
    -- Settings
    `duration_minutes` INT UNSIGNED DEFAULT NULL, -- Duration of the Quest in Minutes (If NULL then no time limit)
    `total_marks` DECIMAL(8,2) NOT NULL DEFAULT 0.00, -- Total Marks for the Quest (If this will be 0 then marks will be calculated based on questions added to the quest)
    `total_questions` INT UNSIGNED NOT NULL DEFAULT 0, -- Total Questions in the Quest (If this will be 0 then questions will be calculated based on questions added to the quest)
    `passing_percentage` DECIMAL(5,2) NOT NULL DEFAULT 33.00, -- Passing Percentage (e.g. 33.00 means 33%)
    `allow_multiple_attempts` TINYINT(1) NOT NULL DEFAULT 0, -- Allow Multiple Attempts (If this will be 1 then students can attempt the quest multiple times)
    `max_attempts` TINYINT UNSIGNED NOT NULL DEFAULT 1, -- Max Attempts Allowed (If Allow Multiple Attempts is 1 then this will be the max attempts allowed for the quest)
