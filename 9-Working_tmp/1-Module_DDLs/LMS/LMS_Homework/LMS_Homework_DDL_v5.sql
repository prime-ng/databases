...
CREATE TABLE IF NOT EXISTS `lms_homework` (
    ...
    `hw_attachment_media_id` Json UNSIGNED DEFAULT NULL,   -- FK to sys_media (Handwritten scan)
    ...
    `allow_late_submission`    TINYINT(1) NOT NULL DEFAULT 0, -- Default late-submission policy (0 = deny, 1 = allow). Can be overridden per-student in lms_homework_assignment
	`realease_condition` ENUM('IMMEDIATE', 'ON_TOPIC_COMPLETE', 'ON_SCHEDULED_DATE') DEFAULT 'ON_TOPIC_COMPLETE', -- Auto-Release Logic
...
--   7. When teacher marks the linked schedule_id topic as completed, app sets is_released=1 for all matching
--   8. `schedule_id` -- Used when release_condition = ON_TOPIC_COMPLETE. system watches this schedule entry; 
--      when teacher marks the topic as completed, homework is auto-released.
--   9. Auto-Release Logic 
...
CREATE TABLE IF NOT EXISTS `lms_homework_assignment` (
    ...
    `release_condition`    ENUM('IMMEDIATE', 'ON_TOPIC_COMPLETE', 'ON_SCHEDULED_DATE') DEFAULT 'ON_TOPIC_COMPLETE', -- Auto-Release Logic
...
--   7.  Late Submission Override (per-student)
...
--   9.  EXEMPTED → Student exempted from this homework (e.g. was absent on topic day)
...
CREATE TABLE IF NOT EXISTS `lms_homework_submissions` (
    ...
    `sub_attachment_media_id`     JSON DEFAULT NULL,          -- Array of sys_media IDs for uploaded files e.g. [{"media_id": 12, "file_name": "hw1.pdf"}, ...]
    -- Submission Timing
    ...
    `resubmission_count`   TINYINT UNSIGNED NOT NULL DEFAULT 0, -- 0 = first submission, 1 = first resubmission, etc. Incremented each time teacher rejects and student resubmits
    -- Evaluation (filled by teacher)
    `status_id`            INT UNSIGNED NOT NULL,      -- FK to sys_dropdown_table.id (SUBMITTED | UNDER_REVIEW | GRADED | REJECTED | RESUBMIT_REQUESTED)
    `is_resubmission_requested`   INT UNSIGNED NOT NULL, 
    ...
    --`teacher_feedback_media_id` JSON DEFAULT NULL,   -- Removed as we will use `sub_attachment_media_id` for both student attachments and teacher feedback files.
    `graded_by`            INT UNSIGNED DEFAULT NULL,  -- FK to sys_users.id (teacher who graded)
    `graded_at`            DATETIME DEFAULT NULL,
    -- Score Publishing
    `score_published_at`   DATETIME DEFAULT NULL,      -- When score was made visible to student. Set by app when auto_publish_score=1 (on grading) or manually when teacher clicks "Publish Score"
    -- Audit
    `is_active`            TINYINT(1) NOT NULL DEFAULT 1,
...
--   5. `attachments_json` - Replaces single attachment_media_id from v2
