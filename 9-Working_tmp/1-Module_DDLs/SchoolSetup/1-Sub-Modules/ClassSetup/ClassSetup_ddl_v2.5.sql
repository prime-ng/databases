...
CREATE TABLE IF NOT EXISTS `sch_class_section_jnt` (
  ...
  CONSTRAINT `fk_classSection_classHouseRoomeId` FOREIGN KEY (`class_house_room_id`) REFERENCES `sch_rooms` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Must have a Record with `is_system` = 1, `code` = 'FREE', `short_name` = 'FREE PERIOD', and `name` = 'FREE PERIOD' as it will be used as default study format in case no period for that subject.
...
CREATE TABLE IF NOT EXISTS `sch_class_group_subject_options_jnt` (
  `id` INT unsigned NOT NULL AUTO_INCREMENT,
  `class_group_id` INT unsigned NOT NULL,       -- FK to 'sch_class_groups_jnt'
  `subject_studyformat_option_id` INT unsigned NOT NULL,    -- FK to 'sch_subject_options'
  `subj_Option_required_weekly_periods` TINYINT UNSIGNED NOT NULL DEFAULT 1,   -- Total periods required per week for this Class Group (Class+{Section}+Subject+StudyFormat)
  -- only to achive Denormalization for easy access in timetable, below fields are added which are already present in sch_subject_study_format_options table
  `class_id` int unsigned NOT NULL,             -- FK to 'sch_classes'
  `section_id` int unsigned NULL,               -- FK to 'sch_sections' (Optional)
  `subject_study_format_id` int unsigned NOT NULL,           -- FK to 'sch_subject_study_format_jnt'
  `subject_studyformat_option_code` varchar(20) NOT NULL,   -- e.g. 'CS','PE','FA' etc. (Added new). (Denormalization for easy access in timetable)
  `subject_studyformat_option_name` varchar(50) NOT NULL,   -- e.g. 'Computer Science', 'Physical Education', 'Fine Arts' etc. (Denormalization for easy access in timetable)
  `subj_required_weekly_periods` TINYINT UNSIGNED NOT NULL DEFAULT 1,   -- Total periods required per week for this Class Group (Class+{Section}+Subject+StudyFormat)
  `subj_option_required_weekly_periods` TINYINT UNSIGNED NOT NULL DEFAULT 1,   -- Total periods required per week for this Class Group (Class+{Section}+Subject+StudyFormat)
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  `deleted_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_clsGrp_subjOpt_clsSecSubjStdFormat` (`class_id`,`section_id`,`subject_study_format_id`,`subject_studyformat_option_id`),
  CONSTRAINT `fk_clsGrp_subjOpt_classGroupId` FOREIGN KEY (`class_group_id`) REFERENCES `sch_class_groups_jnt` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_clsGrp_subjOpt_subjOptionId` FOREIGN KEY (`subject_studyformat_option_id`) REFERENCES `sch_subject_study_format_options` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_clsGrp_subjOpt_classId` FOREIGN KEY (`class_id`) REFERENCES `sch_classes` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_clsGrp_subjOpt_sectionId` FOREIGN KEY (`section_id`) REFERENCES `sch_sections` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_clsGrp_subjOpt_subjectStudyFormatId` FOREIGN KEY (`subject_study_format_id`) REFERENCES `sch_subject_study_format_jnt` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
--
-- Added New Table: `sch_class_group_subject_options_jnt`
--
-- Table: `sch_class_groups_jnt`
-- Added Fields - `has_multiple_options` tinyint(1) NOT NULL DEFAULT '0', Fethc =h value from sch_subject_study_format_jnt.has_multiple_options