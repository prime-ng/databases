...
--
-- PURPOSE:
--   Provides a configurable system for schools to assign which visual template (tmp_templates) to use for each output purpose (Marksheet Printing,
--   Student ID Card, Staff ID Card, etc.), per class group, per class, or school-wide. Bridges the generic Template module with all consuming
...
--   D-TMP-001: Cross-module FK to msh_class_groups.id is acceptable. The class grouping concept (Primary, Secondary) is identical between MSG and Template output. 
--              Creating a duplicate sch_class_groups table would cause data drift. Template only READs msh_class_groups — no write coupling. If more modules need 
--              this grouping in future, promote msh_class_groups to sch_class_groups via migration.
--
--   D-TMP-002: tmp_template_purposes is separate from tmp_templates.type. The existing type column is free-text VARCHAR(255) with no validation, no uniqueness, 
--              and is nullable. It cannot reliably enforce purpose-based assignment. The type column continues as a user-friendly categorization label; 
--              tmp_template_purposes provides the system-enforced purpose registry.
--
--   D-TMP-003: Uniqueness enforced via generated scope_hash column. MySQL treats NULLs as distinct in UNIQUE indexes, so a composite UNIQUE on (purpose_id, 
--              session_id, class_id, class_group_id) would allow duplicate school-wide assignments (both NULLs). The scope_hash column generates a deterministic
--              string "PURPOSE:SESSION:C{id}|G{id}|SCHOOL" enabling a single UNIQUE index that covers all three scope types.
--
--   D-TMP-004: No FK coupling to msh_config_templates. Template visual assignment (HOW it looks) and marksheet computation config (WHAT scores are computed) 
--              are separate concerns. Resolved independently and combined at render time. See Resolution Logic below.
--
...
CREATE TABLE IF NOT EXISTS `tmp_templates_variables_mapping` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(50) NOT NULL,
    `description` VARCHAR(255) DEFAULT NULL,
