...
# 10. `lib_curricular_alignment` — How to Determine Each Counter Field

> Addendum requested separately for `Library_ddl_v6.sql` → `lib_curricular_alignment`
> (`student_usage_count`, `exam_reference_count`, `assignment_citations`). Full detail
> (including the populating stored procedure) is in §5.5 — this section restates the
> determination logic for each field individually for quick reference.

## 10.1 `student_usage_count`
**Type:** Scheduled Job (Weekly/Termly) — populated by `calculate_curricular_alignment` (see §5.5).

**Determination:** Count of `lib_transactions` (book issue records) for `ca.book_id`, where
the borrowing member is a **Student** currently enrolled in `ca.class_id`, and the issue
happened within `ca.academic_year_id`.
...
SELECT COUNT(*) FROM lib_transactions t
  ...
  INNER JOIN lib_members m     ON m.id = t.member_id
  INNER JOIN sys_users u       ON u.id = m.user_id
  -- cross-module: student's class for the given academic year
  INNER JOIN sch_student_enrollments se ON se.user_id = u.id
                                        AND se.academic_year_id = ca.academic_year_id
 WHERE c.book_id   = ca.book_id
   AND se.class_id = ca.class_id
   AND m.user_type = 'Student'
   AND t.issue_date BETWEEN (SELECT start_date FROM academic_years WHERE id = ca.academic_year_id)
                         AND (SELECT end_date   FROM academic_years WHERE id = ca.academic_year_id);
```

- Source tables (all present in `Library_ddl_v6.sql`): `lib_transactions`, `lib_book_copies`, `lib_members`.
- Cross-module table required: `sch_student_enrollments` (SchoolSetup/Student module) — links a `sys_users` row to a `class_id` for a given `academic_year_id`. Not part of the Library DDL.
- `ca.subject_id` is not used to filter `lib_transactions` directly (the transaction table has no `subject_id`); the subject linkage comes from the `lib_curricular_alignment` row itself (one row per class+subject+book combination), so `student_usage_count` reflects "usage by students of this class" for that book, repeated per subject row if the same book is mapped to multiple subjects for the class.

## 10.2 `exam_reference_count`
**Type:** Cross-module — App Real-time increment (preferred) or Scheduled Job recount.

**Determination:** Count of how many times `ca.book_id` is tagged/referenced as a source/reference
material on exam questions belonging to `ca.subject_id` / `ca.class_id` / `ca.academic_year_id`.

- `Library_ddl_v6.sql` has **no native table** for this — there is no exam/question-bank table
  in the Library schema. This is a **cross-module dependency** on the LMS Exam / Question Bank
  module (e.g. a `qst_questions` or `exm_*` table with a `reference_book_id` /
  `resource_id` column pointing back to `lib_books_master.id`).
- **Recommended implementation:**
  1. When a faculty member tags an exam question with a library resource (in the Exam/Question
     Bank module UI), write that link to the cross-module mapping table
     (e.g. `qst_question_resource_jnt(question_id, book_id)`).
  2. `calculate_curricular_alignment` recounts on its weekly/termly run:
     ```sql
     UPDATE lib_curricular_alignment ca
        SET ca.exam_reference_count = (
              SELECT COUNT(*) FROM qst_question_resource_jnt qrj
                INNER JOIN qst_questions q ON q.id = qrj.question_id
               WHERE qrj.book_id    = ca.book_id
                 AND q.subject_id   = ca.subject_id
                 AND q.class_id     = ca.class_id
                 AND q.academic_year_id = ca.academic_year_id
            )
      WHERE ca.academic_year_id = :p_academic_year_id;
     ```
  3. Until the cross-module mapping table exists, `exam_reference_count` stays at its DDL
     default of `0` (per Implementation Note 5 above).

## 10.3 `assignment_citations`
**Type:** Cross-module — App Real-time increment (preferred) or Scheduled Job recount.

**Determination:** Count of how many times `ca.book_id` is cited as a reference/source in
student Assignment/Homework submissions for `ca.subject_id` / `ca.class_id` / `ca.academic_year_id`.

- Like §10.2, `Library_ddl_v6.sql` has **no native table** for assignments — this depends on
  the Assignment/Homework module (e.g. `hmw_submissions` or `hmw_submission_citations` with a
  `book_id` FK to `lib_books_master.id`).
- **Recommended implementation:**
  1. When a student (or teacher, on review) cites a library book as a reference in a
     homework/assignment submission, write a row to the cross-module citation table
     (e.g. `hmw_submission_citation_jnt(submission_id, book_id)`).
  2. `calculate_curricular_alignment` recounts on its weekly/termly run:
     ```sql
     UPDATE lib_curricular_alignment ca
        SET ca.assignment_citations = (
              SELECT COUNT(*) FROM hmw_submission_citation_jnt scj
                INNER JOIN hmw_submissions s ON s.id = scj.submission_id
               WHERE scj.book_id      = ca.book_id
                 AND s.subject_id     = ca.subject_id
                 AND s.class_id       = ca.class_id
                 AND s.academic_year_id = ca.academic_year_id
            )
      WHERE ca.academic_year_id = :p_academic_year_id;
     ```
  3. Until the cross-module citation table exists, `assignment_citations` stays at its DDL
     default of `0` (per Implementation Note 5 above).

## 10.4 Net Effect on `alignment_score`
Both `exam_reference_count` and `assignment_citations` feed `alignment_score`
(§5.5 formula, weighted 15% each). Until the Exam and Assignment cross-module mappings are
built, `alignment_score` is effectively driven only by `student_usage_count` (40%) and
`faculty_rating` (30%), re-normalized — i.e. those two terms evaluate to `0` and do not error
(the formula uses `NULLIF(...,0)` guards so division by zero is avoided).
