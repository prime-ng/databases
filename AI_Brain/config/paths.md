...
```
WORK_OUTPUT_DEFAULT = {OLD_REPO}
```

> **Rule:** All Claude work output (analysis, prompts, context files, gap reports, etc.)
> goes into `{OLD_REPO}` by default. Only store in `{DB_REPO}` when explicitly told.
...
MODULE_DDLS    = {DB_REPO}/1-Module_DDLs
...
```
LIFECYCLE_BLUEPRINT = {DESIGN_ARCH}/Development_Lifecycle_Blueprint_v2.md
HPC_GAP_ANALYSIS    = {HPC_CONTEXT}/HPC_Gap_Analysis_Complete.md
RBS_MAPPING         = {RBS_DIR}/PrimeAI_RBS_Menu_Mapping_v2.0.md
GAP_ANALYSIS_V1     = {GAP_ANALYSIS}/PrimeAI_Gap_Analysis_v1.0.md
