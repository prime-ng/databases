...
> **Last Phase-2 deep audit:** 2026-04-09 — 6-agent parallel re-audit of modules that changed since 2026-04-02 (StudentPortal, Inventory, LmsExam/Quiz/Homework/Quests, StudentProfile Leave, Admission+Vendor, Hpc). Findings in `lessons/known-issues.md` section "Deep Audit — 2026-04-09 (Phase 2)".
...
| Module | Code Exists | Completion | Key Gaps (updated 2026-04-02 deep audit) |
|--------|------------|------------|----------|
| **SchoolSetup** | 41 ctrl, 42 mdl, 0 svc, 27 req | **~40%** | is_super_admin writable via UserController + in User $fillable + in UserRequest; SchoolSetupController all stubs; 3 routes → missing methods (trashedClassSubgroup, assignSubjects, StudyFormat); 5 backup controller files; student routes all commented out; rand() returns fake dashboard data; $request->all() in OrganizationController/GroupController. **NEW (2026-04-08):** Employee Leave Management DDL designed — `Employee_setup_ddl_v2.sql` (8 new `sch_*` tables: approval policies, multi-level approvers, leave applications, approval trail, documents, remarks, balance ledger). Code not yet implemented. **NEW (2026-05-04, D33):** `Employee_setup_ddl_v4.sql` — 25 tables (was 13). Fixed 3 CREATE-time bugs (attendance COMMENT quote, competancy typo, broken UNIQUE on profile). Added 12 new tables (addresses, emergency contacts, bank details, documents, role-history audit, separations workflow, leave-types master, leave-config, holidays, shifts, shift assignments, attendance punches, attendance corrections). Added 19 personal/identity/employment fields to `sch_employees`. Resolved dangling FKs to `sch_leave_types` and `sch_leave_config`. Strictly additive — no renames or type changes. Code not yet implemented. **NEW (2026-05-07, D35):** PTM (Parent-Teacher Meeting) sub-module DDL v2 — `1-DDL_Tenant_Modules/2-SchoolSetup/PTM/Ptm_Setup_ddl_v2.sql`. 10 tables (`ptm_*`): events + event_settings + event_class_section_jnt + batches + batch_slot_definitions + assignments + assignment_teacher_jnt + blockouts + slots + slot_bookings. Convention rebuild over v1 stub (added InnoDB/utf8mb4, audit columns, FKs to `sch_class_section_jnt` / `sch_academic_sessions` / `sch_academic_terms` / `sys_users` / `std_students`). Supports both SCHOOL_ALLOCATED and PARENT_PICK allocation modes, multi-teacher splits, teacher blockouts, group slots (capacity > 1), cross-class no-double-booking enforced at DB via UNIQUE `(teacher_id, slot_start)` on `ptm_slots`, "1 confirmed booking per student per teacher per event" enforced via VIRTUAL generated column `active_booking_key`. Code not yet implemented. |
| **Feedback (NEW — 2026-04-09)** | 0 ctrl, 0 mdl, 0 svc, 0 req | **DDL only** | Dedicated module `39-Feedback/` with `fbk_*` prefix (11 tenant tables). Supports: Student/Parent → Teacher, NEP 2020 Teacher → Student, NEP 2020 Student → Peer, Student/Parent → non-teaching staff (Transport/Canteen/Library/Hostel/Security/Coach), Admin → Teacher, Teacher 360°, Self-Reflection. Polymorphic target (user/student/employee/department) with generated `_uq` columns for dedup. DDL files: `StudentFeedback_ddl_v1.sql` (Teacher-only; superseded) → `StudentFeedback_ddl_v2.sql` (generic; superseded) → **`{DB_REPO}/1-Module_DDLs/Feedback/StudentFeedback_ddl_v3.sql` (current, 2026-04-09)** — all 13 ENUMs rebuilt on `sys_dropdown_table` FKs per D29; 12 became dropdown FKs, 1 (target_population_mode) became `is_auto_populated_targets` TINYINT(1); SECTION 0 of v3 lists the full `sys_dropdown_needs` + `sys_dropdown_table` seed manifest. Code not yet implemented. |
...
|--------|------------|------------|-------|
| **Admission** | 15 ctrl, 20 mdl, 6 svc, 20 req, 46 views, 220 route lines | **~35%** (2026-04-09 Phase 2, ↑15) | **MAJOR IMPROVEMENT verified 2026-04-09:** All 15 controllers now have explicit `Gate::authorize()` calls — service-layer-only auth replaced with defense-in-depth. All 3 new FormRequests (StoreBehaviorActionRequest, StoreBehaviorIncidentRequest, UpdateBehaviorIncidentRequest) properly DI-injected into AlumniController — none orphaned. EnquiryController::index() + ApplicationController::index() use proper `with()` eager loading — no N+1. AdmissionPipelineService has real FSM+fee logic. **Only remaining issue:** BUG-ADM-003 — AdmissionPipelineService.php:73 still references `$application->cycle_id` (old attribute) — missed from the cycle_id→admission_cycle_id rename commits (1bbf9d56e, ae38c3c4e). All 15 controllers routed, no unrouted controllers. Prompt ready. |
| **Cafeteria** | 16 ctrl, 21 mdl, 6 svc, 16 req, 62 views, 169 route lines | **~18%** | OrderController has real logic via OrderService. **Student IDOR in apiIndex()** — student_id from query string unverified. No Gate. Prompt ready. |
| **Certificate** | 9 ctrl, 10 mdl, 3 svc, 10 req, 36 views, 1 job, 134 route lines | **~15%** | Scaffold stubs only. No real implementation. Prompt ready. |
| **FrontOffice** | 20 ctrl, 22 mdl, 4 svc, 3 req, 78 views, 1 job, 223 route lines | **~12%** | Scaffold stubs only. Only 3 FormRequests for 20 controllers. Prompt ready. |
| **HrStaff** | 22 ctrl, 33 mdl, 15 svc, 23 req, 92 views, 240 route lines | **~20%** | PayrollController well-implemented with Gate + FormRequests + service layer. **Arbitrary column override risk via field_name** in override(). Prompt ready. |
| **Inventory** | 20 ctrl, 28 mdl, 14 svc, 18 req, 68 views, 1 job, 212 route lines | **~22%** | Services doubled since 2026-04-02 (7→14) — added Asset, Godown, GrnPosting, InventoryReport, PurchaseOrder, PurchaseRequisition, Quotation, RateContract, ReorderAlert, StockAdjustment, StockGroup, StockIssue, StockLedger, StockValuation. +5 FormRequests, +17 views, +36 route lines. Deep audit of new services not yet run. Prompt ready. |
