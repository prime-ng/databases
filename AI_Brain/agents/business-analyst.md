...
- [2026-06-30] All modules: `EnsureTenantHasModule` is missing from 100% of route groups (13/13 confirmed in a single-day multi-module batch). Do not re-discover per module — mark it confirmed in the Gap Analysis and move on to module-specific gaps.
- [2026-06-30] SyllabusBooks: Grep for `Modules\Prime\Models\AcademicSession` across all tenant controllers before closing any gap analysis — this cross-layer import is a P0 data-isolation breach and is invisible from FRD inspection alone. Command: `grep -r "Modules\\\\Prime\\\\Models\\\\AcademicSession" Modules/`.
- [2026-06-30] QuestionBank / TimetableFoundation: After any D22 routes-and-policies migration, verify that every Policy class in `Modules/{Module}/app/Policies/` has exactly one `Gate::policy()` call in the module's ServiceProvider. Duplicate or missing registrations are silent — no error, no warning. A dead policy means auth passes unconditionally for that resource.
- [2026-06-30] Vendor: PII fields (PAN, bank account) must be flagged in the Data Dictionary privacy column as "Sensitive (PII)" and cross-referenced in the NFR Catalog under Security. The BA's job is to flag these in Section 5 (Data Requirements) and Section 9.2 (NFR Security) — the Technical Auditor then confirms whether encryption is applied.
- [2026-06-30] All modules: A module's CLAUDE.md label ("Pending", "100%") is frequently stale. The canonical status source is the BA Complete Analysis Pack completion % field + the module-knowledge file. When you see a discrepancy, note it explicitly and update modules-map.md and MEMORY.md — do not trust CLAUDE.md as a status oracle.
