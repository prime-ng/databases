...
7. **Spatie permission checks at BOTH levels:**
   > 📄 **Full Gold-Standard Guide**: See `rules/permission-rules.md` for naming conventions, Blade patterns & Controller Gate examples.
   - Route middleware: `->middleware('permission:manage-students')`
   - Controller/Policy: `Gate::authorize('tenant.<module-slug>.<action>')` — use `Gate::authorize` over `$this->authorize()` for consistency.
   - Blade Views: `@can('tenant.<slug>.<action>') ... @endcan` and `@canany([...]) ... @endcanany`
   - Permission key format: `tenant.<module-slug>.<action>` (e.g. `tenant.vendor.forceDelete`)
...
10a. **Never add Privilege Elevation fields to `$fillable` on any models (e.g., User).** Fields like `is_super_admin`, `super_admin_flag`, `password`, or critical RBAC rates must be assigned explicitly (e.g., `$user->is_super_admin = true`) to prevent mass-assignment privilege escalation vulnerabilities.
